...
namespace game
{
	...
	static const playermodelinfo playermodels[5] = {
		...
		{"snoutx10k",
		 ...
		 NULL,
		 "snoutx10k/wings",
		 {"snoutx10k/armor/blue", "snoutx10k/armor/green", "snoutx10k/armor/yellow"},
		 "snoutx10k",
		 "snoutx10k_blue",
		 "snoutx10k_red",
		 true},
		//{ "ogro/green", "ogro/blue", "ogro/red", "mrfixit/hudguns", "ogro/vwep", NULL, { NULL, NULL, NULL }, "ogro",
		//"ogro_blue", "ogro_red", false },
		{"ogro2",
		 "ogro2/blue",
		 "ogro2/red",
		 "mrfixit/hudguns",
		 NULL,
		 "ogro2/quad",
		 {"ogro2/armor/blue", "ogro2/armor/green", "ogro2/armor/yellow"},
		 "ogro",
		 "ogro_blue",
		 "ogro_red",
		 true},
		{"inky",
		 ...
		 NULL,
		 "inky/quad",
		 {"inky/armor/blue", "inky/armor/green", "inky/armor/yellow"},
		 "inky",
		 "inky_blue",
		 "inky_red",
		 true},
		{"captaincannon",
		 ...
		 NULL,
		 "captaincannon/quad",
		 {"captaincannon/armor/blue", "captaincannon/armor/green", "captaincannon/armor/yellow"},
		 "captaincannon",
		 "captaincannon_blue",
		 "captaincannon_red",
		 true}};
	...
	const playermodelinfo &getplayermodelinfo(fpsent *d)
	{
		const playermodelinfo *mdl =
			getplayermodelinfo(d == player1 || forceplayermodels ? playermodel : d->playermodel);
		if (!mdl)
	...
	void preloadplayermodel()
	{
		loopi(sizeof(playermodels) / sizeof(playermodels[0]))
		{
			...
			if (m_teammode)
			{
				...
				preloadmodel(mdl->ffa);
			if (mdl->vwep)
				preloadmodel(mdl->vwep);
			if (mdl->quad)
	...
	void renderplayer(fpsent *d, const playermodelinfo &mdl, int team, float fade, bool mainpass)
	{
		int lastaction = d->lastaction,
			hold = mdl.vwep || d->gunselect == GUN_PISTOL ? 0 : (ANIM_HOLD1 + d->gunselect) | ANIM_LOOP,
			attack = ANIM_ATTACK1 + d->gunselect, delay = mdl.vwep ? 300 : guns[d->gunselect].attackdelay + 50;
		if (intermission && d->state != CS_DEAD)
		{
			lastaction = 0;
			hold = attack = ANIM_LOSE | ANIM_LOOP;
			delay = 0;
			if (m_teammode ? bestteams.htfind(d->team) >= 0 : bestplayers.find(d) >= 0)
		...
		modelattach a[5];
		static const char *const vweps[] = {"vwep/fist",  "vwep/shotg", "vwep/chaing", "vwep/rocket",
											"vwep/rifle", "vwep/gl",	"vwep/pistol"};
		int ai = 0;
		...
		{
			case 1:
				mdlname = mdl.blueteam;
				break;
			case 2:
				mdlname = mdl.redteam;
				break;
		}
		renderclient(d, mdlname, a[0].tag ? a : NULL, hold, attack, delay, lastaction,
					 intermission && d->state != CS_DEAD ? 0 : d->lastpain, fade, ragdoll && mdl.ragdoll);
#if 0
	...
	void renderstatusicons(fpsent *d, int team, float yoffset)
	{
		...
		if (statusicons && (d->state == CS_ALIVE || d->state == CS_LAGGED))
		{
			if (d->quadmillis)
				icons++;
			if (d->maxhealth > 100)
				icons += (min(d->maxhealth, boost.max) - 100 + boost.info - 1) / boost.info;
	...
	{
		if (!statusbars || m_insta || (player1->state == CS_SPECTATOR ? statusbars <= 1 : team != 1) ||
			(d->state != CS_ALIVE && d->state != CS_LAGGED))
			return 0;
		...
		int color =
			d->health <= 25 ? 0xFF0000 : (d->health <= 50 ? 0xFF8000 : (d->health <= 100 ? 0x40FF80 : 0x40C0FF));
	...
	void rendergame(bool mainpass)
	{
		...
		loopv(players)
		{
			...
			if (d == player1 || d->state == CS_SPECTATOR || d->state == CS_SPAWNING || d->lifesequence < 0 ||
				...
				continue;
			int team = 0;
			if (teamskins || m_teammode)
				team = isteam(player1->team, d->team) ? 1 : 2;
		...
		if (isthirdperson() && !followingplayer() && (player1->state != CS_DEAD || hidedead != 1))
			renderplayer(player1, getplayermodelinfo(player1), teamskins || m_teammode ? 1 : 0, 1, mainpass);
		rendermonsters();
		rendermovables();
	...
	void drawhudmodel(fpsent *d, int anim, float speed = 0, int base = 0)
	{
		...
		rendermodel(NULL, gunname, anim, sway, testhudgun ? 0 : d->yaw + 90, testhudgun ? 0 : d->pitch,
					MDL_LIGHT | MDL_HUD, interp, a, base, (int)ceil(speed));
		if (d->muzzle.x >= 0)
