...
namespace entities
{
	...
	{
		static const char *const entmdlnames[] = {NULL,
												  NULL,
												  NULL,
												  NULL,
												  NULL,
		...
		loopv(ents)
		{
			...
			switch (e.type)
			{
				...
				case TELEPORT:
					...
					break;
				default:
					if (!e.spawned() || e.type < I_SHELLS || e.type > I_QUAD)
						continue;
			}
			...
			if (mdlname)
			{
				...
				rendermodel(&e.light, mdlname, ANIM_MAPMODEL | ANIM_LOOP, p, lastmillis / (float)revs, 0,
	...
	void teleporteffects(fpsent *d, int tp, int td, bool local)
	{
		if (ents.inrange(tp) && ents[tp]->type == TELEPORT)
		{
			...
			if (e.attr4 >= 0)
			{
				...
				if (d != h && ents.inrange(td) && ents[td]->type == TELEDEST)
	...
	void trypickup(int n, fpsent *d)
	{
		...
		switch (e->type)
		{
			...
			case TELEPORT: {
				if (d->lastpickup == e->type && lastmillis - d->lastpickupmillis < 500)
					break;
				if (e->attr3 > 0)
				{
			...
			case RESPAWNPOINT:
				if (!m_classicsp || d != player1 || n == respawnent)
					break;
				respawnent = n;
			...
			case JUMPPAD: {
				if (d->lastpickup == e->type && lastmillis - d->lastpickupmillis < 300)
					break;
				d->lastpickup = e->type;
				d->lastpickupmillis = lastmillis;
	...
	void checkitems(fpsent *d)
	{
		...
		loopv(ents)
		{
			...
			if (e.type == NOTUSED)
				continue;
			if ((!e.spawned() || e.nopickup()) && e.type != TELEPORT && e.type != JUMPPAD && e.type != RESPAWNPOINT)
				continue;
	...
	void checkquad(int time, fpsent *d)
	{
		if (d->quadmillis && (d->quadmillis -= time) <= 0)
		{
			...
			if (d == h)
				conoutf(CON_GAMEINFO, "\f2quad damage is over");
		}
	}
	...
	void putitems(packetbuf &p) // puts items in network stream and also spawns them locally
	{
		...
		loopv(ents) if (ents[i]->type >= I_SHELLS && ents[i]->type <= I_QUAD &&
						(!m_noammo || ents[i]->type < I_SHELLS || ents[i]->type > I_CARTRIDGES))
		{
			putint(p, i);
	...
	static inline void setuptriggerflags(fpsentity &e)
	{
		cleartriggerflags(e);
		e.flags |= EF_ANIM;
		if (checktriggertype(e.attr3, TRIG_COLLIDE | TRIG_DISAPPEAR))
			e.flags |= EF_NOSHADOW;
		if (!checktriggertype(e.attr3, TRIG_COLLIDE))
			e.flags |= EF_NOCOLLIDE;
		switch (e.triggerstate)
		{
			case TRIGGERING:
				if (checktriggertype(e.attr3, TRIG_COLLIDE) && lastmillis - e.lasttrigger >= 500)
					e.flags |= EF_NOCOLLIDE;
				break;
			case TRIGGERED:
				if (checktriggertype(e.attr3, TRIG_COLLIDE))
	...
	void unlocktriggers(int tag, int oldstate = TRIGGER_RESET, int newstate = TRIGGERING)
	{
		loopv(ents)
		{
			...
			{
				if (newstate == TRIGGER_RESETTING && checktriggertype(e.attr3, TRIG_COLLIDE) && overlapsdynent(e.o, 20))
					continue;
				e.triggerstate = newstate;
				e.lasttrigger = lastmillis;
				if (checktriggertype(e.attr3, TRIG_RUMBLE))
	...
	ICOMMAND(trigger, "ii", (int *tag, int *state), {
		if (*state)
			...
			unlocktriggers(*tag, TRIGGERED, TRIGGER_RESETTING);
	});
			...
			switch (e.triggerstate)
			{
				...
				case TRIGGER_RESETTING:
					if (lastmillis - e.lasttrigger >= 1000)
					{
						if (e.attr4)
						{
							if (e.triggerstate == TRIGGERING)
								unlocktriggers(e.attr4);
							else
								unlocktriggers(e.attr4, TRIGGERED, TRIGGER_RESETTING);
						}
						if (checktriggertype(e.attr3, TRIG_DISAPPEAR))
							e.triggerstate = TRIGGER_DISAPPEARED;
						else if (e.triggerstate == TRIGGERING && checktriggertype(e.attr3, TRIG_TOGGLE))
							e.triggerstate = TRIGGERED;
						else
							e.triggerstate = TRIGGER_RESET;
					}
					setuptriggerflags(e);
				...
				case TRIGGER_RESET:
					if (e.lasttrigger)
					{
						if (checktriggertype(e.attr3, TRIG_AUTO_RESET | TRIG_MANY | TRIG_LOCKED) &&
							e.o.dist(o) - player1->radius >= (checktriggertype(e.attr3, TRIG_COLLIDE) ? 20 : 12))
							e.lasttrigger = 0;
						break;
					}
					else if (e.o.dist(o) - player1->radius >= (checktriggertype(e.attr3, TRIG_COLLIDE) ? 20 : 12))
						break;
					...
					if (checktriggertype(e.attr3, TRIG_RUMBLE))
						playsound(S_RUMBLE, &e.o);
					if (checktriggertype(e.attr3, TRIG_ENDSP))
						endsp(false);
					if (e.attr4)
						doleveltrigger(e.attr4, 1);
					break;
				case TRIGGERED:
					...
					{
						if (lastmillis - e.lasttrigger < 6000)
							break;
					}
					else if (checktriggertype(e.attr3, TRIG_MANY))
					{
						...
						break;
					if (checktriggertype(e.attr3, TRIG_COLLIDE) && overlapsdynent(e.o, 20))
						break;
	...
	void animatemapmodel(const extentity &e, int &anim, int &basetime)
	{
		...
		if (validtrigger(f.attr3))
			switch (f.triggerstate)
			{
				case TRIGGER_RESET:
	...
	void entradius(extentity &e, bool color)
	{
		switch (e.type)
		{
			...
			case JUMPPAD:
				renderentarrow(
					e, vec((int)(char)e.attr3 * 10.0f, (int)(char)e.attr2 * 10.0f, e.attr1 * 12.5f).normalize(), 4);
				break;
			...
			case MAPMODEL:
				if (validtrigger(e.attr3))
					renderentring(e, checktriggertype(e.attr3, TRIG_COLLIDE) ? 20 : 12);
				break;
		}
	}
	...
	void editent(int i, bool local)
	{
		...
		if (e.type == ET_MAPMODEL && validtrigger(e.attr3))
		{
			...
			cleartriggerflags(e);
		if (local)
			addmsg(N_EDITENT, "rii3ii5", i, (int)(e.o.x * DMF), (int)(e.o.y * DMF), (int)(e.o.z * DMF), e.type, e.attr1,
				   e.attr2, e.attr3, e.attr4, e.attr5);
	}

	float dropheight(entity &e)
	{
		if (e.type == BASE || e.type == FLAG)
			return 0.0f;
		return 4.0f;
	}
#endif
