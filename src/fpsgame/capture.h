	...
	#endif
{
	...
	struct baseinfo
	{
		...
		bool valid() const
		{
			return ammotype > 0 && ammotype <= I_CARTRIDGES - I_SHELLS + 1;
		}
		...
		bool leave(const char *team)
		{
			...
			if (strcmp(enemy, team) || enemies <= 0)
				return false;
			enemies--;
			return !enemies;
		}
		...
		int occupy(const char *team, int units)
		{
			...
			else if (converted < (owner[0] ? int(OCCUPYENEMYLIMIT) : int(OCCUPYNEUTRALLIMIT)))
				return -1;
			if (owner[0])
			{
				owner[0] = '\0';
	...
	{
		if (m_regencapture || !autorepammo || d != player1 || d->state != CS_ALIVE)
			return;
		vec o = d->feetpos();
		loopv(bases)
		{
			...
			if (b.valid() && insidebase(b, d->feetpos()) && !strcmp(b.owner, d->team) && b.o.dist(o) < 12)
			{
				if (d->lastrepammo != i)
				{
					if (b.ammo > 0 && !player1->hasmaxammo(b.ammotype - 1 + I_SHELLS))
						addmsg(N_REPAMMO, "rc", d);
					d->lastrepammo = i;
				}
				return;
			}
	...
	void rendertether(fpsent *d)
	{
		...
		if (d->state == CS_ALIVE)
		{
			...
			{
				baseinfo &b = bases[i];
				if (!b.valid() || !insidebase(b, d->feetpos()) ||
					(strcmp(b.owner, d->team) && strcmp(b.enemy, d->team)))
					continue;
				if (d->lastbase < 0 && (lookupmaterial(d->feetpos()) & MATF_CLIP) == MAT_GAMECLIP)
					break;
				particle_flare(pos, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - AMMOHEIGHT - 4.4f), 0, PART_LIGHTNING,
							   strcmp(d->team, player1->team) ? 0xFF2222 : 0x2222FF, 1.0f);
				if (oldbase < 0)
				{
					particle_fireball(pos, 4.8f, PART_EXPLOSION, 250,
									  strcmp(d->team, player1->team) ? 0x802020 : 0x2020FF, 4.8f);
					particle_splash(PART_SPARK, 50, 250, pos, strcmp(d->team, player1->team) ? 0x802020 : 0x2020FF,
									0.24f);
				}
				d->lastbase = i;
			}
	...
	void rendergame()
	{
		...
		loopv(bases)
		{
			...
			if (!b.valid())
				continue;
			const char *basename =
				b.owner[0] ? (strcmp(b.owner, player1->team) ? "base/red" : "base/blue") : "base/neutral";
			rendermodel(&b.light, basename, ANIM_MAPMODEL | ANIM_LOOP, b.o, 0, 0,
						MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
			float fradius = 1.0f, fheight = 0.5f;
			regular_particle_flame(PART_FLAME, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - 4.5f), fradius, fheight,
								   b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 3,
								   2.0f);
			// regular_particle_flame(PART_SMOKE, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - 4.5f + 4.0f*min(fradius,
			// fheight)), fradius, fheight, 0x303020, 1, 4.0f, 100.0f, 2000.0f, -20);

			//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner,
			//            player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
			...
			if (m_regencapture)
			{
				...
				rendermodel(&b.light, ammoname, ANIM_MAPMODEL | ANIM_LOOP, ammopos, lastmillis / 10.0f, 0,
							MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
			}
			else
				loopj(b.ammo)
				{
					...
					ammopos.z += 4;
					rendermodel(&b.light, entities::entmdlname(I_SHELLS + b.ammotype - 1), ANIM_MAPMODEL | ANIM_LOOP,
								ammopos, 0, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED);
				}
			...
			if (b.owner[0])
			{
				...
				if (!b.name[0])
					formatstring(b.info, "base %d: %s", b.tag, b.owner);
				else if (basenumbers)
					formatstring(b.info, "%s (%d): %s", b.name, b.tag, b.owner);
				else
					formatstring(b.info, "%s: %s", b.name, b.owner);
				tcolor = isowner ? 0x6496FF : 0xFF4B19;
			}
			else if (b.enemy[0])
			{
				if (!b.name[0])
					formatstring(b.info, "base %d: %s", b.tag, b.enemy);
				else if (basenumbers)
					formatstring(b.info, "%s (%d): %s", b.name, b.tag, b.enemy);
				else
					formatstring(b.info, "%s: %s", b.name, b.enemy);
				if (strcmp(b.enemy, player1->team))
				{
					tcolor = 0xFF4B19;
					mtype = PART_METER;
					mcolor = 0xFF1932;
				...
				formatstring(b.info, "base %d", b.tag);
			else if (basenumbers)
				formatstring(b.info, "%s (%d)", b.name, b.tag);
			else
				copystring(b.info, b.name);
			...
			{
				above.z += 3.0f;
				particle_meter(above,
							   b.converted / float((b.owner[0] ? int(OCCUPYENEMYLIMIT) : int(OCCUPYNEUTRALLIMIT))),
							   mtype, 1, mcolor, mcolor2, 2.0f);
			}
		}
	}
	...
	void drawblips(fpsent *d, float blipsize, int fw, int fh, int type, bool skipenemy = false)
	{
		...
		loopv(bases)
		{
			...
			if (!b.valid())
				continue;
			if (skipenemy && b.enemy[0])
				continue;
			switch (type)
			{
				case 1:
					if (!b.owner[0] || strcmp(b.owner, player1->team))
						continue;
					break;
				case 0:
					if (b.owner[0])
				...
				case -1:
					...
					break;
				case -2:
					if (!b.enemy[0] || !strcmp(b.enemy, player1->team))
						continue;
					break;
			}
			vec dir(d->o);
	...
	void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
	{
		...
		if (owner[0])
		{
			...
			{
				if (!b.name[0])
					conoutf(CON_GAMEINFO, "%s captured base %d", teamcolor(owner, owner), b.tag);
				else if (basenumbers)
					conoutf(CON_GAMEINFO, "%s captured %s (%d)", teamcolor(owner, owner), b.name, b.tag);
				else
					conoutf(CON_GAMEINFO, "%s captured %s", teamcolor(owner, owner), b.name);
				if (!strcmp(owner, player1->team))
		...
		{
			if (!b.name[0])
				conoutf(CON_GAMEINFO, "%s lost base %d", teamcolor(b.owner, b.owner), b.tag);
			else if (basenumbers)
				conoutf(CON_GAMEINFO, "%s lost %s (%d)", teamcolor(b.owner, b.owner), b.name, b.tag);
			else
				conoutf(CON_GAMEINFO, "%s lost %s", teamcolor(b.owner, b.owner), b.name);
			if (!strcmp(b.owner, player1->team))
				playsound(S_V_BASELOST);
		}
		if (strcmp(b.owner, owner))
			particle_splash(PART_SPARK, 200, 250, b.ammopos,
							owner[0] ? (strcmp(owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 0.24f);
		copystring(b.owner, owner);
		copystring(b.enemy, enemy);
		b.converted = converted;
		if (ammo > b.ammo)
		{
			...
			if (icon >= 0)
				particle_icon(vec(b.ammopos.x, b.ammopos.y, b.ammopos.z + AMMOHEIGHT + 1.0f), icon % 4, icon / 4,
							  PART_HUD_ICON, 2000, 0xFFFFFF, 2.0f, -8);
		}
		b.ammo = ammo;
	}
	...
	void setscore(int base, const char *team, int total)
	{
		...
		else if (bases.inrange(base))
		{
			...
			if (!strcmp(b.owner, team))
			{
				...
				particle_textcopy(above, msg, PART_TEXT, 2000, isteam(team, player1->team) ? 0x6496FF : 0xFF4B19, 4.0f,
								  -8);
			}
		}
	}
	...
	float ratespawn(fpsent *d, const extentity &e)
	{
		...
		{
			baseinfo &b = bases[i];
			if (!b.owner[0] || strcmp(b.owner, d->team))
				continue;
	...
	void aifind(fpsent *d, ai::aistate &b, vector<ai::interest> &interests)
	{
		...
		loopvj(bases)
		{
			...
			if ((regen && f.owner[0] && !strcmp(f.owner, d->team)) ||
				(targets.empty() && (!f.owner[0] || strcmp(f.owner, d->team) || f.enemy[0])))
			{
	...
	bool aidefend(fpsent *d, ai::aistate &b)
	{
		...
		return ai::defend(d, b, f.o, float(CAPTURERADIUS), float(CAPTURERADIUS * (2 + (walk * 2))),
						  walk); // less wander than ctf
	}
...
ICOMMAND(insidebases, "", (), {
	vector<char> buf;
	if (m_capture && player1->state == CS_ALIVE)
	...
	{
		if (m_noitems || notgotbases || ci->state.state != CS_ALIVE || !ci->team[0])
			return;
		loopv(bases)
		{
			baseinfo &b = bases[i];
			if (b.valid() && insidebase(b, ci->state.o) && !ci->state.hasmaxammo(b.ammotype - 1 + I_SHELLS) &&
				b.takeammo(ci->team))
			{
				sendbaseinfo(i);
	...
	{
		if (!team[0] || gamemillis >= gamelimit)
			return;
		loopv(bases)
		{
			baseinfo &b = bases[i];
			if (!b.valid())
				continue;
			bool leave = !oldclip && insidebase(b, oldpos), enter = !newclip && insidebase(b, newpos);
			if (leave && !enter && b.leave(team))
				sendbaseinfo(i);
			else if (enter && !leave && b.enter(team))
				sendbaseinfo(i);
			else if (leave && enter && b.steal(team))
				stealbase(i, team);
		}
	}
		...
		loopv(clients)
		{
			...
			if (ci->state.state == CS_ALIVE && ci->team[0] && !strcmp(ci->team, b.owner) && insidebase(b, ci->state.o))
			{
				...
				if (ci->state.armourtype != A_GREEN || ci->state.armour < itemstats[I_GREENARMOUR - I_SHELLS].max)
				{
					...
					ci->state.armour =
						min(ci->state.armour + ticks * REGENARMOUR, itemstats[I_GREENARMOUR - I_SHELLS].max);
					notify = true;
				}
				if (b.valid())
				{
				...
				if (notify)
					sendf(-1, 1, "ri6", N_BASEREGEN, ci->clientnum, ci->state.health, ci->state.armour, b.ammotype,
						  b.valid() ? ci->state.ammo[b.ammotype] : 0);
			}
		}
	}
		...
		loopv(bases)
		{
			...
			if (b.enemy[0])
			{
				if (!b.owners || !b.enemies)
					b.occupy(b.enemy, OCCUPYBONUS * (b.enemies ? 1 : -1) +
										  OCCUPYPOINTS * (b.enemies ? b.enemies : -(1 + b.owners)) * t);
				sendbaseinfo(i);
			}
			else if (b.owner[0])
			{
	...
	{
		if (notgotbases || ci->state.state != CS_ALIVE || ci->gameclip)
			return;
	...
	{
		return m_regencapture || connecting || !ci->state.lastdeath ||
...
case N_BASES:
	if (smode == &capturemode)
		capturemode.parsebases(p, (ci->state.state != CS_SPECTATOR || ci->privilege || ci->local) &&
									  !strcmp(ci->clientmap, smapname));
	break;

case N_REPAMMO:
	if ((ci->state.state != CS_SPECTATOR || ci->local || ci->privilege) && cq && smode == &capturemode)
		capturemode.replenishammo(cq);
	break;
...
case N_BASEINFO: {
	...
	if (m_capture)
		capturemode.updatebase(base, owner, enemy, converted, ammo);
	break;
}
...
case N_BASEREGEN: {
	...
	if (regen && m_capture)
	{
		...
		regen->armour = armour;
		if (ammotype >= GUN_SG && ammotype <= GUN_PISTOL)
			regen->ammo[ammotype] = ammo;
	}
	break;
}
...
case N_BASESCORE: {
	...
	int total = getint(p);
	if (m_capture)
		capturemode.setscore(base, text, total);
	break;
}
...
case N_REPAMMO: {
	...
	if (r && m_capture)
		capturemode.receiveammo(r, ammotype);
	break;
}
