...
namespace game
{
	...
	int getweapon(const char *name)
	{
		...
		if (isdigit(name[0]))
			...
			loopi(sizeof(abbrevs) / sizeof(abbrevs[0])) if (!strcasecmp(abbrevs[i], name)) return i;
		return -1;
	}
	...
	void setweapon(const char *name, bool force = false)
	{
		...
		if (player1->state != CS_ALIVE || gun < GUN_FIST || gun > GUN_PISTOL)
			return;
		if (force || player1->ammo[gun])
			gunselect(gun, player1);
		else
	...
	void weaponswitch(fpsent *d)
	{
		if (d->state != CS_ALIVE)
			return;
		int s = d->gunselect;
		if (s != GUN_CG && d->ammo[GUN_CG])
			s = GUN_CG;
		else if (s != GUN_RL && d->ammo[GUN_RL])
			s = GUN_RL;
		else if (s != GUN_SG && d->ammo[GUN_SG])
			s = GUN_SG;
		else if (s != GUN_RIFLE && d->ammo[GUN_RIFLE])
			s = GUN_RIFLE;
		else if (s != GUN_GL && d->ammo[GUN_GL])
			s = GUN_GL;
		else if (s != GUN_PISTOL && d->ammo[GUN_PISTOL])
			s = GUN_PISTOL;
		else
			s = GUN_FIST;
	...
	ICOMMAND(weapon, "V", (tagval * args, int numargs), {
		...
		loopi(7)
		{
			...
			{
				int gun = getweapon(name);
				if (gun >= GUN_FIST && gun <= GUN_PISTOL && gun != player1->gunselect && player1->ammo[gun])
				{
					gunselect(gun, player1);
					return;
				}
	...
	void newbouncer(const vec &from, const vec &to, bool local, int id, fpsent *owner, int type, int lifetime,
					int speed, entitylight *light = NULL)
	{
		...
		if (type == BNC_GRENADE)
		{
			...
			if (owner == followingplayer(player1) && !isthirdperson())
				bnc.offset.sub(owner->o).rescale(16).add(owner->o);
		}
		else
			bnc.offset = from;
	...
	void bounced(physent *d, const vec &surface)
	{
		...
		if (b->bouncetype != BNC_GIBS || b->bounces >= 2)
			return;
		b->bounces++;
		adddecal(DECAL_BLOOD, vec(b->o).sub(vec(surface).mul(b->radius)), surface, 2.96f / b->bounces,
				 bvec(0x60, 0xFF, 0xFF), rnd(4));
	}
	...
	void hit(int damage, dynent *d, fpsent *at, const vec &vel, int gun, float info1, int info2 = 1)
	{
		...
		if (f->type == ENT_AI || !m_mp(gamemode) || f == at)
			f->hitpush(damage, vel, at, gun);

		if (f->type == ENT_AI)
			hitmonster(damage, (monster *)f, at, vel, gun);
	...
	void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun)
	{
		...
		particle_fireball(v, guns[gun].exprad, gun != GUN_GL ? PART_EXPLOSION : PART_EXPLOSION_BLUE,
						  gun != GUN_GL ? -1 : int((guns[gun].exprad - 4.0f) * 15), color, 4.0f);
		if (gun == GUN_RL)
			adddynlight(v, 1.15f * guns[gun].exprad, vec(2, 1.5f, 1), 700, 100, 0, guns[gun].exprad / 2,
						vec(1, 0.75f, 0.5f));
		else if (gun == GUN_GL)
			adddynlight(v, 1.15f * guns[gun].exprad, vec(0.5f, 1.5f, 2), 600, 100, 0, 8, vec(0.25f, 1, 1));
		else
			adddynlight(v, 1.15f * guns[gun].exprad, vec(2, 1.5f, 1), 700, 100);
		...
		loopi(numdyn)
		{
			...
			if (o->o.reject(v, o->radius + guns[gun].exprad) || o == safe)
				continue;
	...
	void updateprojectiles(int time)
	{
		loopv(projs)
		{
			...
			{
				if (p.local)
					addmsg(N_EXPLODE, "rci3iv", p.owner, lastmillis - maptime, p.gun, p.id - maptime, hits.length(),
	...
	void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id,
					 int prevaction) // create visual effect from a shot
	{
		...
		{
			case GUN_FIST:
				if (d->type == ENT_PLAYER && chainsawhudgun)
					sound = S_CHAINSAW_ATTACK;
				break;

			case GUN_SG: {
				if (!local)
					createrays(gun, from, to);
				...
				loopi(guns[gun].rays)
				{
					...
					if (!local)
						adddecal(DECAL_BULLET, rays[i], vec(from).sub(rays[i]).safenormalize(), 2.0f);
				}
				if (muzzlelight)
					adddynlight(hudgunorigin(gun, d->o, to, d), 30, vec(0.5f, 0.375f, 0.25f), 100, 100, DL_FLASH, 0,
								vec(0, 0, 0), d);
				break;
			}
			...
			case GUN_PISTOL: {
				...
				if (muzzleflash && d->muzzle.x >= 0)
					particle_flare(d->muzzle, d->muzzle, gun == GUN_CG ? 100 : 200, PART_MUZZLE_FLASH1, 0xFFFFFF,
								   gun == GUN_CG ? 2.25f : 1.25f, d);
				if (!local)
					adddecal(DECAL_BULLET, to, vec(from).sub(to).safenormalize(), 2.0f);
				if (muzzlelight)
					adddynlight(hudgunorigin(gun, d->o, to, d), gun == GUN_CG ? 30 : 15, vec(0.5f, 0.375f, 0.25f),
								gun == GUN_CG ? 50 : 100, gun == GUN_CG ? 50 : 100, DL_FLASH, 0, vec(0, 0, 0), d);
				break;
			}
			...
			case GUN_GL: {
				...
				if (muzzlelight)
					adddynlight(hudgunorigin(gun, d->o, to, d), 20, vec(0.5f, 0.375f, 0.25f), 100, 100, DL_FLASH, 0,
								vec(0, 0, 0), d);
			...
			case GUN_RIFLE:
				...
				if (!local)
					adddecal(DECAL_BULLET, to, vec(from).sub(to).safenormalize(), 3.0f);
				if (muzzlelight)
					adddynlight(hudgunorigin(gun, d->o, to, d), 25, vec(0.5f, 0.375f, 0.25f), 75, 75, DL_FLASH, 0,
								vec(0, 0, 0), d);
				break;
		}

		bool looped = false;
		if (d->attacksound >= 0 && d->attacksound != sound)
			d->stopattacksound();
		if (d->idlesound >= 0)
			d->stopidlesound();
		...
		if (d->quadmillis && lastmillis - prevaction > 200 && !looped)
			playsound(S_ITEMPUP, d == h ? NULL : &d->o);
	}
	...
	void particletrack(physent *owner, vec &o, vec &d)
	{
		...
		if (pl->muzzle.x < 0 || pl->lastattackgun != pl->gunselect)
			return;
		float dist = o.dist(d);
		o = pl->muzzle;
		if (dist <= 0)
	...
	void raydamage(vec &from, vec &to, fpsent *d)
	{
		...
		else if (d->gunselect != GUN_FIST && d->gunselect != GUN_BITE)
	...
	void shoot(fpsent *d, const vec &targ)
	{
		...
		if (guns[d->gunselect].rays > 1)
			createrays(d->gunselect, from, to);
		else if (guns[d->gunselect].spread)
			offsetray(from, to, guns[d->gunselect].spread, guns[d->gunselect].range, to);
		...
		if (d == player1 || d->ai)
		{
			addmsg(N_SHOOT, "rci2i6iv", d, lastmillis - maptime, d->gunselect, (int)(from.x * DMF), (int)(from.y * DMF),
				   (int)(from.z * DMF), (int)(to.x * DMF), (int)(to.y * DMF), (int)(to.z * DMF), hits.length(),
				   hits.length() * sizeof(hitmsg) / sizeof(int), hits.getbuf());
		}
	...
	static const char *const debrisnames[4] = {"debris/debris01", "debris/debris02", "debris/debris03",
											   "debris/debris04"};
	static const char *const barreldebrisnames[4] = {"barreldebris/debris01", "barreldebris/debris02",
													 "barreldebris/debris03", "barreldebris/debris04"};
	...
	void renderbouncers()
	{
		...
		loopv(bouncers)
		{
			...
			pitch = -bnc.roll;
			if (bnc.bouncetype == BNC_GRENADE)
				rendermodel(&bnc.light, "projectiles/grenade", ANIM_MAPMODEL | ANIM_LOOP, pos, yaw, pitch,
							MDL_CULL_VFC | MDL_CULL_OCCLUDED | MDL_LIGHT | MDL_LIGHT_FAST | MDL_DYNSHADOW);
			else
			{
				const char *mdl = NULL;
				...
				switch (bnc.bouncetype)
				{
					...
					case BNC_DEBRIS:
						...
						break;
					case BNC_BARRELDEBRIS:
						mdl = barreldebrisnames[bnc.variant];
						break;
					default:
						continue;
				}
	...
	void renderprojectiles()
	{
		...
		loopv(projs)
		{
			...
			v.mul(3);
			v.add(pos);
			rendermodel(&p.light, "projectiles/rocket", ANIM_MAPMODEL | ANIM_LOOP, v, yaw, pitch,
						MDL_CULL_VFC | MDL_CULL_OCCLUDED | MDL_LIGHT | MDL_LIGHT_FAST);
		}
	}
	...
	void checkattacksound(fpsent *d, bool local)
	{
		...
		if (gun >= 0 && gun < NUMGUNS && d->clientnum >= 0 && d->state == CS_ALIVE && d->lastattackgun == gun &&
			lastmillis - d->lastaction < guns[gun].attackdelay + 50)
		{
	...
	void updateweapons(int curtime)
	{
		...
		updatebouncers(
			curtime); // need to do this after the player shoots so grenades don't end up inside player's BB next frame
