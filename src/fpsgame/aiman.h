...
namespace aiman
{
	...
	void calcteams(vector<teamscore> &teams)
	{
		...
		loopv(clients)
		{
			...
			if (ci->state.state == CS_SPECTATOR || !ci->team[0])
				continue;
			teamscore *t = NULL;
			loopvj(teams) if (!strcmp(teams[j].team, ci->team))
			{
				t = &teams[j];
				break;
			}
		...
		{
			loopi(sizeof(defaults) / sizeof(defaults[0])) if (teams.htfind(defaults[i]) < 0)
	...
	void reinitai(clientinfo *ci)
	{
		...
		{
			sendf(-1, 1, "ri6ss", N_INITAI, ci->clientnum, ci->ownernum, ci->state.aitype, ci->state.skill,
				  ci->playermodel, ci->name, ci->team);
			if (ci->aireinit == 2)
			{
				ci->reassign();
