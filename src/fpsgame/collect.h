	...
	#endif
{
	...
	void setup()
	{
		...
		loopv(ments)
		{
			...
			if (e.type != FLAG || e.attr2 < 1 || e.attr2 > 2)
				continue;
			if (!addbase(bases.length(), e.o, e.attr2))
				break;
		}
		notgotbases = false;
	}
	...
	void droptokens(clientinfo *ci, bool penalty = false)
	{
		if (notgotbases)
			return;
		int team = collectteambase(ci->team), totalenemy = penalty ? 0 : ci->state.tokens, totalfriendly = 1,
			expired = 0;
		packetbuf p(300, ENET_PACKET_FLAG_RELIABLE);
		loopvrev(tokens)
		{
			token &t = tokens[i];
			if (t.dropper == ci->clientnum &&
				(t.team == team ? ++totalfriendly > TOKENLIMIT : ++totalenemy > TOKENLIMIT))
			{
				if (!expired)
	...
	{
		if (notgotbases || !bases.inrange(basenum) || ci->state.state != CS_ALIVE || !ci->team[0])
			return;
		base &b = bases[basenum];
		if (!collectbaseteam(b.team))
			return;
		...
		if (ci->state.tokens > 0)
		{
			...
			sendf(-1, 1, "ri7", N_DEPOSITTOKENS, ci->clientnum, basenum, ci->state.tokens, team, score,
				  ci->state.flags);
			ci->state.tokens = 0;
			if (score >= SCORELIMIT)
				startintermission();
		...
		{
			if (gamemillis < b.laststeal + STEALTOKENTIME)
				return;
			if (totalscore(b.team) <= 0)
				return;
			int stolen = 0;
			...
			if (stolen < TOKENLIMIT)
			{
				...
				sendf(-1, 1, "ri9i3", N_STEALTOKENS, ci->clientnum, team, basenum, b.team, score, int(t.o.x * DMF),
					  int(t.o.y * DMF), int(t.o.z * DMF), t.id, t.yaw, -1);
			}
		}
	}
	...
	{
		if (notgotbases || ci->state.state != CS_ALIVE || !ci->team[0])
			return;
	...
	void update()
	{
		if (gamemillis >= gamelimit || notgotbases)
			return;
		vector<int> resets;
		loopvrev(tokens)
		{
	...
	void drawblip(fpsent *d, float x, float y, float s, const vec &pos, float size = 0.05f)
	{
		...
		float xoffset = -size, yoffset = -size, dist = dir.magnitude2(), maxdist = 1 - 0.05f - 0.05f;
		if (dist >= maxdist)
	...
	void drawbaseblip(fpsent *d, float x, float y, float s, int i)
	{
		base &b = bases[i];
		settexture(
			b.team == collectteambase(player1->team) ? "packages/hud/blip_blue.png" : "packages/hud/blip_red.png", 3);
	...
	void drawhud(fpsent *d, int w, int h)
	{
		...
		settexture(team == collectteambase(player1->team) ? "packages/hud/blip_red_skull.png"
														  : "packages/hud/blip_blue_skull.png",
				   3);
		loopv(players)
		{
			fpsent *o = players[i];
	...
	void rendergame()
	{
		...
		loopv(bases)
		{
			...
			regular_particle_flame(PART_FLAME, vec(b.tokenpos.x, b.tokenpos.y, b.tokenpos.z - 4.5f), fradius, fheight,
								   b.team == team ? 0x2020FF : 0x802020, 3, 2.0f);
			vec tokenpos(b.tokenpos);
			tokenpos.z -= theight.z / 2 + sinf(lastmillis / 100.0f) / 20;
			float alpha =
				player1->state == CS_ALIVE && player1->tokens <= 0 && lastmillis < b.laststeal + STEALTOKENTIME ? 0.5f
																												: 1.0f;
			rendermodel(&b.light, b.team == team ? "skull/blue" : "skull/red", ANIM_MAPMODEL | ANIM_LOOP, tokenpos,
						lastmillis / 10.0f, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED, NULL, NULL, 0, 0, alpha);
			formatstring(b.info, "%d", totalscore(b.team));
			vec above(b.tokenpos);
			above.z += TOKENHEIGHT;
			if (b.info[0])
				particle_text(above, b.info, PART_TEXT, 1, b.team == team ? 0x6496FF : 0xFF4B19, 2.0f);
		}
		loopv(tokens)
		{
			token &t = tokens[i];
			...
			rendermodel(&t.light, t.team == team || (t.team < 0 && -t.team != team) ? "skull/blue" : "skull/red",
						ANIM_MAPMODEL | ANIM_LOOP, p, lastmillis / 10.0f, 0,
		...
		{
			fpsent *d = players[i];
			if (d->state != CS_ALIVE || d->tokens <= 0 || d == exclude)
				continue;
			...
			loopj(d->tokens)
			{
				rendermodel(&light, dteam != team ? "skull/blue" : "skull/red", ANIM_MAPMODEL | ANIM_LOOP, pos,
							d->yaw + 90, 0, MDL_SHADOW | MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED);
				pos.z += TOKENHEIGHT + 1;
			}
		}
	}
	...
	void parsetokens(ucharbuf &p, bool commit)
	{
		...
		loopi(numtokens)
		{
			...
			o = movetoken(o, yaw);
			if (o.z >= 0)
				droptoken(id, o, team, lastmillis);
		}
		for (;;)
		{
			int cn = getint(p);
	...
	void baseexplosion(int i, int team, const vec &loc)
	{
		int fcolor;
		vec color;
		if (team == collectteambase(player1->team))
		{
			fcolor = 0x2020FF;
	...
	{
		if (showfrom)
			baseexplosion(i, team, from);
		if (from == to)
			return;
		if (showto)
			baseexplosion(i, team, to);
		particle_flare(from, to, 600, PART_LIGHTNING, team == collectteambase(player1->team) ? 0x2222FF : 0xFF2222,
					   1.0f);
	}
	...
	void taketoken(fpsent *d, int id, int total)
	{
		...
		if (t)
		{
			playsound(t->team == team || (t->team < 0 && -t->team != team) ? S_ITEMAMMO : S_ITEMHEALTH,
					  d != player1 ? &d->o : NULL);
			removetoken(id);
		}
		d->tokens = total;
	...
	token *droptoken(fpsent *d, int id, const vec &o, int team, int yaw, int n)
	{
		...
		if (!n)
			playsound(S_ITEMSPAWN, d ? &d->o : &pos);
		if (d)
		{
			if (!n)
			{
				particle_fireball(d->o, 4.8f, PART_EXPLOSION, 500,
								  team == collectteambase(player1->team) ? 0x2020FF : 0x802020, 4.8f);
				particle_splash(PART_SPARK, 50, 250, d->o, team == collectteambase(player1->team) ? 0x2020FF : 0x802020,
								0.24f);
			}
			particle_flare(d->o, vec(t.o.x, t.o.y, t.o.z + 0.5f * (TOKENHEIGHT + 1)), 500, PART_LIGHTNING,
						   team == collectteambase(player1->team) ? 0x2222FF : 0xFF2222, 1.0f);
		}
		return &t;
	}
	...
	void stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam, int score)
	{
		...
		if (bases.inrange(basenum))
		{
			...
			if (!n)
			{
				...
				conoutf(CON_GAMEINFO, "%s stole a skull from %s", teamcolorname(d),
						teamcolor("your team", collectbaseteam(enemyteam), "the enemy team"));
				teamsound(d, S_FLAGDROP, &b.tokenpos);
			}
			if (t)
				particle_flare(b.tokenpos, vec(t->o.x, t->o.y, t->o.z + 0.5f * (TOKENHEIGHT + 1)), 500, PART_LIGHTNING,
							   team == collectteambase(player1->team) ? 0x2222FF : 0xFF2222, 1.0f);
		}
	}
	...
	void deposittokens(fpsent *d, int basenum, int deposited, int team, int score, int flags)
	{
		if (bases.inrange(basenum))
		{
			...
			loopv(bases)
			{
				base &h = bases[i];
				if (h.team == team)
					baseeffect(i, team, h.tokenpos, b.tokenpos, !n++);
			}
		}
		d->tokens = 0;
		d->flags = flags;
		...
		conoutf(CON_GAMEINFO, "%s collected %d %s for %s", teamcolorname(d), deposited,
				deposited == 1 ? "skull" : "skulls", teamcolor("your team", collectbaseteam(team), "the enemy team"));
		playsound(team == collectteambase(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);

		if (score >= SCORELIMIT)
			conoutf(CON_GAMEINFO, "%s collected %d skulls",
					teamcolor("your team", collectbaseteam(team), "the enemy team"), score);
	}
	...
	void checkitems(fpsent *d)
	{
		...
		if (d->tokens > 0 || o != d->lastcollect)
		{
			...
			{
				base &b = bases[i];
				if (!collectbaseteam(b.team) || b.team == team)
					continue;
		...
		if (d->tokens < TOKENLIMIT)
			loopv(tokens)
			{
				token &t = tokens[i];
				if (o.dist(t.o) < TOKENRADIUS && d->lastcollect.dist(t.o) >= TOKENRADIUS &&
					(lookupmaterial(o) & MATF_CLIP) != MAT_GAMECLIP &&
					(lookupmaterial(t.o) & MATF_CLIP) != MAT_GAMECLIP)
	...
	bool aicheck(fpsent *d, ai::aistate &b)
	{
		...
		if (d->tokens > 0)
		{
			...
			if (best < 0 || !ai::makeroute(d, b, bases[best].o))
				return false;
		...
		else
		{
			...
			if (best < 0 || !ai::makeroute(d, b, tokens[best].o))
				return false;
	...
	bool aipursue(fpsent *d, ai::aistate &b)
	{
		if (b.target < 0)
		{
			if (d->tokens <= 0 || !bases.inrange(-(b.target + 1)))
				return false;
			base &g = bases[-(b.target + 1)];
			if (g.team == collectteambase(d->team))
				return false;
...
case N_INITTOKENS:
	if (smode == &collectmode)
		collectmode.parsebases(p, (ci->state.state != CS_SPECTATOR || ci->privilege || ci->local) &&
									  !strcmp(ci->clientmap, smapname));
	break;

case N_TAKETOKEN: {
	int id = getint(p);
	if ((ci->state.state != CS_SPECTATOR || ci->local || ci->privilege) && cq && smode == &collectmode)
		collectmode.taketoken(cq, id);
	break;
}
...
case N_DEPOSITTOKENS: {
	int id = getint(p);
	if ((ci->state.state != CS_SPECTATOR || ci->local || ci->privilege) && cq && smode == &collectmode)
		collectmode.deposittokens(cq, id);
	break;
}
...
case N_TAKETOKEN: {
	...
	if (o && m_collect)
		collectmode.taketoken(o, id, total);
	break;
}
...
case N_DROPTOKENS: {
	...
	for (int n = 0;; n++)
	{
		...
		if (p.overread())
			break;
		if (o && m_collect)
			collectmode.droptoken(o, id, droploc, team, yaw, n);
	}
	break;
}
...
case N_STEALTOKENS: {
	...
	for (int n = 0;; n++)
	{
		...
		if (p.overread())
			break;
		if (o && m_collect)
			collectmode.stealtoken(o, id, droploc, team, yaw, n, basenum, enemyteam, score);
	}
	break;
}

case N_DEPOSITTOKENS: {
	int ocn = getint(p), base = getint(p), deposited = getint(p), team = getint(p), score = getint(p),
		flags = getint(p);
	fpsent *o = ocn == player1->clientnum ? player1 : newclient(ocn);
	if (o && m_collect)
		collectmode.deposittokens(o, base, deposited, team, score, flags);
	break;
}
