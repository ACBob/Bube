...
namespace game
{
	...
	static const monstertype monstertypes[NUMMONSTERTYPES] = {
		...
		{GUN_SG, 13, 120, 1, 100, 300, 400, 4, 14, 115, S_PAIN1, S_DIE1, "ratamahatta", "monster/rat",
		 "monster/rat/vwep"},
		{GUN_RIFLE, 14, 200, 1, 80, 400, 300, 4, 18, 145, S_PAIN1, S_DIE1, "a slith", "monster/slith",
		 "monster/slith/vwep"},
		...
		{GUN_ICEBALL, 11, 250, 1, 0, 10, 400, 6, 18, 160, S_PAIN1, S_DIE1, "a knight", "monster/knight",
		 "monster/knight/vwep"},
		{GUN_SLIMEBALL, 15, 100, 1, 0, 200, 400, 2, 10, 60, S_PAIN1, S_DIE1, "a goblin", "monster/goblin",
		 "monster/goblin/vwep"},
	...
	void preloadmonsters()
	{
		...
		if (m_dmsp)
			preloadsound(S_V_FIGHT);
		if (m_classicsp)
			preloadsound(S_V_RESPAWNPOINT);
	}
	...
	void clearmonsters() // called after map start or when toggling edit mode to reset/spawn all monsters to initial
						 // state
	{
	...
	void monsterkilled()
	{
		...
		if (remain > 0 && remain <= 5)
			conoutf(CON_GAMEINFO, "\f2only %d monster(s) remaining", remain);
	}
	...
	void updatemonsters(int curtime)
	{
		...
		if (killsendsp && monstertotal && !spawnremain && numkilled == monstertotal)
			endsp(true);
	...
	void rendermonsters()
	{
		loopv(monsters)
		{
			...
			if (m.state != CS_DEAD || lastmillis - m.lastpain < 10000)
			{
				...
				renderclient(&m, monstertypes[m.mtype].mdlname, vwep, 0,
							 m.monsterstate == M_ATTACKING ? -ANIM_ATTACK1 : 0, 300, m.lastaction, m.lastpain, fade);
			}
		}
	}
	...
	void spsummary(int accuracy)
	{
		...
		score += pen;
		if (pen)
			conoutf(CON_GAMEINFO, "\f2time taken: %d seconds (%d simulated seconds)", pen,
					(lastmillis - maptime) / 1000);
		pen = player1->deaths * 60;
		score += pen;
		if (pen)
			conoutf(CON_GAMEINFO, "\f2time penalty for %d deaths (1 minute each): %d seconds", player1->deaths, pen);
		pen = remain * 10;
		score += pen;
		if (pen)
			conoutf(CON_GAMEINFO, "\f2time penalty for %d monsters remaining (10 seconds each): %d seconds", remain,
					pen);
		pen = (10 - skill) * 20;
		score += pen;
		if (pen)
			conoutf(CON_GAMEINFO, "\f2time penalty for lower skill level (20 seconds each): %d seconds", pen);
		pen = 100 - accuracy;
		score += pen;
		if (pen)
			conoutf(CON_GAMEINFO, "\f2time penalty for missed shots (1 second each %%): %d seconds", pen);
		...
		alias(aname, nscore);
		conoutf(CON_GAMEINFO, "\f2TOTAL SCORE (time + time penalties): %d seconds (best so far: %d seconds)", score,
				bestscore);
	}
} // namespace game
