...
namespace game
{
	...
	void drawteammate(fpsent *d, float x, float y, float s, fpsent *o, float scale)
	{
		...
		float bs = 0.06f * s, bx = x + s * 0.5f * (1.0f + dir.x), by = y + s * 0.5f * (1.0f + dir.y);
		vec v(-0.5f, -0.5f, 0);
	...
	void drawteammates(fpsent *d, float x, float y, float s)
	{
		...
		loopv(players)
		{
			...
			if (o != d && o->state == CS_ALIVE && isteam(o->team, d->team))
			{
				if (!alive++)
				{
					settexture(isteam(d->team, player1->team) ? "packages/hud/blip_blue_alive.png"
															  : "packages/hud/blip_red_alive.png");
					gle::defvertex(2);
					gle::deftexcoord0();
					gle::begin(GL_QUADS);
				}
		...
		loopv(players)
		{
			...
			if (o != d && o->state == CS_DEAD && isteam(o->team, d->team))
			{
				if (!dead++)
				{
					settexture(isteam(d->team, player1->team) ? "packages/hud/blip_blue_dead.png"
															  : "packages/hud/blip_red_dead.png");
					gle::defvertex(2);
					gle::deftexcoord0();
					gle::begin(GL_QUADS);
				}
...
#include "collect.h"
	...
	void switchteam(const char *team)
	{
		if (player1->clientnum < 0)
			filtertext(player1->team, team, false, false, MAXTEAMLEN);
		else
	...
	struct authkey
	{
		...
		authkey(const char *name, const char *key, const char *desc)
			: name(newstring(name)), key(newstring(key)), desc(newstring(desc)), lastauth(0)
		{
		}
	...
	void getpubkey(const char *desc)
	{
		...
		{
			if (desc[0])
				conoutf(CON_ERROR, "no authkey found: %s", desc);
			else
				conoutf(CON_ERROR, "no global authkey found");
			return;
		}
		vector<char> pubkey;
		if (!calcpubkey(k->key, pubkey))
		{
			conoutf(CON_ERROR, "failed calculating pubkey");
			return;
		}
	...
	void saveauthkeys()
	{
		...
		if (!f)
		{
			conoutf(CON_ERROR, "failed to open auth.cfg for writing");
			return;
		}
		loopv(authkeys)
		{
	...
	ICOMMAND(dauthkick, "sss", (const char *desc, const char *victim, const char *reason),
	...
	ICOMMANDS("m_noitems", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_noitems);
	});
	ICOMMANDS("m_noammo", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_noammo);
	});
	ICOMMANDS("m_insta", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_insta);
	});
	ICOMMANDS("m_tactics", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_tactics);
	});
	ICOMMANDS("m_efficiency", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_efficiency);
	});
	ICOMMANDS("m_capture", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_capture);
	});
	ICOMMANDS("m_regencapture", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_regencapture);
	});
	ICOMMANDS("m_ctf", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_ctf);
	});
	ICOMMANDS("m_protect", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_protect);
	});
	ICOMMANDS("m_hold", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_hold);
	});
	ICOMMANDS("m_collect", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_collect);
	});
	...
	ICOMMANDS("m_sp", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_sp);
	});
	ICOMMANDS("m_dmsp", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_dmsp);
	});
	ICOMMANDS("m_classicsp", "i", (int *mode), {
		int gamemode = *mode;
		intret(m_classicsp);
	});
		...
		if (m_edit)
			switch (op)
			{
				...
				case EDIT_DELCUBE: {
					...
					addmsg(N_EDITF + op, "ri9i4", sel.o.x, sel.o.y, sel.o.z, sel.s.x, sel.s.y, sel.s.z, sel.grid,
						   sel.orient, sel.cx, sel.cxs, sel.cy, sel.cys, sel.corner);
					break;
				}
				case EDIT_ROTATE: {
					addmsg(N_EDITF + op, "ri9i5", sel.o.x, sel.o.y, sel.o.z, sel.s.x, sel.s.y, sel.s.z, sel.grid,
						   sel.orient, sel.cx, sel.cxs, sel.cy, sel.cys, sel.corner, arg1);
					break;
				}
				case EDIT_MAT:
				case EDIT_FACE: {
					addmsg(N_EDITF + op, "ri9i6", sel.o.x, sel.o.y, sel.o.z, sel.s.x, sel.s.y, sel.s.z, sel.grid,
						   sel.orient, sel.cx, sel.cxs, sel.cy, sel.cys, sel.corner, arg1, arg2);
					break;
				}
				case EDIT_TEX: {
					int tex1 = shouldpacktex(arg1);
					if (addmsg(N_EDITF + op, "ri9i6", sel.o.x, sel.o.y, sel.o.z, sel.s.x, sel.s.y, sel.s.z, sel.grid,
							   sel.orient, sel.cx, sel.cxs, sel.cy, sel.cys, sel.corner, tex1 ? tex1 : arg1, arg2))
					{
						messages.pad(2);
				...
				case EDIT_REPLACE: {
					...
					if (addmsg(N_EDITF + op, "ri9i7", sel.o.x, sel.o.y, sel.o.z, sel.s.x, sel.s.y, sel.s.z, sel.grid,
							   sel.orient, sel.cx, sel.cxs, sel.cy, sel.cys, sel.corner, tex1 ? tex1 : arg1,
							   tex2 ? tex2 : arg2, arg3))
					{
						messages.pad(2);
						int offset = messages.length();
			...
			switch (id->type)
			{
				case ID_VAR: {
					...
					if (val < 0)
						...
						formatstring(str, "0x%.6X (%d, %d, %d)", val, (val >> 16) & 0xFF, (val >> 8) & 0xFF,
									 val & 0xFF);
					else
				...
				case ID_FVAR:
					conoutf(CON_INFO, id->index, "%s set map var \"%s\" to %s", colorname(d), id->name,
	...
	{
		conoutf(CON_TEAMCHAT, "\fs\f8[team]\fr %s: \f8%s", chatcolorname(player1), text);
	...
	static void sendposition(fpsent *d, packetbuf &q)
	{
		...
		uint veldir =
			(velyaw < 0 ? 360 + int(velyaw) % 360 : int(velyaw) % 360) + clamp(int(velpitch + 90), 0, 180) * 360;
		q.put(veldir & 0xFF);
		q.put((veldir >> 8) & 0xFF);
		if (fall > 0)
		{
		...
		while (p.remaining())
			switch (type = getint(p))
			{
				...
				case N_SERVINFO: // welcome messsage from the server
				{
					...
					{
						conoutf(CON_ERROR, "you are using a different game protocol (you: %d, server: %d)",
				...
				case N_SHOTFX: {
					...
					if (!s)
						break;
					if (gun > GUN_FIST && gun <= GUN_PISTOL && s->ammo[gun])
						s->ammo[gun]--;
				...
				case N_DAMAGE: {
					int tcn = getint(p), acn = getint(p), damage = getint(p), armour = getint(p), health = getint(p);
					fpsent *target = getclient(tcn), *actor = getclient(acn);
					if (!target || !actor)
						break;
				...
				case N_DIED: {
					...
					fpsent *victim = getclient(vcn), *actor = getclient(acn);
					if (!actor)
						break;
					actor->frags = frags;
					if (m_teammode)
				...
				case N_TEAMINFO:
					...
					{
						getstring(text, p);
						if (p.overread() || !text[0])
							break;
						int frags = getint(p);
						if (p.overread())
							break;
						if (m_teammode)
							setteaminfo(text, frags);
					}
					break;
                ...
                if(name) particle_text(entities::ents[i]->o, name, PART_TEXT, 2000, 0x32FF64, 4.0f, -8);
					...
					if (icon >= 0)
						particle_icon(vec(0.0f, 0.0f, 4.0f).add(entities::ents[i]->o), icon % 4, icon / 4,
...
#undef PARSEMESSAGES
		...
		while (p.remaining())
			switch (type = getint(p))
			{
				...
				case N_SENDDEMO: {
					...
					if (len < 4 || strcasecmp(&fname[len - 4], ".dmo"))
						concatstring(fname, ".dmo");
					stream *demo = NULL;
					if (const char *buf = server::getdemofile(fname, true))
						demo = openrawfile(buf, "wb");
					if (!demo)
						demo = openrawfile(fname, "wb");
					if (!demo)
						return;
	...
	void getmap()
	{
		if (!m_edit)
		{
			conoutf(CON_ERROR, "\"getmap\" only works in coop edit mode");
			return;
		}
		conoutf("getting map...");
		addmsg(N_GETMAP, "r");
	...
	void getdemo(char *val, char *name)
	{
		...
		if (name[0])
		{
			if (demoreqs.length() >= MAXDEMOREQS)
				demoreqs.remove(0);
