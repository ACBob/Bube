	...
	#endif
{
	...
	struct flag
	{
		...
		int holdcounter() const
		{
			if (!holdtime)
				return 0;
			if (!hasowner() && droptime && droptime >= holdtime)
				return max((droptime - holdtime) - (lastmillis - droptime), 0);
	...
	#endif
	{
		if (holdspawns.length() >= MAXHOLDSPAWNS)
			return false;
	...
	#endif
	{
		...
		{
			if (f.droptime && f.droptime < owntime)
				f.holdtime += 2 * (owntime - f.droptime);
	...
	{
		if (!m_hold || holdspawns.empty())
			return;
		while (flags.length() < HOLDFLAGS)
		{
			int i = flags.length();
			if (!addflag(i, vec(0, 0, 0), 0, 0))
				break;
			flag &f = flags[i];
			spawnflag(i);
	...
	void setup()
	{
		...
		if (m_hold)
		{
			...
			{
				entity &e = ments[i];
				if (e.type != FLAG || e.attr2 < 1 || e.attr2 > 2)
					continue;
				if (!addflag(flags.length(), e.o, e.attr2, m_protect ? lastmillis : 0))
					break;
			}
		notgotflags = false;
	}
	...
	void dropflag(clientinfo *ci, clientinfo *dropper = NULL)
	{
		...
		loopv(flags) if (flags[i].owner == ci->clientnum)
		{
			...
			else
			{
				...
				dropflag(i, vec(o).div(DMF), lastmillis, dropper ? dropper->clientnum : ci->clientnum,
						 dropper && dropper != ci);
			}
		}
	}
	...
	void died(clientinfo *ci, clientinfo *actor)
	{
		if (notgotflags)
			return;
		if (m_hold)
			loopv(flags) if (flags[i].owner == ci->clientnum) deadflag(i);
	...
	{
		return m_efficiency || !m_protect ? connecting || !ci->state.lastdeath ||
												gamemillis + curtime - ci->state.lastdeath >= RESPAWNSECS * 1000
										  : true;
	}
	...
	void spawnflag(int i)
	{
		...
		loopj(4)
		{
			...
			if (spawnindex != flags[i].spawnindex)
				break;
		}
	...
	void scoreflag(clientinfo *ci, int goal, int relay = -1)
	{
		...
		if (m_hold)
			spawnflag(goal);
		sendf(-1, 1, "rii9", N_SCOREFLAG, ci->clientnum, relay, relay >= 0 ? ++flags[relay].version : -1, goal,
			  ++flags[goal].version, flags[goal].spawnindex, team, score, ci->state.flags);
		if (score >= FLAGLIMIT)
			startintermission();
	}
	...
	void takeflag(clientinfo *ci, int i, int version)
	{
		if (notgotflags || !flags.inrange(i) || ci->state.state != CS_ALIVE || !ci->team[0])
			return;
		flag &f = flags[i];
		if ((m_hold ? f.spawnindex < 0 : !ctfflagteam(f.team)) || f.owner >= 0 || f.version != version ||
			(f.droptime && f.dropper == ci->clientnum && f.dropcount >= 3))
			return;
	...
	void update()
	{
		...
		loopv(flags)
		{
			...
			if (f.owner < 0 && f.droptime && lastmillis - f.droptime >= RESETFLAGTIME)
			{
				...
				if (m_hold)
					spawnflag(i);
				sendf(-1, 1, "ri6", N_RESETFLAG, i, ++f.version, f.spawnindex, m_hold ? 0 : f.team,
					  m_hold ? 0 : addscore(f.team, m_protect ? -1 : 0));
			}
		...
		loopi(numflags)
		{
			...
			if (commit && notgotflags)
			{
				if (m_hold)
					...
					addflag(i, o, team, m_protect ? lastmillis : 0);
			}
		}
		if (commit && notgotflags)
		{
	...
	{
		if (m_hold)
			preloadmodel("flags/neutral");
		else
		{
			preloadmodel("flags/red");
			preloadmodel("flags/blue");
		}
		static const char *sounds[] = {S_FLAGPICKUP, S_FLAGDROP, S_FLAGRETURN, S_FLAGSCORE, S_FLAGRESET, S_FLAGFAIL};
	...
	void drawblip(fpsent *d, float x, float y, float s, const vec &pos, bool flagblip)
	{
		...
		float size = flagblip ? 0.1f : 0.05f, xoffset = flagblip ? -2 * (3 / 32.0f) * size : -size,
			  yoffset = flagblip ? -2 * (1 - 3 / 32.0f) * size : -size, dist = dir.magnitude2(),
	...
	{
		flag &f = flags[i];
		settexture(m_hold && (!flagblip || !f.owner || lastmillis % 1000 < 500)
					   ? (flagblip ? "packages/hud/blip_neutral_flag.png" : "packages/hud/blip_neutral.png")
				   ...
				   3);
		drawblip(d, x, y, s, flagblip ? (f.owner ? f.owner->o : (f.droptime ? f.droploc : f.spawnloc)) : f.spawnloc,
				 flagblip);
	}
	...
	void drawhud(fpsent *d, int w, int h)
	{
		if (d->state == CS_ALIVE)
		{
			loopv(flags) if (flags[i].owner == d)
			{
				...
				drawicon(m_hold ? HICON_NEUTRAL_FLAG
								: (f.team == ctfteamflag(player1->team) ? HICON_BLUE_FLAG : HICON_RED_FLAG),
						 x, HICON_Y);
				if (m_hold)
				{
					pushhudmatrix();
					...
					flushhudmatrix();
					draw_textf("%d", (x + HICON_SIZE + HICON_SPACE) / 2, HICON_TEXTY / 2,
							   max(HOLDSECS - f.holdcounter() / 1000, 0));
					pophudmatrix();
				}
				break;
			}
		...
		{
			flag &f = flags[i];
			if (m_hold ? f.spawnindex < 0 : !ctfflagteam(f.team))
				continue;
			if (!m_hold)
				drawblip(d, x, y, s, i, false);
			if (f.owner)
			{
				if (!m_hold && lastmillis % 1000 >= 500)
					continue;
			}
		...
		if (d->state == CS_DEAD && (m_efficiency || !m_protect))
		{
			...
			if (wait >= 0)
			{
				...
				flushhudmatrix();
				bool flash =
					wait > 0 && d == player1 && lastspawnattempt >= d->lastpain && lastmillis < lastspawnattempt + 100;
				draw_textf("%s%d", (x + s / 2) / 2 - (wait >= 10 ? 28 : 16), (y + s / 2) / 2 - 32, flash ? "\f3" : "",
						   wait);
				pophudmatrix();
			}
		}
	...
	{
		if (m_hold)
			loopv(flags) if (flags[i].owner == victim) deadflag(i);
	}
	...
	void rendergame()
	{
		loopv(flags)
		{
			...
			if (!f.owner && f.droptime && f.droploc.x < 0)
				continue;
			if (m_hold && f.spawnindex < 0)
				continue;
			const char *flagname = m_hold && (!f.owner || lastmillis % 1000 < 500)
									   ? "flags/neutral"
									   : (m_hold ? ctfteamflag(f.owner->team) : f.team) == ctfteamflag(player1->team)
											 ? "flags/blue"
											 : "flags/red";
			...
			if (m_hold)
				rendermodel(!f.droptime && !f.owner ? &f.light : NULL, flagname, ANIM_MAPMODEL | ANIM_LOOP, pos, angle,
							0, MDL_GHOST | MDL_CULL_VFC | (f.droptime || f.owner ? MDL_LIGHT : 0), NULL, NULL, 0, 0,
							0.5f + 0.5f * (2 * fabs(fmod(lastmillis / 1000.0f, 1.0f) - 0.5f)));
			rendermodel(!f.droptime && !f.owner ? &f.light : NULL, flagname, ANIM_MAPMODEL | ANIM_LOOP, pos, angle, 0,
						MDL_DYNSHADOW | MDL_CULL_VFC | MDL_CULL_OCCLUDED | (f.droptime || f.owner ? MDL_LIGHT : 0),
						NULL, NULL, 0, 0,
						0.3f + (f.vistime ? 0.7f * min((lastmillis - f.vistime) / 1000.0f, 1.0f) : 0.0f));
			...
			if (m_hold && canaddparticles() && f.team && f.holdtime)
			{
				...
				if (counter > 0)
				{
					...
					particle_textcopy(above, msg, PART_TEXT, 1,
									  f.team == ctfteamflag(player1->team) ? 0x6496FF : 0xFF4B19, 4.0f);
				}
			}
			if (m_protect && canaddparticles() && f.owner && insidebase(f, f.owner->feetpos()))
			{
				particle_flare(pos, f.spawnloc, 0, PART_LIGHTNING,
							   strcmp(f.owner->team, player1->team) ? 0xFF2222 : 0x2222FF, 1.0f);
				if (!flags.inrange(f.owner->lastbase))
				{
					particle_fireball(pos, 4.8f, PART_EXPLOSION, 250,
									  strcmp(f.owner->team, player1->team) ? 0x802020 : 0x2020FF, 4.8f);
					particle_splash(PART_SPARK, 50, 250, pos,
									strcmp(f.owner->team, player1->team) ? 0x802020 : 0x2020FF, 0.24f);
				}
				f.owner->lastbase = i;
			}
		}
		if (m_protect && canaddparticles())
			loopv(players)
			{
				fpsent *d = players[i];
				if (!flags.inrange(d->lastbase))
					continue;
				flag &f = flags[d->lastbase];
				if (f.owner == d && insidebase(f, d->feetpos()))
					continue;
				d->lastbase = -1;
				float angle;
				vec pos = interpflagpos(f, angle);
				particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(d->team, player1->team) ? 0x802020 : 0x2020FF,
								  4.8f);
	...
	void setup()
	{
		...
		if (m_hold)
		{
			...
			if (holdspawns.length())
				while (flags.length() < HOLDFLAGS)
					addflag(flags.length(), vec(0, 0, 0), 0, -1000);
		}
		else
		{
			loopv(entities::ents)
			{
				...
				if (e->type != FLAG)
					continue;
				if (e->attr2 < 1 || e->attr2 > 2)
					continue;
				int index = flags.length();
				if (!addflag(index, e->o, e->attr2, m_protect ? 0 : -1000))
					continue;
		...
		loopi(numflags)
		{
			...
			if (commit && flags.inrange(i))
			{
				...
				if (m_hold)
				{
					...
					{
						f.team = holdteam;
						f.holdtime = lastmillis - holdtime;
					}
					else
					{
						f.team = 0;
	...
	void flagexplosion(int i, int team, const vec &loc)
	{
		...
		{
			fcolor = 0xFF8080;
			color = vec(1, 0.75f, 0.5f);
		}
		else if (team == ctfteamflag(player1->team))
		{
			fcolor = 0x2020FF;
	...
	void flageffect(int i, int team, const vec &from, const vec &to)
	{
		...
		if (from.x >= 0 && to.x >= 0)
			particle_flare(fromexp, toexp, 600, PART_LIGHTNING,
						   !team ? 0xFFC0A0 : (team == ctfteamflag(player1->team) ? 0x2222FF : 0xFF2222), 1.0f);
	}
	...
	void returnflag(fpsent *d, int i, int version)
	{
		...
		f.interptime = 0;
		returnflag(i);
		if (m_protect && d->feetpos().dist(f.spawnloc) < FLAGRADIUS)
			d->flagpickup |= 1 << f.id;
	...
	{
		if (!holdspawns.inrange(f.spawnindex))
			return;
	...
	void resetflag(int i, int version, int spawnindex, int team, int score)
	{
		...
		if (m_hold)
			spawnflag(f);
		if (shouldeffect)
			flageffect(i, m_hold ? 0 : team, interpflagpos(f), f.spawnloc);
		f.interptime = 0;
	...
	void scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team,
				   int score, int dflags)
	{
		setscore(team, score);
		if (flags.inrange(goal))
		{
			...
			if (relay >= 0)
			{
				...
				flageffect(goal, team, interpflagpos(f), f.spawnloc);
			f.interptime = 0;
		...
		d->flags = dflags;
		conoutf(CON_GAMEINFO, "%s scored for %s", teamcolorname(d),
				teamcolor("your team", ctfflagteam(team), "the enemy team"));
		playsound(team == ctfteamflag(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);

		if (score >= FLAGLIMIT)
			conoutf(CON_GAMEINFO, "%s captured %d flags", teamcolor("your team", ctfflagteam(team), "the enemy team"),
					score);
	}
	...
	void takeflag(fpsent *d, int i, int version)
	{
		...
		f.interptime = lastmillis;
		if (m_hold)
			conoutf(CON_GAMEINFO, "%s picked up the flag for %s", teamcolorname(d),
					teamcolor("your team", d->team, "the enemy team"));
		else if (m_protect || f.droptime)
			conoutf(CON_GAMEINFO, "%s picked up %s", teamcolorname(d), teamcolorflag(f));
		else
	...
	void checkitems(fpsent *d)
	{
		...
		{
			flag &f = flags[i];
			if ((m_hold ? f.spawnindex < 0 : !ctfflagteam(f.team) || f.team == ctfteamflag(d->team)) || f.owner ||
				(f.droptime && f.droploc.x < 0))
				continue;
			...
			if (o.dist(loc) < FLAGRADIUS)
			{
				if (d->flagpickup & (1 << f.id))
					continue;
				if (m_hold || ((lookupmaterial(o) & MATF_CLIP) != MAT_GAMECLIP &&
							   (lookupmaterial(loc) & MATF_CLIP) != MAT_GAMECLIP))
				{
					tookflag = true;
		...
		{
			flag &f = flags[i];
			if (!ctfflagteam(f.team) || f.team != ctfteamflag(d->team) || f.owner || (f.droptime && f.droploc.x < 0))
				continue;
			const vec &loc = f.droptime ? f.droploc : f.spawnloc;
			if (o.dist(loc) < FLAGRADIUS)
			{
				if (!tookflag && d->flagpickup & (1 << f.id))
					continue;
				if ((lookupmaterial(o) & MATF_CLIP) != MAT_GAMECLIP &&
					(lookupmaterial(loc) & MATF_CLIP) != MAT_GAMECLIP)
					addmsg(N_TAKEFLAG, "rcii", d, i, f.version);
				d->flagpickup |= 1 << f.id;
			}
	...
	void respawned(fpsent *d)
	{
		...
		{
			flag &f = flags[i];
			if ((m_hold ? f.spawnindex < 0 : !ctfflagteam(f.team)) || f.owner || (f.droptime && f.droploc.x < 0))
				continue;
	...
	{ // avoid spawning too far away from active flag
		...
		loopv(flags)
		{
			...
			if (f.spawnindex < 0 || (!f.owner && (!f.droptime || f.droploc.x < 0)))
				continue;
	...
	bool aihomerun(fpsent *d, ai::aistate &b)
	{
		if (m_protect || m_hold)
		{
			...
			loopk(2)
			{
				...
				if (ai::parseinterests(d, b, interests, false, true))
					return true;
			}
		}
		else
		...
		if (b.type == ai::AI_S_INTEREST && b.targtype == ai::AI_T_NODE)
	...
	bool aicheck(fpsent *d, ai::aistate &b)
	{
		...
		loopv(flags)
		{
			...
			else if (g.team == ctfteamflag(d->team) &&
					 ((g.owner && g.team != ctfteamflag(g.owner->team)) || g.droptime))
				takenflags.add(i);
		}
	...
	void aifind(fpsent *d, ai::aistate &b, vector<ai::interest> &interests)
	{
		...
		loopvj(flags)
		{
			...
			if ((!m_protect && !m_hold) || f.owner != d)
			{
				...
				fpsent *e = NULL;
				loopi(numdynents()) if ((e = (fpsent *)iterdynents(i)) && !e->ai && e->state == CS_ALIVE &&
										isteam(d->team, e->team))
				{ // try to guess what non ai are doing
					vec ep = e->feetpos();
					if (targets.find(e->clientnum) < 0 &&
						(ep.squaredist(f.pos()) <= (FLAGRADIUS * FLAGRADIUS * 4) || f.owner == e))
						targets.add(e->clientnum);
				}
				if (home)
				{
					bool guard = false;
					if ((f.owner && f.team != ctfteamflag(f.owner->team)) || f.droptime || targets.empty())
						guard = true;
					...
					{ // see if we can relieve someone who only has a piece of crap
						...
						loopvk(targets) if ((t = getclient(targets[k])))
						{
							if ((t->ai && !t->hasammo(t->ai->weappref)) ||
								(!t->ai && (t->gunselect == GUN_FIST || t->gunselect == GUN_PISTOL)))
							{
								guard = true;
								break;
							}
	...
	bool aidefend(fpsent *d, ai::aistate &b)
	{
		...
		if (flags.inrange(b.target))
		{
			...
			{ // get out of the way of the returnee!
				...
				{
					if (!m_protect && !m_hold && g.owner && !strcmp(g.owner->team, d->team))
						walk = 1;
...
case N_TRYDROPFLAG: {
	if ((ci->state.state != CS_SPECTATOR || ci->local || ci->privilege) && cq && smode == &ctfmode)
		ctfmode.dropflag(cq);
	break;
}
...
case N_INITFLAGS:
	if (smode == &ctfmode)
		ctfmode.parseflags(p, (ci->state.state != CS_SPECTATOR || ci->privilege || ci->local) &&
...
case N_SCOREFLAG: {
	int ocn = getint(p), relayflag = getint(p), relayversion = getint(p), goalflag = getint(p), goalversion = getint(p),
		goalspawn = getint(p), team = getint(p), score = getint(p), oflags = getint(p);
	fpsent *o = ocn == player1->clientnum ? player1 : newclient(ocn);
	if (o && m_ctf)
		ctfmode.scoreflag(o, relayflag, relayversion, goalflag, goalversion, goalspawn, team, score, oflags);
	break;
}
...
case N_RESETFLAG: {
	...
	if (m_ctf)
		ctfmode.resetflag(flag, version, spawnindex, team, score);
	break;
}
