...
static void show_genpvs_progress(int unique = pvs.length(), int processed = numviewcells)
{
	...
	defformatstring(text1, "%d%% - %d of %d view cells (%d unique)", int(bar1 * 100), processed, totalviewcells,
					unique);
...
void testpvs(int *vcsize)
{
	...
	loopi(MAXWATERPVS) lockedwaterplanes[i] = waterplanes[i].height;
	lockpvs = 1;
	conoutf("generated test view cell of size %d at %.1f, %.1f, %.1f (%d B)", size, camera1->o.x, camera1->o.y,
			camera1->o.z, len);
...
void genpvs(int *viewcellsize)
{
	...
	if (genpvs_canceled)
	{
		...
		conoutf("generated %d unique view cells totaling %.1f kB and averaging %d B (%.1f seconds)", pvs.length(),
				pvsbuf.length() / 1024.0f, pvsbuf.length() / max(pvs.length(), 1), (end - start) / 1000.0f);
}
...
void pvsstats()
{
	conoutf("%d unique view cells totaling %.1f kB and averaging %d B", pvs.length(), pvsbuf.length() / 1024.0f,
