...
static void debugalias()
{
	...
	for (identlink *l = aliasstack; l != &noalias; l = l->next)
	{
		...
		++depth;
		if (depth < dbgalias)
			conoutf(CON_ERROR, "  %d) %s", total - depth + 1, id->name);
		else if (l->next == &noalias)
			conoutf(CON_ERROR, depth == dbgalias ? "  %d) %s" : "  ..%d) %s", total - depth + 1, id->name);
	}
}
	...
	for (; i < n; i++)
		switch (v[i].type)
		{
			...
			case VAL_STR:
				...
				break;
			case VAL_INT:
				if (numlen + MAXSTRLEN > int(sizeof(numbuf)))
					goto overflow;
...
static void compilelookup(vector<uint> &code, const char *&p, int ltype)
{
	...
	switch (*++p)
	{
		case '(':
		case '[':
			if (!compileword(code, p, VAL_STR, lookup, lookuplen))
				goto invalid;
			break;
		case '$':
		...
		lookupid:
			...
			if (id)
				...
				{
					case ID_VAR:
						code.add(CODE_IVAR | ((ltype >= VAL_ANY ? VAL_INT : ltype) << CODE_RET) | (id->index << 8));
						goto done;
					case ID_FVAR:
						code.add(CODE_FVAR | ((ltype >= VAL_ANY ? VAL_FLOAT : ltype) << CODE_RET) | (id->index << 8));
						goto done;
					case ID_SVAR:
						code.add(CODE_SVAR | ((ltype >= VAL_ANY ? VAL_STR : ltype) << CODE_RET) | (id->index << 8));
						goto done;
					case ID_ALIAS:
						code.add((id->index < MAXARGS ? CODE_LOOKUPARG : CODE_LOOKUP) |
								 ((ltype >= VAL_ANY ? VAL_STR : ltype) << CODE_RET) | (id->index << 8));
						goto done;
					case ID_COMMAND: {
	...
	switch (*p)
	{
		...
		lookupid:
			...
			if (id)
				...
				{
					case ID_VAR:
						code.add(CODE_IVAR | RET_STR | (id->index << 8));
						goto done;
					case ID_FVAR:
						code.add(CODE_FVAR | RET_STR | (id->index << 8));
						goto done;
					case ID_SVAR:
						code.add(CODE_SVAR | RET_STR | (id->index << 8));
						goto done;
					case ID_ALIAS:
						code.add((id->index < MAXARGS ? CODE_LOOKUPARG : CODE_LOOKUP) | RET_STR | (id->index << 8));
						goto done;
				}
			compilestr(code, lookup, lookuplen, true);
			code.add(CODE_LOOKUPU | RET_STR);
...
static void compileblock(vector<uint> &code, const char *&p, int wordtype)
{
	...
	switch (wordtype)
	{
		...
		default:
			...
			{
				if (p - 1 <= start)
					compileval(code, wordtype, NULL, 0);
				else
...
static bool compileword(vector<uint> &code, const char *&p, int wordtype, char *&word, int &wordlen)
{
	...
	{
		case '\"':
			word = cutstring(p, wordlen);
			break;
		case '$':
			compilelookup(code, p, wordtype);
			return true;
		case '(':
			p++;
			code.add(CODE_ENTER);
...
static void compilestatements(vector<uint> &code, const char *&p, int rettype, int brak)
{
				...
				case '\0':
					...
					if (idname)
					{
						...
						if (id)
							...
							{
								case ID_ALIAS:
									if (!(more = compilearg(code, p, VAL_ANY)))
										compilestr(code);
									code.add((id->index < MAXARGS ? CODE_ALIASARG : CODE_ALIAS) | (id->index << 8));
									goto endcommand;
								case ID_VAR:
									if (!(more = compilearg(code, p, VAL_INT)))
										compileint(code);
									code.add(CODE_IVAR1 | (id->index << 8));
									goto endcommand;
								case ID_FVAR:
									if (!(more = compilearg(code, p, VAL_FLOAT)))
										compilefloat(code);
									code.add(CODE_FVAR1 | (id->index << 8));
									goto endcommand;
								case ID_SVAR:
									if (!(more = compilearg(code, p, VAL_STR)))
										compilestr(code);
	...
	compilecommand:
		...
		{
		noid:
			while (numargs < MAXARGS && (more = compilearg(code, p, VAL_ANY)))
				numargs++;
			code.add(CODE_CALLU);
		}
		else
		{
			...
			if (!id)
			{
				...
				switch (id->type)
				{
					...
					case ID_COMMAND: {
						...
						for (const char *fmt = id->args; *fmt; fmt++)
							switch (*fmt)
							{
								case 's':
									if (more)
										more = compilearg(code, p, VAL_STR);
									if (!more)
									{
										if (rep)
											break;
									...
									{
										int numconc = 0;
										while (numargs + numconc < MAXARGS && (more = compilearg(code, p, VAL_STR)))
											numconc++;
										if (numconc > 0)
											code.add(CODE_CONC | RET_STR | ((numconc + 1) << 8));
									}
									numargs++;
									break;
								case 'i':
									if (more)
										more = compilearg(code, p, VAL_INT);
									if (!more)
									{
										if (rep)
											break;
								...
								case 'f':
									if (more)
										more = compilearg(code, p, VAL_FLOAT);
									if (!more)
									{
										if (rep)
											break;
								...
								case 't':
									if (more)
										more = compilearg(code, p, VAL_ANY);
									if (!more)
									{
										if (rep)
											break;
					...
					case ID_LOCAL:
						if (more)
							while (numargs < MAXARGS && (more = compilearg(code, p, VAL_IDENT)))
								numargs++;
						if (more)
							while ((more = compilearg(code, p, VAL_ANY)))
								code.add(CODE_POP);
						code.add(CODE_LOCAL);
						break;
					case ID_VAR:
						...
						else if (!(id->flags & IDF_HEX) || !(more = compilearg(code, p, VAL_INT)))
							code.add(CODE_IVAR1 | (id->index << 8));
						else if (!(more = compilearg(code, p, VAL_INT)))
							code.add(CODE_IVAR2 | (id->index << 8));
						else
							code.add(CODE_IVAR3 | (id->index << 8));
						break;
					case ID_FVAR:
						if (!(more = compilearg(code, p, VAL_FLOAT)))
							code.add(CODE_PRINT | (id->index << 8));
					...
					case ID_SVAR:
						...
						{
							int numconc = 0;
							while (numconc + 1 < MAXARGS && (more = compilearg(code, p, VAL_ANY)))
								numconc++;
							if (numconc > 0)
								code.add(CODE_CONC | RET_STR | ((numconc + 1) << 8));
							code.add(CODE_SVAR1 | (id->index << 8));
	...
	endstatement:
		if (more)
			while (compilearg(code, p, VAL_ANY))
				code.add(CODE_POP);
		p += strcspn(p, ")];/\n\0");
		int c = *p++;
		switch (c)
...
{
	if (i < 0)
		conoutf(CON_INFO, id->index, "%s = %d", id->name, i);
	else if (id->flags & IDF_HEX && id->maxval == 0xFFFFFF)
		conoutf(CON_INFO, id->index, "%s = 0x%.6X (%d, %d, %d)", id->name, i, (i >> 16) & 0xFF, (i >> 8) & 0xFF,
				i & 0xFF);
	else
...
template <class V> static void printvar(ident *id, int type, V &val)
{
	switch (type)
	{
		case VAL_INT:
			printvar(id, val.getint());
			break;
		case VAL_FLOAT:
			printfvar(id, val.getfloat());
			break;
		default:
			printsvar(id, val.getstr());
			break;
	}
}
...
typedef void(__cdecl *comfun12)(void *, void *, void *, void *, void *, void *, void *, void *, void *, void *, void *,
								void *);
	...
	for (const char *fmt = id->args; *fmt; fmt++)
		switch (*fmt)
		{
			...
			case 'D':
				if (++i < numargs)
					freearg(args[i]);
				args[i].setint(addreleaseaction(conc(args, i, true, id->name)) ? 1 : 0);
				fakeargs++;
				break;
#endif
			case 'C': {
...
#define CALLCOM(n)                                                                                                     \
	...
	{                                                                                                                  \
		case 0:                                                                                                        \
			...
			break;                                                                                                     \
		case 1:                                                                                                        \
			((comfun1)id->fun)(ARG(0));                                                                                \
			break;                                                                                                     \
		case 2:                                                                                                        \
			((comfun2)id->fun)(ARG(0), ARG(1));                                                                        \
			break;                                                                                                     \
		case 3:                                                                                                        \
			((comfun3)id->fun)(ARG(0), ARG(1), ARG(2));                                                                \
			break;                                                                                                     \
		case 4:                                                                                                        \
			((comfun4)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3));                                                        \
			break;                                                                                                     \
		case 5:                                                                                                        \
			((comfun5)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4));                                                \
			break;                                                                                                     \
		case 6:                                                                                                        \
			((comfun6)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5));                                        \
			break;                                                                                                     \
		case 7:                                                                                                        \
			((comfun7)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6));                                \
			break;                                                                                                     \
		case 8:                                                                                                        \
			((comfun8)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7));                        \
			break;                                                                                                     \
		case 9:                                                                                                        \
			((comfun9)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7), ARG(8));                \
			break;                                                                                                     \
		case 10:                                                                                                       \
			((comfun10)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7), ARG(8), ARG(9));       \
			break;                                                                                                     \
		case 11:                                                                                                       \
			((comfun11)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7), ARG(8), ARG(9),        \
								ARG(10));                                                                              \
			break;                                                                                                     \
		case 12:                                                                                                       \
			((comfun12)id->fun)(ARG(0), ARG(1), ARG(2), ARG(3), ARG(4), ARG(5), ARG(6), ARG(7), ARG(8), ARG(9),        \
								ARG(10), ARG(11));                                                                     \
			break;                                                                                                     \
	}
	++i;
...
#define LOOKUPU(aval, sval, ival, fval, nval)                                                                          \
	{                                                                                                                  \
		tagval &arg = args[numargs - 1];                                                                               \
		if (arg.type != VAL_STR && arg.type != VAL_MACRO)                                                              \
			continue;                                                                                                  \
		...
		continue;                                                                                                      \
				...
				LOOKUPU(arg.setstr(newstring(id->getstr())), arg.setstr(newstring(*id->storage.s)),
						arg.setstr(newstring(intstr(*id->storage.i))), arg.setstr(newstring(floatstr(*id->storage.f))),
	...
	{                                                                                                                  \
		...
		continue;                                                                                                      \
			...
			case CODE_LOOKUPU | RET_INT:
				LOOKUPU(arg.setint(id->getint()), arg.setint(parseint(*id->storage.s)), arg.setint(*id->storage.i),
						arg.setint(int(*id->storage.f)), arg.setint(0));
			...
			case CODE_LOOKUPU | RET_FLOAT:
				LOOKUPU(arg.setfloat(id->getfloat()), arg.setfloat(parsefloat(*id->storage.s)),
						arg.setfloat(float(*id->storage.i)), arg.setfloat(*id->storage.f), arg.setfloat(0.0f));
			...
			case CODE_LOOKUPU | RET_NULL:
				LOOKUPU(id->getval(arg), arg.setstr(newstring(*id->storage.s)), arg.setint(*id->storage.i),
						arg.setfloat(*id->storage.f), arg.setnull());
			...
			case CODE_SVAR | RET_FLOAT:
				...
				continue;
			case CODE_SVAR1:
				setsvarchecked(identmap[op >> 8], args[0].s);
				freeargs(args, numargs, 0);
				continue;
			...
			case CODE_IVAR | RET_FLOAT:
				...
				continue;
			case CODE_IVAR1:
				setvarchecked(identmap[op >> 8], args[0].i);
				numargs = 0;
				continue;
			case CODE_IVAR2:
				setvarchecked(identmap[op >> 8], (args[0].i << 16) | (args[1].i << 8));
				numargs = 0;
				continue;
			case CODE_IVAR3:
				setvarchecked(identmap[op >> 8], (args[0].i << 16) | (args[1].i << 8) | args[2].i);
				numargs = 0;
				continue;
			...
			case CODE_FVAR | RET_INT:
				...
				continue;
			case CODE_FVAR1:
				setfvarchecked(identmap[op >> 8], args[0].f);
				numargs = 0;
				continue;
			...
			case CODE_COMD | RET_STR:
			case CODE_COMD | RET_FLOAT:
			case CODE_COMD | RET_INT:
				id = identmap[op >> 8];
				...
				switch (id->type)
				{
					...
					case ID_VAR:
						if (numargs <= 1)
							...
							setvarchecked(id, &args[1], numargs - 1);
						goto forceresult;
					case ID_FVAR:
						if (numargs <= 1)
							printvar(id);
						else
							setfvarchecked(id, forcefloat(args[1]));
						goto forceresult;
					case ID_SVAR:
						if (numargs <= 1)
							printvar(id);
						else
							setsvarchecked(id, forcestr(args[1]));
						goto forceresult;
					case ID_ALIAS:
						if (id->index < MAXARGS && !(aliasstack->usedargs & (1 << id->index)))
							goto forceresult;
...
bool execfile(const char *cfgfile, bool msg)
{
	...
	{
		if (msg)
			conoutf(CON_ERROR, "could not read \"%s\"", cfgfile);
		return false;
	}
...
void writecfg(const char *name)
{
	...
	if (!f)
		return;
	f->printf("// automatically written on exit, DO NOT MODIFY\n// delete this file to have %s overwrite these "
...
ICOMMAND(loopconcat, "rie", (ident * id, int *n, uint *body), {
	if (*n > 0 && id->type == ID_ALIAS)
		commandret->setstr(loopconc(id, *n, body, true));
});

ICOMMAND(loopconcatword, "rie", (ident * id, int *n, uint *body), {
	if (*n > 0 && id->type == ID_ALIAS)
		commandret->setstr(loopconc(id, *n, body, false));
});
...
{
	if (id->type != ID_ALIAS || v->type == VAL_NULL)
		return;
	if (id->valtype == VAL_NULL)
	{
	noprefix:
...
static bool parselist(const char *&s, const char *&start = liststart, const char *&end = listend,
					  const char *&quotestart = listquotestart, const char *&quoteend = listquoteend)
{
	skiplist(s);
	switch (*s)
	{
...
void at(tagval *args, int numargs)
{
	...
	for (int i = 1; i < numargs; i++)
	{
		...
		for (; pos > 0; pos--)
			if (!parselist(list))
				break;
		if (pos > 0 || !parselist(list, start, end))
			start = end = "";
	}
...
void loopsublist(ident *id, const char *list, int *skip, int *count, const uint *body)
{
	...
	for (const char *s = list, *start, *end; parselist(s, start, end) && n < len; n++)
		if (n >= offset)
		{
			char *val = newstring(start, end - start);
			setiter(*id, val, stack);
...
void listfilter(ident *id, const char *list, const uint *body)
{
	...
	vector<char> r;
	int n = 0;
	for (const char *s = list, *start, *end, *quotestart, *quoteend; parselist(s, start, end, quotestart, quoteend);
		 n++)
	{
		char *val = newstring(start, end - start);
		setiter(*id, val, stack);
...
ICOMMAND(loopfiles, "rsse", (ident * id, char *dir, char *ext, uint *body), {
	...
	loopvrev(files)
	{
		...
		loopj(i) if (!strcmp(files[j], file))
		{
			redundant = true;
			break;
		}
		if (redundant)
			delete[] files.removeunordered(i);
	}
	loopv(files)
	{
		char *file = files[i];
...
struct sortfun
{
	...
	{
		if (x->valtype != VAL_MACRO)
			x->valtype = VAL_MACRO;
		cleancode(*x);
		x->val.code = (const uint *)xval.str;
		if (y->valtype != VAL_MACRO)
			y->valtype = VAL_MACRO;
		cleancode(*y);
...
ICOMMAND(strstr, "ss", (char *a, char *b), {
	char *s = strstr(a, b);
	intret(s ? s - a : -1);
});
...
{
	int slen = strlen(s), vlen = strlen(vals), offset = clamp(*skip, 0, slen), len = clamp(*count, 0, slen - offset);
