...
static const bvec4 matnormals[6] = {bvec4(0x80, 0, 0), bvec4(0x7F, 0, 0), bvec4(0, 0x80, 0),
									bvec4(0, 0x7F, 0), bvec4(0, 0, 0x80), bvec4(0, 0, 0x7F)};
...
const struct material
{
	const char *name;
	ushort id;
} materials[] = {{"air", MAT_AIR},			 {"water", MAT_WATER},		{"water1", MAT_WATER},
				 {"water2", MAT_WATER + 1},	 {"water3", MAT_WATER + 2}, {"water4", MAT_WATER + 3},
				 {"glass", MAT_GLASS},		 {"glass1", MAT_GLASS},		{"glass2", MAT_GLASS + 1},
				 {"glass3", MAT_GLASS + 2},	 {"glass4", MAT_GLASS + 3}, {"lava", MAT_LAVA},
				 {"lava1", MAT_LAVA},		 {"lava2", MAT_LAVA + 1},	{"lava3", MAT_LAVA + 2},
				 {"lava4", MAT_LAVA + 3},	 {"clip", MAT_CLIP},		{"noclip", MAT_NOCLIP},
				 {"gameclip", MAT_GAMECLIP}, {"death", MAT_DEATH},		{"alpha", MAT_ALPHA}};
...
void setupmaterials(int start, int len)
{
		...
		loopj(va->matsurfs)
		{
			...
			if (matvol == MAT_WATER && m.orient == O_TOP)
			{
				...
				loopvk(water)
				{
					...
					if (m.material != n.material || m.o.z != n.o.z)
						continue;
					if (n.o.x + n.rsize == m.o.x || m.o.x + m.rsize == n.o.x)
					{
						if (n.o.y + n.csize > m.o.y && n.o.y < m.o.y + m.csize)
							uf.unite(m.index, n.index);
					}
					else if (n.o.y + n.csize == m.o.y || m.o.y + m.csize == n.o.y)
					{
						if (n.o.x + n.rsize > m.o.x && n.o.x < m.o.x + m.rsize)
							uf.unite(m.index, n.index);
					}
				}
	...
	loopv(water)
	{
		...
		if (m.light && (!m.light->attr1 || !n.light || (n.light->attr1 && m.light->attr1 > n.light->attr1)))
			n.light = m.light;
...
static inline bool vismatcmp(const materialsurface *xm, const materialsurface *ym)
{
	...
	if (!sortedit)
	{
		if ((x.material & MATF_VOLUME) == MAT_LAVA)
		{
			if ((y.material & MATF_VOLUME) != MAT_LAVA)
				return true;
		}
		else if ((y.material & MATF_VOLUME) == MAT_LAVA)
			return false;
	}
...
void sortmaterials(vector<materialsurface *> &vismats)
{
	...
	if (reflecting)
		sortorigin.z = int(reflectz - (camera1->o.z - reflectz));
	vec dir;
	...
	{
		if (!va->matsurfs || va->occluded >= OCCLUDE_BB)
			continue;
		...
		loopi(va->matsurfs)
		{
			...
			{
				int matvol = m.material & MATF_VOLUME;
				if (matvol == MAT_WATER && (m.orient == O_TOP || (refracting < 0 && reflectz > worldsize)))
				{
					i += m.skip;
					continue;
				}
...
void rendermatgrid(vector<materialsurface *> &vismats)
{
	...
	loopvrev(vismats)
	{
		...
		if (m.material != lastmat)
		{
			...
			{
				case MAT_WATER:
					color = bvec4(0, 0, 85, 255);
					break; // blue
				case MAT_CLIP:
					color = bvec4(85, 0, 0, 255);
					break; // red
				case MAT_GLASS:
					color = bvec4(0, 85, 85, 255);
					break; // cyan
				case MAT_NOCLIP:
					color = bvec4(0, 85, 0, 255);
					break; // green
				case MAT_LAVA:
					color = bvec4(85, 40, 0, 255);
					break; // orange
				case MAT_GAMECLIP:
					color = bvec4(85, 85, 0, 255);
					break; // yellow
				case MAT_DEATH:
					color = bvec4(40, 40, 40, 255);
					break; // black
				case MAT_ALPHA:
					color = bvec4(85, 0, 85, 255);
					break; // pink
				default:
					continue;
			}
	...
	if (editmode && showmat && !drawtex)
	{
		...
		loopv(vismats)
		{
			...
			if (lastmat != m.material)
			{
				...
				{
					case MAT_WATER:
						color = bvec4(255, 128, 0, 255);
						break; // blue
					case MAT_CLIP:
						color = bvec4(0, 255, 255, 255);
						break; // red
					case MAT_GLASS:
						color = bvec4(255, 0, 0, 255);
						break; // cyan
					case MAT_NOCLIP:
						color = bvec4(255, 0, 255, 255);
						break; // green
					case MAT_LAVA:
						color = bvec4(0, 128, 255, 255);
						break; // orange
					case MAT_GAMECLIP:
						color = bvec4(0, 0, 255, 255);
						break; // yellow
					case MAT_DEATH:
						color = bvec4(192, 192, 192, 255);
						break; // black
					case MAT_ALPHA:
						color = bvec4(0, 255, 0, 255);
						break; // pink
					default:
						continue;
				}
		...
		loopv(vismats)
		{
			...
			if (lastmat != m.material || lastorient != m.orient ||
				(matvol == MAT_GLASS && envmapped && m.envmap != envmapped))
			{
				...
				switch (matvol)
				{
					case MAT_WATER:
						...
						if (!mslot->loaded || !mslot->sts.inrange(1))
							continue;
						else
						{
							...
							if (wfcol.iszero())
								wfcol = getwatercolor(m.material);
							gle::color(wfcol, 192);
							...
							else
							{
								...
								{
									if (waterfallenv)
										SETSHADER(waterfallenvrefract);
									else
										SETSHADER(waterfallrefract);
									if (blended)
									{
										glDisable(GL_BLEND);
										blended = false;
								...
								if (usedwaterfall != m.material)
								{
									...
									glBindTexture(GL_TEXTURE_2D,
												  mslot->sts.inrange(4) ? mslot->sts[4].t->id : notexture->id);
									glActiveTexture_(GL_TEXTURE2);
									glBindTexture(GL_TEXTURE_2D,
												  mslot->sts.inrange(5) ? mslot->sts[5].t->id : notexture->id);
									if (waterfallenv)
									{
					...
					case MAT_LAVA:
						if (lastmat == m.material && lastorient != O_TOP && m.orient != O_TOP)
							break;
						mslot = &lookupmaterialslot(m.material, false);
						if (!mslot->loaded)
							continue;
						...
						if (m.orient != O_TOP)
						{
							...
							setuplava(mslot->sts[0].t, mslot->scale);
						break;

					case MAT_GLASS:
						if ((m.envmap == EMID_NONE || !glassenv || envmapped == m.envmap) && lastmat == m.material)
							break;
						changematerial(lastmat, lastorient);
