...
struct editor
{
	...
	editor(const char *name, int mode, const char *initval)
		: mode(mode), active(true), rendered(false), name(newstring(name)), filename(NULL), cx(0), cy(0), mx(-1),
		  maxx(-1), maxy(-1), scrolly(0), linewrap(false), pixelwidth(-1), pixelheight(-1)
	{
		// printf("editor %08x '%s'\n", this, name);
	...
	void draw(int x, int y, int color, bool hit)
	{
		...
		for (int i = scrolly; i < lines.length(); i++)
		{
			...
			draw_text(lines[i].text, x, y + h, color >> 16, (color >> 8) & 0xFF, color & 0xFF, 0xFF,
					  hit && (cy == i) ? cx : -1, maxwidth);
...
#define TEXTCOMMAND(f, s, d, body)                                                                                     \
	ICOMMAND(f, s, d, editor *top = currentfocus(); if (!top || identflags & IDF_OVERRIDDEN) return; body)

ICOMMAND(
	textlist, "", (), // @DEBUG return list of all the editors
	vector<char> s;
	loopv(editors) {
		if (i > 0)
			s.put(", ", 2);
...
TEXTCOMMAND(textshow, "", (), // @DEBUG return the start of the buffer
			editline line;
			line.combinelines(top->lines); result(line.text); line.clear(););
ICOMMAND(textfocus, "si",
...
TEXTCOMMAND(textmode, "i", (int *m), // (1= keep while focused, 2= keep while used in gui, 3= keep forever (i.e. until
									 // mode changes)) topmost editor, return current setting if no args
			if (*m) top->mode = *m;
...
TEXTCOMMAND(textsave, "s", (char *file), // saves the topmost (filename is optional)
			...
			top->save(););
TEXTCOMMAND(
	textload, "s", (char *file), // loads into the topmost editor, returns filename if no args
	if (*file) {
		top->setfile(path(file, true));
		top->load();
	} else if (top->filename) result(top->filename););
TEXTCOMMAND(textinit, "sss",
			(char *name, char *file,
