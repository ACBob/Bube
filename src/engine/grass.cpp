...
static void gengrassquads(grassgroup *&group, const grasswedge &w, const grasstri &g, Texture *tex)
{
	...
	for (int i = maxstep; i >= minstep;
		 i--, offset--, leftp.add(leftdir), rightp.add(rightdir), leftb += leftdb, rightb += rightdb, dist -= grassstep)
	{
		if (dist <= leftdist)
		{
		...
		{
			if (w.bound1.dist(p2) >= grassmargin)
				continue;
			p1.add(vec(across).mul(leftb - grassmargin));
		}
		if (rightb > grassmargin)
		{
			if (w.bound2.dist(p1) >= grassmargin)
				continue;
		...
		svec2 lm1(short(g.tcu.dot(p1)), short(g.tcv.dot(p1))), lm2(short(g.tcu.dot(p2)), short(g.tcv.dot(p2)));
...
static void gengrassquads(vtxarray *va)
{
	loopv(va->grasstris)
	{
		...
		loopi(NUMGRASSWEDGES)
		{
			...
			if (w.bound1.dist(g.center) > g.radius + grassmargin || w.bound2.dist(g.center) > g.radius + grassmargin)
				continue;
...
void rendergrass()
{
	if (!grass || !grassdist || grassgroups.empty() || dbggrass)
		return;
	...
	LOCALPARAMF(grassmargin, grassmargin, grassmargin ? grassmarginfade / grassmargin : 0.0f,
				grassmargin ? grassmarginfade : 1.0f);
	...
	loopv(grassgroups)
	{
		...
		{
			if (refracting < 0 ? g.tri->minz > reflectz : g.tri->maxz + grassheight < reflectz)
				continue;
