...
void fatal(const char *fmt, ...)
{
	...
	if (logfile)
		logoutf("%s", msg);
	#ifdef WIN32
	MessageBox(NULL, msg, "BubeEngine fatal error", MB_OK | MB_SYSTEMMODAL);
	#else
...
void checkserversockets() // reply all server info requests
{
	...
	if (enet_socketset_select(maxsock, &readset, &writeset, 0) <= 0)
		return;
	...
	loopi(2)
	{
		...
		if (sock == ENET_SOCKET_NULL || !ENET_SOCKETSET_CHECK(readset, sock))
			continue;

		buf.data = pong;
		buf.dataLength = sizeof(pong);
		int len = enet_socket_receive(sock, &pongaddr, &buf, 1);
		if (len < 0 || len > MAXPINGDATA)
			continue;
...
SVAR(serverip, "");
VARF(serverport, 0, server::serverport(), 0xFFFF - 1, {
	if (!serverport)
				 ...
				 uint timeout) // main server update, called from main loop in sp, or from below in dedicated server
{
	...
	if (totalmillis - laststatus > 60 * 1000) // display bandwidth stats, useful for server ops
	{
		...
		if (nonlocalclients || serverhost->totalSentData || serverhost->totalReceivedData)
			logoutf("status: %d remote clients, %.1f send, %.1f rec (K/sec)", nonlocalclients,
					serverhost->totalSentData / 60.0f / 1024, serverhost->totalReceivedData / 60.0f / 1024);
		...
		case WM_APP:
			...
			switch (lParam)
			{
				...
				case WM_RBUTTONUP: {
					POINT pos;
					GetCursorPos(&pos);
					TrackPopupMenu(appmenu, TPM_CENTERALIGN | TPM_BOTTOMALIGN | TPM_RIGHTBUTTON, pos.x, pos.y, 0, hWnd,
								   NULL);
...
static void setupwindow(const char *title)
{
	...
	appwindow = CreateWindow(MAKEINTATOM(wndclass), title, 0, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, HWND_MESSAGE, NULL,
							 appinstance, NULL);
	if (!appwindow)
...
bool setuplistenserver(bool dedicated)
{
	...
	serverhost = enet_host_create(&address, min(maxclients + server::reserveclients(), MAXCLIENTS),
								  server::numchannels(), 0, serveruprate);
	if (!serverhost)
	...
	if (pongsock == ENET_SOCKET_NULL)
		return servererror(dedicated, "could not create server info socket");
	else
...
void startlistenserver(int *usemaster)
{
	...
	conoutf("listen server started for %d clients%s", maxclients,
...
bool serveroption(char *opt)
{
	switch (opt[1])
	{
		...
		case 'm':
			...
			setvar("updatemaster", mastername[0] ? 1 : 0);
			return true;
#ifdef STANDALONE
		case 'q':
