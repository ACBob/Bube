...
struct verthash
{
	...
	int addvert(const vec &pos, const vec2 &tc = vec2(0, 0), const svec2 &lm = svec2(0, 0),
				const bvec &norm = bvec(128, 128, 128), const bvec4 &tangent = bvec4(128, 128, 128, 128))
	{
		vertex vtx;
		vtx.pos = pos;
		vtx.tc = tc;
...
struct sortkey
{
	...
	{
		return tex == o.tex && lmid == o.lmid && envmap == o.envmap && dim == o.dim && layer == o.layer &&
			   alpha == o.alpha;
	}
};
...
struct vacollect : verthash
{
	...
	{
		uint lastlmid[8] = {LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT,
							LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT},
			 firstlmid[8] = {LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT,
							 LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT, LMID_AMBIENT};
		...
		{
			int offset = 2 * j;
			if (firstlmid[offset] == LMID_AMBIENT && firstlmid[offset + 1] == LMID_AMBIENT)
				continue;
			loopi(max(firstlit[offset], firstlit[offset + 1]))
			{
				sortkey &k = texs[i];
				if ((j ? k.layer != LAYER_BLEND : k.layer == LAYER_BLEND) || k.alpha)
					continue;
				if (k.lmid != LMID_AMBIENT)
					continue;
				Shader *s = lookupvslot(k.tex, false).slot->shader;
				int type = offset + (s->type & SHADER_NORMALSLMS ? LM_BUMPMAP0 : LM_DIFFUSE);
				if (firstlmid[type] == LMID_AMBIENT)
					continue;
		...
		loopv(remap)
		{
			...
			if (dst)
				loopl(2) loopvj(t.tris[l]) dst->tris[l].add(t.tris[l][j]);
		}
	}
	...
	void optimize()
	{
		...
		enumeratekt(
			indices, sortkey, k, sortval, t, loopl(2) if (t.tris[l].length() && t.unlit <= 0) {
...
#define GENVERTS(type, ptr, body)                                                                                      \
	...
	void setupdata(vtxarray *va)
	{
		...
		if (va->sky + va->explicitsky)
		{
			...
			if (va->voffset)
				loopi(va->sky + va->explicitsky) skydata[i] += va->voffset;
		}
		...
		if (va->texs)
		{
			...
			loopv(texs)
			{
				...
				{
					va->texs--;
					va->tris -= e.length[1] / 3;
					va->blends++;
					va->blendtris += e.length[1] / 3;
				}
				...
				else if (k.alpha == ALPHA_FRONT)
				{
					...
					va->tris -= e.length[1] / 3;
					va->alphafront++;
					va->alphafronttris += e.length[1] / 3;
				}
	...
	bool emptyva()
	{
		return verts.empty() && matsurfs.empty() && skyindices.empty() && explicitskyindices.empty() &&
			   grasstris.empty() && mapmodels.empty();
	}
} vc;
...
extern const vec orientation_tangent[8][3] = {
	{vec(0, 1, 0), vec(1, 0, 0), vec(1, 0, 0)},	   {vec(0, 0, -1), vec(0, 0, -1), vec(0, 1, 0)},
	{vec(0, -1, 0), vec(-1, 0, 0), vec(-1, 0, 0)}, {vec(0, 0, 1), vec(0, 0, 1), vec(0, -1, 0)},
	{vec(0, -1, 0), vec(-1, 0, 0), vec(-1, 0, 0)}, {vec(0, 1, 0), vec(1, 0, 0), vec(1, 0, 0)},
	{vec(0, 0, -1), vec(0, 0, -1), vec(0, 1, 0)},  {vec(0, 0, 1), vec(0, 0, 1), vec(0, -1, 0)},
};
extern const vec orientation_bitangent[8][3] = {
	{vec(0, 0, -1), vec(0, 0, -1), vec(0, 1, 0)}, {vec(0, -1, 0), vec(-1, 0, 0), vec(-1, 0, 0)},
	{vec(0, 0, 1), vec(0, 0, 1), vec(0, -1, 0)},  {vec(0, 1, 0), vec(1, 0, 0), vec(1, 0, 0)},
	{vec(0, 0, -1), vec(0, 0, -1), vec(0, 1, 0)}, {vec(0, 0, 1), vec(0, 0, 1), vec(0, -1, 0)},
	{vec(0, 1, 0), vec(1, 0, 0), vec(1, 0, 0)},	  {vec(0, -1, 0), vec(-1, 0, 0), vec(-1, 0, 0)},
};

void addtris(const sortkey &key, int orient, vertex *verts, int *index, int numverts, int convex, int shadowmask,
			 int tj)
{
	...
	{
		vector<ushort> &idxs =
			key.tex == DEFAULT_SKY ? vc.explicitskyindices : vc.indices[key].tris[(shadowmask >> i) & 1];
...
void addgrasstri(int face, vertex *verts, int numv, ushort texture, ushort lmid)
{
	...
	float tc1u = verts[i1].lm.x, tc1v = verts[i1].lm.y, tc2u = verts[i2].lm.x - verts[i1].lm.x,
		  tc2v = verts[i2].lm.y - verts[i1].lm.y, tc3u = verts[i3].lm.x - verts[i1].lm.x,
...
static inline void calctexgen(VSlot &vslot, int dim, vec4 &sgen, vec4 &tgen)
{
	...
	float k = TEX_SCALE / vslot.scale, xs = r.flipx ? -tex->xs : tex->xs, ys = r.flipy ? -tex->ys : tex->ys,
		  sk = k / xs, tk = k / ys, soff = -(r.swapxy ? vslot.offset.y : vslot.offset.x) / xs,
...
void addcubeverts(VSlot &vslot, int orient, int size, vec *pos, int convex, ushort texture, ushort lmid,
				  vertinfo *vinfo, int numverts, int tj = -1, ushort envmap = EMID_NONE, int grassy = 0,
				  bool alpha = false, int layer = LAYER_TOP)
{
	...
	if (lightmaps.inrange(lmid - LMID_RESERVED))
	{
		...
		if ((lm->type & LM_TYPE) == LM_DIFFUSE ||
			((lm->type & LM_TYPE) == LM_BUMPMAP0 && lightmaps.inrange(lmid + 1 - LMID_RESERVED) &&
	...
	loopk(numverts)
	{
		...
		if (lmtex)
		{
			v.lm = svec2(short(ceil((lm->offsetx + vinfo[k].u * (float(LM_PACKW) / float(USHRT_MAX + 1)) + 0.5f) *
									float(SHRT_MAX) / lmtex->w)),
						 short(ceil((lm->offsety + vinfo[k].v * (float(LM_PACKH) / float(USHRT_MAX + 1)) + 0.5f) *
									float(SHRT_MAX) / lmtex->h)));
		}
		else
			v.lm = svec2(0, 0);
		...
		{
			v.norm =
				vinfo && vinfo[k].norm && envmap != EMID_NONE ? bvec(decodenormal(vinfo[k].norm)) : bvec(128, 128, 255);
	...
	if (lmid >= LMID_RESERVED)
		lmid = lm ? lm->tex : LMID_AMBIENT;

	sortkey key(texture, lmid, !vslot.scroll.iszero() ? dim : 3, layer == LAYER_BLEND ? LAYER_BLEND : LAYER_TOP, envmap,
				alpha ? (vslot.alphaback ? ALPHA_BACK : (vslot.alphafront ? ALPHA_FRONT : NO_ALPHA)) : NO_ALPHA);
	...
	if (grassy)
	{
		for (int i = 0; i < numverts - 2; i += 2)
		{
			...
			if (i + 3 < numverts && index[0] != index[i + 2] && index[i + 2] != index[i + 3] &&
				...
				faces |= 2;
			if (grassy > 1 && faces == 3)
				addgrasstri(i, verts, 4, texture, lmid);
			else
			{
				if (faces & 1)
					addgrasstri(i, verts, 3, texture, lmid);
				if (faces & 2)
					addgrasstri(i + 1, verts, 3, texture, lmid);
			}
		}
	}
}
		...
		loopj(numverts)
		{
			...
			if (exists)
			{
				...
				while (cur >= 0)
				{
					...
					{
						if (ce.offset < p.offset)
							break;
						if (p.flags & CE_DUP ? ce.offset + ce.size <= p.offset + p.size
...
void gencubeverts(cube &c, const ivec &co, int size, int csi)
{
	...
	loopi(6) if (vismask & (1 << i) && (vis = visibletris(c, i, co, size)))
	{
		...
		if (!c.ext)
			addcubeverts(vslot, i, size, pos, convex, c.texture[i], LMID_AMBIENT, NULL, numverts, hastj, envmap, grassy,
						 (c.material & MAT_ALPHA) != 0);
		else
		{
			const surfaceinfo &surf = c.ext->surfaces[i];
			if (!surf.numverts || surf.numverts & LAYER_TOP)
				addcubeverts(vslot, i, size, pos, convex, c.texture[i], surf.lmid[0], verts, numverts, hastj, envmap,
							 grassy, (c.material & MAT_ALPHA) != 0, LAYER_TOP | (surf.numverts & LAYER_BLEND));
			if (surf.numverts & LAYER_BOTTOM)
				addcubeverts(layer ? *layer : vslot, i, size, pos, convex, vslot.layer, surf.lmid[1],
							 surf.numverts & LAYER_DUP ? verts + numverts : verts, numverts, hastj, envmap2);
		}
	}
}
...
static inline int hasskyfaces(cube &c, const ivec &co, int size, int faces[6] = dummyskyfaces)
{
	...
	{
		if (co.x == 0)
			faces[numfaces++] = O_LEFT;
		if (co.x + size == worldsize)
			faces[numfaces++] = O_RIGHT;
		if (co.y == 0)
			faces[numfaces++] = O_BACK;
		if (co.y + size == worldsize)
			faces[numfaces++] = O_FRONT;
		if (co.z == 0)
			faces[numfaces++] = O_BOTTOM;
		if (co.z + size == worldsize)
			faces[numfaces++] = O_TOP;
	}
	else if (!isentirelysolid(c))
	{
		if (co.x == 0 && !skyoccluded(c, O_LEFT))
			faces[numfaces++] = O_LEFT;
		if (co.x + size == worldsize && !skyoccluded(c, O_RIGHT))
			faces[numfaces++] = O_RIGHT;
		if (co.y == 0 && !skyoccluded(c, O_BACK))
			faces[numfaces++] = O_BACK;
		if (co.y + size == worldsize && !skyoccluded(c, O_FRONT))
			faces[numfaces++] = O_FRONT;
		if (co.z == 0 && !skyoccluded(c, O_BOTTOM))
			faces[numfaces++] = O_BOTTOM;
		if (co.z + size == worldsize && !skyoccluded(c, O_TOP))
			faces[numfaces++] = O_TOP;
	}
	return numfaces;
}
...
void updatevabb(vtxarray *va, bool force)
{
	...
	if (va->skyfaces)
	{
		va->skyfaces |= 0x80;
		if (va->sky)
			loop(dim, 3) if (va->skyfaces & (3 << (2 * dim)))
			{
				int r = R[dim], c = C[dim];
				if ((va->skyfaces & (1 << (2 * dim)) && va->o[dim] < va->bbmin[dim]) ||
					(va->skyfaces & (2 << (2 * dim)) && va->o[dim] + va->size > va->bbmax[dim]) ||
					va->o[r] < va->bbmin[r] || va->o[r] + va->size > va->bbmax[r] || va->o[c] < va->bbmin[c] ||
					va->o[c] + va->size > va->bbmax[c])
				{
					va->skyfaces &= ~0x80;
					break;
				}
...
void addmergedverts(int level, const ivec &o)
{
	...
	loopv(mfl)
	{
		...
		addcubeverts(vslot, mf.orient, 1 << level, pos, 0, mf.tex, mf.lmid, mf.verts, numverts, mf.tjoints, mf.envmap,
					 grassy, (mf.mat & MAT_ALPHA) != 0, mf.numverts & LAYER_BLEND);
		vahasmerges |= MERGE_USE;
	}
	mfl.setsize(0);
}

void rendercube(cube &c, const ivec &co, int size, int csi,
				int &maxlevel) // creates vertices and indices ready to be put into a va
{
	...
	if (c.children)
	{
		...
		if (c.ext)
		{
			if (c.ext->ents && c.ext->ents->mapmodels.length())
