struct vertmodel : animmodel
{
	...
	struct vertmesh : mesh
	{
		...
		void calctangents(bool areaweight = true)
		{
			...
			mesh::calctangents(bumpverts, verts, tcverts, numverts, tris, numtris, areaweight,
							   ((vertmeshgroup *)group)->numframes);
		}
		...
		template <class T> static inline void fillvert(T &vv, int j, tcvert &tc, vert &v)
		{
			vv.tc = tc.tc;
		}
...
#define ipvertp(attrib)                                                                                                \
	v.attrib.lerp(pvert1[i].attrib, pvert2[i].attrib, as.prev.t)                                                       \
		.lerp(vec().lerp(vert1[i].attrib, vert2[i].attrib, as.cur.t), as.interp)
#define ipbvertp(attrib, type)                                                                                         \
	v.attrib.lerp(type().lerp(bpvert1[i].attrib, bpvert2[i].attrib, as.prev.t),                                        \
				  type().lerp(bvert1[i].attrib, bvert2[i].attrib, as.cur.t), as.interp)
		...
		body;                                                                                                          \
			...
			if (tangents)
			{
				...
				if (as.interp < 1)
					iploop(vvertbump, {
						ipvertp(pos);
						ipbvertp(tangent, vec4);
					}) else iploop(vvertbump, {
						ipvert(pos);
						ipbvert(tangent, vec4);
					})
			}
			else
			{
...
#undef ipbvertp
		...
		void render(const animstate *as, skin &s, vbocacheentry &vc)
		{
			...
			glDrawRangeElements_(GL_TRIANGLES, minvert, maxvert, elen, GL_UNSIGNED_SHORT,
								 &((vertmeshgroup *)group)->edata[eoffset]);
			glde++;
	...
	struct vertmeshgroup : meshgroup
	{
		...
		vertmeshgroup()
			: numframes(0), tags(NULL), numtags(0), edata(NULL), ebuf(0), vtangents(false), vlen(0), vertsize(0),
			  vdata(NULL)
		{
		}
		...
		void genvbo(bool tangents, vbocacheentry &vc)
		{
			...
			if (tangents)
				loopv(meshes)((vertmesh *)meshes[i])->calctangents();
...
template <class MDL> struct vertcommands : modelcommands<MDL, struct MDL::vertmesh>
{
	...
	static void loadpart(char *model, float *smooth)
	{
		...
		mdl.meshes = MDL::loading->sharemeshes(path(filename),
