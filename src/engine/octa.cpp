...
static void genedgespanvert(ivec &p, cube &c, vec &v)
{
	...
	if (plane1 == plane2)
		genvertp(c, p, p1, p2, plane1, true);
	if (plane1 == plane3)
		genvertp(c, p, p1, p2, plane1, true);
	if (plane2 == plane3)
		genvertp(c, p, p2, p3, plane2, true);
...
{
	if (isentirelysolid(c) || isempty(c))
		return;
	cube o = c;
...
template <class T> static inline void gencubevert(const cube &c, int i, T &v)
{
	switch (i)
	{
		default:
...
const uchar faceedgesidx[6][4] = // ordered edges surrounding each orient
	{
		...
		{4, 5, 8, 10}, {6, 7, 9, 11}, {8, 9, 0, 2}, {10, 11, 1, 3}, {0, 1, 4, 6}, {2, 3, 5, 7},
};
...
static inline int genfacevecs(const cube &cu, int orient, const ivec &pos, int size, bool solid, ivec2 *fvecs,
							  const ivec *v = NULL)
{
	int i = 0;
	if (solid)
...
#define GENFACEVERT(orient, vert, xv, yv, zv, x, y, z)                                                                 \
	{                                                                                                                  \
		...
		i++;                                                                                                           \
			...
			GENFACEVERTS(pos.x, pos.x + size, pos.y, pos.y + size, pos.z, pos.z + size, f.x =, f.x =, f.y =, f.y =,
						 (void), (void))
#undef GENFACEVERT
		}
		return 4;
...
static inline bool occludesface(const cube &c, int orient, const ivec &o, int size, const ivec &vo, int vsize,
								ushort vmat, ushort nmat, ushort matmask, const ivec2 *vf, int numv)
{
	int dim = dimension(orient);
	if (!c.children)
	{
...
void mincubeface(const cube &cu, int orient, const ivec &o, int size, const facebounds &orig, facebounds &cf,
				 ushort nmat, ushort matmask)
{
	int dim = dimension(orient);
	if (cu.children)
	{
