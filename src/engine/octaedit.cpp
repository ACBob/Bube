...
cube &blockcube(int x, int y, int z, const block3 &b,
				int rgrid) // looks up a world cube, based on coordinates mapped by the block
{
...
void rendereditcursor()
{
	...
	if (moving)
	{
		...
		if (editmoveplane(vec(sel.o), camdir, od, sel.o[D[od]] + odc * sel.grid * sel.s[D[od]], handle, dest,
						  moving == 1))
		{
			if (moving == 1)
			{
	...
	else
	{
		...
		wdist = rayent(player->o, camdir, 1e16f,
					   (editmode && showmat ? RAY_EDITMAT : 0) // select cubes first
						   ...
						   RAY_SKIPFIRST | (passthroughcube || passthrough ? RAY_PASS : 0),
					   gridsize, entorient, ent);
...
void changed(const block3 &sel, bool commit = true)
{
	if (sel.s.iszero())
		return;
	readychanges(ivec(sel.o).sub(1), ivec(sel.s).mul(sel.grid).add(sel.o).add(1), worldroot, ivec(0, 0, 0),
				 worldsize / 2);
	haschanged = true;
...
template <class B> static void packcube(cube &c, B &buf)
{
	if (c.children)
	{
		buf.put(0xFF);
...
template <class B> static bool packblock(block3 &b, B &buf)
{
...
template <class B> static void unpackcube(cube &c, B &buf)
{
	int mat = buf.get();
	if (mat == 0xFF)
	{
...
template <class B> static bool unpackblock(block3 *&b, B &buf)
{
	if (b)
	{
		freeblock(b);
...
COMMAND(hmapcancel, "");
ICOMMAND(hmapselect, "", (), int t = lookupcube(cur).texture[orient]; int i = htextures.find(t);
		 if (i < 0) htextures.add(t); else htextures.remove(i););

inline bool isheightmap(int o, int d, bool empty, cube *c)
{
	return havesel || (empty && isempty(*c)) || htextures.empty() || htextures.find(c->texture[o]) >= 0;
}
...
ICOMMAND(getvcolor, "i", (int *tex), {
	VSlot &vslot = lookupvslot(*tex, false);
	defformatstring(str, "%s %s %s", floatstr(vslot.colorscale.r), floatstr(vslot.colorscale.g),
					floatstr(vslot.colorscale.b));
	result(str);
});
...
ICOMMAND(getvshaderparam, "is", (int *tex, const char *name), {
	...
	loopv(vslot.params)
	{
		...
		{
			defformatstring(str, "%s %s %s %s", floatstr(p.val[0]), floatstr(p.val[1]), floatstr(p.val[2]),
							floatstr(p.val[3]));
			result(str);
			return;
		}
...
void mprotate(int cw, selinfo &sel, bool local)
{
	...
	loop(z, sel.s[D[d]]) loopi(cw > 0 ? 1 : 3)
	{
		...
		loop(y, ss / 2) loop(x, ss - 1 - y * 2)
			rotatequad(selcube(ss - 1 - y, x + y, z), selcube(x + y, y, z), selcube(y, ss - 1 - x - y, z),
...
struct texturegui : g3d_callback
{
	...
	{
		int origtab = menutab,
			numtabs = max((slots.length() + texguiwidth * texguiheight - 1) / (texguiwidth * texguiheight), 1);
		if (!firstpass)
			texguinum = -1;
		...
		loopi(numtabs)
		{
			...
			if (i + 1 != origtab)
				continue; // don't load textures on non-visible tabs!
			Slot *rollover = NULL;
			loop(h, texguiheight)
			{
				g.pushlist();
	...
	void showtextures(bool on)
	{
		...
		if ((menuon = on))
		{
			if (menustart <= lasttexmillis)
				menutab = 1 + clamp(lookupvslot(lasttex, false).slot->index, 0, slots.length() - 1) /
								  (texguiwidth * texguiheight);
	...
	void show()
	{
		...
		filltexlist();
		extern int usegui2d;
		if (!editmode || ((!texgui2d || !usegui2d) && camera1->o.dist(menupos) > menuautoclose))
		{
			menuon = false;
			texguinum = -1;
		}
