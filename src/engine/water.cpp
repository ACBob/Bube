...
VARFP(waterreflect, 0, 1, 1, {
	cleanreflections();
	preloadwatershaders();
});
VARFP(waterrefract, 0, 1, 1, {
	cleanreflections();
	preloadwatershaders();
});
VARFP(waterenvmap, 0, 1, 1, {
	cleanreflections();
	preloadwatershaders();
});
VARFP(waterfallrefract, 0, 0, 1, {
	cleanreflections();
	preloadwatershaders();
});
...
struct waterstrip
{
	...
	int numverts() const
	{
		return 2 * ((y2 - y1) / subdiv + 1) * ((x2 - x1) / subdiv);
	}
...
void rendervertwater(int subdiv, int xo, int yo, int z, int size, int mat)
{
	if (wsize == size && wsubdiv == subdiv && wz == z)
	{
		if (wx2 == xo)
		{
			if (wy1 == yo && wy2 == yo + size)
			{
				wx2 += size;
				return;
			}
		}
		else if (wy2 == yo && wx1 == xo && wx2 == xo + size)
		{
			wy2 += size;
			return;
		}
...
struct Reflection
{
	...
	Reflection()
		: tex(0), refracttex(0), material(-1), height(-1), depth(0), age(0), init(false), query(NULL), prevquery(NULL)
	{
	}
};
...
#define WATERVARS(name)                                                                                                \
	...
	HVARFR(name##fallcolour, 0, 0, 0xFFFFFF, {                                                                         \
		name##fallcolor =                                                                                              \
			bvec((name##fallcolour >> 16) & 0xFF, (name##fallcolour >> 8) & 0xFF, name##fallcolour & 0xFF);            \
	});
...
{
	return waterfallrefraction.refracttex ? waterfallrefraction.refracttex : notexture->id;
}

VAR(oqwater, 0, 2, 2);
VARFP(waterfade, 0, 1, 1, {
	cleanreflections();
	preloadwatershaders();
});
...
void preloadwatershaders(bool force)
{
	...
	if (waterenvmap && !waterreflect)
		...
		useshaderbyname(waterrefract ? (waterfade ? "waterfade" : "waterrefract")
									 : (waterreflect ? "waterreflect" : "water"));
	...
	if (waterfallrefract)
		useshaderbyname(waterfallenv ? "waterfallenvrefract" : "waterfallrefract");
}
...
#define SETWATERSHADER(which, name)                                                                                    \
	...
	if (glaring)
		SETWATERSHADER(above, waterglare);
	else if (drawtex == DRAWTEX_MINIMAP)
		aboveshader = notextureshader;
	else if (waterenvmap && !waterreflect)
	{
		if (waterrefract)
		{
			if (waterfade)
				SETWATERSHADER(above, waterenvfade);
			else
				SETWATERSHADER(above, waterenvrefract);
		}
		else
	...
	{
		if (waterfade)
			SETWATERSHADER(above, waterfade);
		else
			SETWATERSHADER(above, waterrefract);
	}
	...
	if (!glaring && drawtex != DRAWTEX_MINIMAP)
	{
		...
		{
			if (waterfade)
				SETWATERSHADER(below, underwaterfade);
			else
				SETWATERSHADER(below, underwaterrefract);
		}
		else
			SETWATERSHADER(below, underwater);
	}

	vec ambient(max(skylightcolor[0], ambientcolor[0]), max(skylightcolor[1], ambientcolor[1]),
				max(skylightcolor[2], ambientcolor[2]));
	float offset = -WATER_OFFSET;
	loopi(MAXREFLECTIONS)
	{
		Reflection &ref = reflections[i];
		if (ref.height < 0 || ref.age || ref.matsurfs.empty())
			continue;
		...
		if (!glaring && drawtex != DRAWTEX_MINIMAP)
		{
			...
			{
				if (waterreflect || !waterenvmap)
					glBindTexture(GL_TEXTURE_2D, waterreflect ? ref.tex : ref.refracttex);
		...
		loopvj(ref.matsurfs)
		{
			...
			if (light != lastlight)
			{
				...
				vec lightcol = (light ? vec(light->attr2, light->attr3, light->attr4) : vec(ambient))
								   .div(255.0f)
								   .mul(wspec / 100.0f);
...
void genwatertex(GLuint &tex, GLuint &fb, GLuint &db, bool refract = false)
{
	static const GLenum colorfmts[] = {GL_RGBA, GL_RGBA8, GL_RGB, GL_RGB8, GL_FALSE},
						depthfmts[] = {GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT16,
									   GL_DEPTH_COMPONENT32, GL_FALSE};
	...
	if (!colorfmt)
		colorfmt = colorfmts[find];

	delete[] buf;

	if (!db)
	{
		glGenRenderbuffers_(1, &db);
		depthfmt = GL_FALSE;
	}
	if (!depthfmt)
		glBindRenderbuffer_(GL_RENDERBUFFER, db);
	find = 0;
	do
	{
		if (!depthfmt)
			glRenderbufferStorage_(GL_RENDERBUFFER, depthfmts[find], size, size);
...
void addwaterfallrefraction(materialsurface &m)
{
	...
	if (!ref.refracttex)
		genwatertex(ref.refracttex, reflectionfb, reflectiondb);
}
...
void addreflection(materialsurface &m)
{
	...
	loopi(MAXREFLECTIONS)
	{
		...
		else if (!oldest || r.age > oldest->age)
			oldest = &r;
	}
	if (!ref)
	{
		if (!oldest || oldest->age < 0)
			return;
		ref = oldest;
	}
	...
	if (waterreflect && !ref->tex)
		genwatertex(ref->tex, reflectionfb, reflectiondb);
	if (waterrefract && !ref->refracttex)
		genwatertex(ref->refracttex, reflectionfb, reflectiondb, true);
}
...
void queryreflections()
{
	...
	for (vtxarray *va = visibleva; va; va = va->next)
	{
		...
		loopi(va->matsurfs)
		{
			...
			if (m.material != lastmat)
			{
				...
				if (m.orient != O_TOP)
				{
					if (!waterfallrefract || !getwaterfog(m.material))
					{
						i += m.skip;
						continue;
					}
	...
	loopi(MAXREFLECTIONS)
	{
		...
		{
			if (waterpvsoccluded(ref.height))
				ref.matsurfs.setsize(0);
		}
	}
	if (waterfallrefract)
	{
		...
		if (ref.height >= 0 && !ref.age && ref.matsurfs.length())
		{
			if (waterpvsoccluded(-1))
				ref.matsurfs.setsize(0);
		}
	}

	if ((editmode && showmat && !drawtex) || !oqfrags || !oqwater || drawtex == DRAWTEX_MINIMAP)
		return;
...
static bool calcscissorbox(Reflection &ref, int size, vec &clipmin, vec &clipmax, int &sx, int &sy, int &sw, int &sh)
{
	...
	{
		vec4 &p = v[i];
		camprojmatrix.transform(vec(i & 1 ? bbmax.x : bbmin.x, i & 2 ? bbmax.y : bbmin.y,
									(i & 4 ? bbmax.z + WATER_AMPLITUDE : bbmin.z - WATER_AMPLITUDE) - WATER_OFFSET),
								p);
		if (p.z >= -p.w)
		{
...
{
	if ((editmode && showmat && !drawtex) || drawtex == DRAWTEX_MINIMAP)
		return;
		...
		loopi(MAXREFLECTIONS)
		{
			...
			if (waterrefract && ref.refracttex)
			{
				...
				maskreflection(ref, offset, false,
							   refractclear || !wfog || (ref.depth >= 10000 && camera1->o.z >= ref.height + offset));
				if (wfog || waterfade)
				{
					savevfcP();
