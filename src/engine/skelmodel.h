...
struct skelmodel : animmodel
{
	...
	struct blendcombo
	{
		...
		void serialize(vvertw &v)
		{
			...
			else
			{
				...
				while (total > 255)
				{
					loopk(4) if (v.weights[k] > 0 && total > 255)
					{
						v.weights[k]--;
						total--;
					}
				...
				while (total < 255)
				{
					loopk(4) if (v.weights[k] < 255 && total < 255)
					{
						v.weights[k]++;
						total++;
					}
	...
	struct animcacheentry
	{
		...
		bool operator==(const animcacheentry &c) const
		{
			...
			return pitch == c.pitch && partmask == c.partmask && ragdoll == c.ragdoll &&
				   (!ragdoll || min(millis, c.millis) >= ragdoll->lastmove);
		}
	};
	...
	struct skelmesh : mesh
	{
		...
		template <class T> int genvbo(vector<ushort> &idxs, int offset, vector<T> &vverts)
		{
			voffset = offset;
			eoffset = idxs.length();
			loopi(numverts)
		...
		template <class T> int genvbo(vector<ushort> &idxs, int offset, vector<T> &vverts, int *htdata, int htlen)
		{
			voffset = offset;
			eoffset = idxs.length();
			minvert = 0xFFFF;
		...
		template <class T> static inline void fillvert(T &vv, int j, vert &v)
		{
			vv.tc = v.tc;
		}

		template <class T> void fillverts(T *vdata)
		{
			vdata += voffset;
			loopi(numverts) fillvert(vdata[i], i, verts[i]);
		}

		void interpverts(const dualquat *RESTRICT bdata1, const dualquat *RESTRICT bdata2, bool tangents,
						 void *RESTRICT vdata, skin &s)
		{
...
#define IPLOOP(type, dosetup, dotransform)                                                                             \
	...
	{                                                                                                                  \
		...
		dotransform;                                                                                                   \
			...
			{
				IPLOOP(vvertn, , { dst.norm = b.transformnormal(src.norm); });
			}
...
#undef IPLOOP
		...
		void setshader(Shader *s)
		{
			...
			if (glaring)
			{
				if (!g->skel->usegpuskel)
					s->setvariant(0, 1);
				else
					s->setvariant(min(maxweights, g->vweights), 1);
			}
		...
		void render(const animstate *as, skin &s, vbocacheentry &vc)
		{
			if (!Shader::lastshader)
				return;
			glDrawRangeElements_(GL_TRIANGLES, minvert, maxvert, elen, GL_UNSIGNED_SHORT,
								 &((skelmeshgroup *)group)->edata[eoffset]);
			glde++;
	...
	struct boneinfo
	{
		...
		boneinfo()
			: name(NULL), parent(-1), children(-1), next(-1), group(INT_MAX), scheduled(-1), interpindex(-1),
			  interpparent(-1), ragdollindex(-1), correctindex(-1), pitchscale(0), pitchoffset(0), pitchmin(0),
			  pitchmax(0)
		{
		}
		~boneinfo()
	...
	struct skeleton
	{
		...
		skeleton()
			: name(NULL), shared(0), bones(NULL), numbones(0), numinterpbones(0), numgpubones(0), numframes(0),
			  framebones(NULL), ragdoll(NULL), usegpuskel(false), blendoffsets(32)
		{
		}
		...
		void calcantipodes()
		{
			...
			loopv(schedule)
			{
				...
				if (i + 1 == schedule.length())
				{
					...
					loopj(numbones) if (bones[j].group < numbones && bones[j].scheduled < 0) conflict =
						min(conflict, abs(bones[j].group));
		...
		void remapbones()
		{
			...
			loopi(numbones)
			{
				...
				if (info.interpindex < 0)
					continue;
				for (int parent = info.parent; parent >= 0 && bones[parent].interpindex < 0;
					 parent = bones[parent].parent)
		...
		{
			return min(maxvsuniforms - reservevpparams - 10, maxskelanimdata) / 2;
		}
		...
		void calcpitchcorrects(float pitch, const vec &axis, const vec &forward)
		{
			...
			loopvj(pitchtargets)
			{
				...
				loopv(pitchcorrects)
				{
					...
					if (c.target != j)
						continue;
					float total = c.parent >= 0 ? pitchcorrects[c.parent].pitchtotal : 0, avail = tpitch - total,
...
#define INTERPBONE(bone)                                                                                               \
	...
	{                                                                                                                  \
		...
		void interpbones(const animstate *as, float pitch, const vec &axis, const vec &forward, int numanimparts,
						 const uchar *partmask, skelcacheentry &sc)
		{
			if (!sc.bdata)
			...
			loopi(numbones) if (bones[i].interpindex >= 0)
			{
				...
				if (as->cur.anim & ANIM_NOPITCH || (as->interp < 1 && as->prev.anim & ANIM_NOPITCH))
					angle *= (as->cur.anim & ANIM_NOPITCH ? 0 : as->interp) +
									   ...
									   ragdolldata *rdata)
		{
			...
			loopv(skelcache)
			{
				...
				if (c.pitch != pitch || c.partmask != partmask || c.ragdoll != rdata ||
					(rdata && c.millis < rdata->lastmove))
					goto mismatch;
				match = true;
				sc = &c;
				break;
	...
	struct skelmeshgroup : meshgroup
	{
		...
		skelmeshgroup()
			: skel(NULL), edata(NULL), ebuf(0), vtangents(false), vlen(0), vertsize(0), vblends(0), vweights(0),
			  vdata(NULL)
		{
		...
		void genvbo(bool tangents, vbocacheentry &vc)
		{
			...
			if (tangents)
				loopv(meshes)((skelmesh *)meshes[i])->calctangents();
...
#define SEARCHCACHE(cachesize, cacheentry, cache, reusecheck)                                                          \
	...
	return cache[cachesize - 1];
		...
		void preload(part *p)
		{
			...
			if (skel->shouldcleanup())
				skel->cleanup();
			else if (tangents != vtangents)
				cleanup();
			skel->preload();
			if (!vbocache->vbuf)
				genvbo(tangents, *vbocache);
		}
		...
		void render(const animstate *as, float pitch, const vec &axis, const vec &forward, dynent *d, part *p)
		{
			...
			skelcacheentry &sc = skel->checkskelcache(
				p, as, pitch, axis, forward,
				as->cur.anim & ANIM_RAGDOLL || !d || !d->ragdoll || d->ragdoll->skel != skel->ragdoll ? NULL
																									  : d->ragdoll);
			if (!(as->cur.anim & ANIM_NORENDER))
			{
	...
	struct skelpart : part
	{
		...
		{
			animpartmask *p =
				(animpartmask *)new uchar[sizeof(animpartmask) + ((skelmeshgroup *)meshes)->skel->numbones - 1];
...
struct skeladjustment
{
	...
	skeladjustment(float yaw, float pitch, float roll, const vec &translate)
		: yaw(yaw), pitch(pitch), roll(roll), translate(translate)
	{
	}
...
template <class MDL> struct skelcommands : modelcommands<MDL, struct MDL::skelmesh>
{
	...
	static void loadpart(char *meshfile, char *skelname, float *smooth)
	{
		if (!MDL::loading)
		{
			conoutf(CON_ERROR, "not loading an %s", MDL::formatname());
			return;
		}
		...
		mdl.meshes = MDL::loading->sharemeshes(path(filename), skelname[0] ? skelname : NULL,
	...
	static void settag(char *name, char *tagname, float *tx, float *ty, float *tz, float *rx, float *ry, float *rz)
	{
		...
		if (i >= 0)
		{
			...
			matrix4x3 m(matrix3(quat(sx * cy * cz - cx * sy * sz, cx * sy * cz + sx * cy * sz,
	...
	static void setpitchcorrect(char *name, char *targetname, float *scale, float *pitchmin, float *pitchmax)
	{
		...
		if (targetbone >= 0)
			loopv(skel->pitchtargets) if (skel->pitchtargets[i].bone == targetbone)
			{
				target = i;
				break;
			}
		...
		loopv(skel->pitchcorrects) if (bone <= skel->pitchcorrects[i].bone)
		{
			pos = i;
			break;
		}
	...
	static void setanimpart(char *maskstr)
	{
		...
		loopv(bonestrs)
		{
			...
			int bone =
				p->meshes ? ((meshgroup *)p->meshes)->skel->findbone(bonestr[0] == '!' ? bonestr + 1 : bonestr) : -1;
			if (bone < 0)
			{
