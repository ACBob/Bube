...
{
	fireballrenderer(const char *texname) : listrenderer(texname, 0, PT_FIREBALL | PT_GLARE | PT_SHADER)
	{
	}
	...
	int finddepthfxranges(void **owners, float *ranges, int numranges, int maxranges, vec &bbmin, vec &bbmax)
	{
		...
		for (listparticle *p = list; p; p = p->next)
		{
			...
			float pmax = p->val, size = p->fade ? float(ts) / p->fade : 1, psize = (p->size + pmax * size) * WOBBLE;
			if (2 * (p->size + pmax) * WOBBLE < depthfxblend ||
				(!depthfxtex.highprecision() && !depthfxtex.emulatehighprecision() &&
				 psize > depthfxscale - depthfxbias) ||
				isfoggedsphere(psize, p->o))
				continue;
			...
			if (depthfxscissor == 2 && !depthfxtex.addscissorbox(p->o, psize))
				continue;
			...
			int pos = numranges;
			loopi(numranges) if (depth < ranges[i])
			{
				pos = i;
				break;
			}
	...
	void renderpart(listparticle *p, const vec &o, const vec &d, int blend, int ts)
	{
		...
		binddepthfxparams(depthfxblend, inside ? blend / (2 * 255.0f) : 0,
						  2 * (p->size + pmax) * WOBBLE >= depthfxblend, p);
