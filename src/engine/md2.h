...
struct md2 : vertloader<md2>
{
	...
	struct md2meshgroup : vertmeshgroup
	{
		...
		bool load(const char *filename)
		{
			...
			if (header.magic != 844121161 || header.version != 8 || header.numframes <= 0 || header.numframes > 1000 ||
				header.numglcommands <= 0 || header.numglcommands > (1 << 20) || header.numvertices <= 0 ||
				header.numvertices > (1 << 20))
			{
				delete file;
				return false;
			}
	...
	struct md2part : part
	{
		...
		void getdefaultanim(animinfo &info, int anim, uint varseed, dynent *d)
		{
			...
			//                      DE DY I  F  B  L  R  H1 H2 H3 H4 H5 H6 H7 A1 A2 A3 A4 A5 A6 A7 PA J   SI SW ED  LA
			//                      T   WI  LO  GI  GS
			static const int animfr[] = {5, 2, 8, 9, 9, 9, 9,  8, 8, 8,	 8,	 8,	 8,	 8,	 6,	 6,
										 6, 6, 6, 6, 6, 7, 11, 8, 9, 10, 14, 12, 13, 15, 17, 16};
	...
	bool loaddefaultparts()
	{
		...
		if (tex == notexture)
			conoutf(CON_ERROR, "could not load model skin for %s", name1);
