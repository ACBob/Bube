...
bool BIH::triintersect(const mesh &m, int tidx, const vec &mo, const vec &mray, float maxdist, float &dist, int mode)
{
	...
	if (m.flags & MESH_ALPHA && (mode & RAY_ALPHAPOLY) == RAY_ALPHAPOLY &&
		(m.tex->alphamask || (lightmapping <= 1 && loadalphamask(m.tex))))
	{
...
inline bool BIH::traverse(const mesh &m, const vec &o, const vec &ray, const vec &invray, float maxdist, float &dist,
						  int mode, node *curnode, float tmin, float tmax)
{
		...
		else
		{
			if (farsplit < tmax)
			{
				if (!curnode->isleaf(faridx))
				{
					...
					{
						if (traverse(m, o, ray, invray, maxdist, dist, mode, curnode + curnode->childindex(nearidx),
									 tmin, min(tmax, nearsplit)))
							return true;
						curnode += curnode->childindex(faridx);
						tmin = max(tmin, farsplit);
						continue;
...
inline bool BIH::traverse(const vec &o, const vec &ray, float maxdist, float &dist, int mode)
{
	...
	{
		mesh &m = meshes[i];
		if (!(mode & RAY_SHADOW) && m.flags & MESH_NOCLIP)
			continue;
...
BIH::BIH(vector<mesh> &buildmeshes)
	: meshes(NULL), nummeshes(0), nodes(NULL), numnodes(0), tribbs(NULL), numtris(0), bbmin(1e16f, 1e16f, 1e16f),
	  bbmax(-1e16f, -1e16f, -1e16f), center(0, 0, 0), radius(0), entradius(0)
{
...
bool mmintersect(const extentity &e, const vec &o, const vec &ray, float maxdist, int mode, float &dist)
{
	...
	if (mode & RAY_SHADOW)
	{
		...
		return false;
	if (!m->bih && (lightmapping > 1 || !m->setBIH()))
		return false;
