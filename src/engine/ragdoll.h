...
struct ragdolldata
{
	...
	ragdolldata(ragdollskel *skel, float scale = 1)
		: skel(skel), millis(lastmillis), collidemillis(0), collisions(0), floating(0), lastmove(lastmillis),
		  unsticks(INT_MAX), radius(0), timestep(0), scale(scale), verts(new vert[skel->verts.length()]),
...
void ragdolldata::updatepos()
{
	loopv(skel->verts)
	{
		...
		if (v.weight)
		{
			...
			if (!collidevert(v.newpos, vec(v.newpos).sub(v.pos), skel->verts[i].radius))
				v.pos = v.newpos;
			else
			{
				vec dir = vec(v.newpos).sub(v.oldpos);
				if (dir.dot(collidewall) < 0)
					v.oldpos = vec(v.pos).sub(dir.reflect(collidewall));
				v.collided = true;
			}
		}
		v.newpos = vec(0, 0, 0);
...
void ragdolldata::move(dynent *pl, float ts)
{
	...
	float tsfric = timestep ? ts / timestep : 1,
		  airfric = ragdollairfric + min((ragdollbodyfricscale * collisions) / skel->verts.length(), 1.0f) *
										 (ragdollbodyfric - ragdollairfric);
	collisions = 0;
	loopv(skel->verts)
	{
		vert &v = verts[i];
