...
{
	char *buf = NULL;
	if (type & CON_TAG_MASK)
		for (int i = conlines.length() - 1; i >= max(conlines.length() - contags, 0); i--)
		{
			int prev = conlines.removing(i).type;
			if (!(prev & CON_TAG_MASK))
				break;
			if (type == prev)
			{
	...
	if (!buf)
		buf = conlines.length() >= maxcon ? conlines.remove().line : newstring("", CONSTRLEN - 1);
...
int rendercommand(int x, int y, int w)
{
	...
	draw_text(s, x, y, 0xFF, 0xFF, 0xFF, 0xFF,
			  (commandpos >= 0) ? (commandpos + 1 + (commandprompt ? strlen(commandprompt) : 1)) : strlen(s), w);
	return height;
}
...
{
	int conpad = fullconsole ? 0 : FONTH / 4, conoff = fullconsole ? FONTH : FONTH / 3,
		conheight =
			min(fullconsole ? ((h * fullconsize / 100) / FONTH) * FONTH : FONTH * consize, h - 2 * (conpad + conoff)),
	...
	int y = drawconlines(conskip, fullconsole ? 0 : confade, conwidth, conheight, conpad + conoff,
						 fullconsole ? fullconfilter : confilter);
	if (!fullconsole && (miniconsize && miniconwidth))
		drawconlines(miniconskip, miniconfade, (miniconwidth * (w - 2 * (conpad + conoff))) / 100,
					 min(FONTH * miniconsize, abovehud - y), conpad + conoff, miniconfilter, abovehud, -1);
...
void inputcommand(char *init, char *action = NULL, char *prompt = NULL,
				  char *flags = NULL) // turns input to the command line on or off
{
...
void pasteconsole()
{
	...
	size_t cblen = strlen(cb), commandlen = strlen(commandbuf),
		   decoded = decodeutf8((uchar *)&commandbuf[commandlen], sizeof(commandbuf) - 1 - commandlen,
								(const uchar *)cb, cblen);
...
struct hline
{
	...
	bool shouldsave()
	{
		return strcmp(commandbuf, buf) || (commandaction ? !action || strcmp(commandaction, action) : action != NULL) ||
			   (commandprompt ? !prompt || strcmp(commandprompt, prompt) : prompt != NULL) || commandflags != flags;
	}
...
struct filesval
{
	...
	filesval(int type, const char *dir, const char *ext)
		: type(type), dir(newstring(dir)), ext(ext && ext[0] ? newstring(ext) : NULL), millis(-1)
	{
	}
	~filesval()
	{
	...
	void update()
	{
		if ((type != FILES_DIR && type != FILES_VAR) || millis >= commandmillis)
			return;
		files.deletearrays();
		if (type == FILES_VAR)
		{
			string buf;
			buf[0] = '\0';
			if (ident *id = readident(dir))
				switch (id->type)
				{
					case ID_SVAR:
						copystring(buf, *id->storage.s);
						break;
					case ID_ALIAS:
						copystring(buf, id->getstr());
						break;
				}
			if (!buf[0])
				copystring(buf, ".");
...
void writecompletions(stream *f)
{
	...
	loopv(cmds)
	{
		...
		if (v->type == FILES_LIST)
		{
			...
			f->printf("%s %s %s %s\n", v->type == FILES_VAR ? "varcomplete" : "complete", escapeid(k),
					  escapestring(v->dir), escapestring(v->ext ? v->ext : "*"));
	}
}
