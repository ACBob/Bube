	...
	struct smdmeshgroup : skelmeshgroup
	{
		...
		void readtriangles(stream *f, char *buf, size_t bufsize)
		{
			...
			while (f->getline(buf, bufsize))
			{
				...
				loopi(3)
				{
					...
					if (sscanf(curbuf, " %d %f %f %f %f %f %f %f %f %d%n", &parent, &key.pos.x, &key.pos.y, &key.pos.z,
							   &key.norm.x, &key.norm.y, &key.norm.z, &key.tc.x, &key.tc.y, &numlinks, &len) < 9)
						goto endsection;
					curbuf += len;
					key.pos.y = -key.pos.y;
		...
		void readskeleton(stream *f, char *buf, size_t bufsize)
		{
			...
			while (f->getline(buf, bufsize))
			{
				...
				rot.x = -rot.x;
				rot.z = -rot.z;
				float cx = cosf(rot.x / 2), sx = sinf(rot.x / 2), cy = cosf(rot.y / 2), sy = sinf(rot.y / 2),
					  cz = cosf(rot.z / 2), sz = sinf(rot.z / 2);
				pos.y = -pos.y;
				dualquat dq(quat(sx * cy * cz - cx * sy * sz, cx * sy * cz + sx * cy * sz, cx * cy * sz - sx * sy * cz,
								 cx * cy * cz + sx * sy * sz),
							pos);
		...
		int readframes(stream *f, char *buf, size_t bufsize, vector<dualquat> &animbones)
		{
			...
			while (f->getline(buf, bufsize))
			{
				...
				float cx = cosf(rot.x / 2), sx = sinf(rot.x / 2), cy = cosf(rot.y / 2), sy = sinf(rot.y / 2),
					  cz = cosf(rot.z / 2), sz = sinf(rot.z / 2);
				pos.y = -pos.y;
				dualquat dq(quat(-(sx * cy * cz - cx * sy * sz), cx * sy * cz + sx * cy * sz,
				...
				if (skel->bones[bone].parent < 0)
					dst = dq;
				else
					dst.mul(skel->bones[skel->bones[bone].parent].base, dq);
		...
		skelanimspec *loadanim(const char *filename)
		{
			...
			memcpy(&framebones[skel->numframes * skel->numbones], animbones.getbuf(),
				   numframes * skel->numbones * sizeof(dualquat));
