...
#define checkragdoll                                                                                                   \
	...
	{                                                                                                                  \
		conoutf(CON_ERROR, "not loading a skeletal model");                                                            \
		return;                                                                                                        \
	}                                                                                                                  \
...
{
	return mapmodels.inrange(i) ? &mapmodels[i] : 0;
}
...
ICOMMAND(mapmodelfind, "s", (char *name), {
	int found = -1;
	loopv(mapmodels) if (strstr(mapmodels[i].name, name))
	{
		found = i;
		break;
	}
...
{
	if (!name || !name[0] || models.access(name))
		return;
...
void preloadusedmapmodels(bool msg, bool bih)
{
	...
	{
		extentity &e = *ents[i];
		if (e.type == ET_MAPMODEL && e.attr2 >= 0 && mapmodels.find(e.attr2) < 0)
			mapmodels.add(e.attr2);
	}
	...
	loopv(mapmodels)
	{
		...
		{
			if (msg)
				conoutf(CON_WARN, "could not find map model: %d", mmindex);
		}
		else if (mmi->name[0] && !loadmodel(NULL, mmindex, msg))
		{
			if (msg)
				conoutf(CON_WARN, "could not load model: %s", mmi->name);
		}
		else if (mmi->m)
		{
			if (bih)
...
model *loadmodel(const char *name, int i, bool msg)
{
	...
	{
		if (!name[0] || loadingmodel || lightmapping > 1)
			return NULL;
		if (msg)
		{
...
void clearmodel(char *name)
{
	...
	if (!m)
	{
		conoutf(CON_WARN, "model %s is not loaded", name);
		return;
	}
...
void endmodelbatches()
{
	...
	loopi(numbatches)
	{
		...
		if (b.flags & (MDL_SHADOW | MDL_DYNSHADOW))
		{
			...
			loopvj(b.batched)
			{
				...
				if (bm.flags & (MDL_SHADOW | MDL_DYNSHADOW))
					renderblob(bm.flags & MDL_DYNSHADOW ? BLOB_DYNAMIC : BLOB_STATIC,
							   bm.d && bm.d->ragdoll ? bm.d->ragdoll->center : bm.pos,
							   bm.d ? bm.d->radius : max(bbradius.x, bbradius.y), bm.transparent);
			}
			flushblobs();
		}
		bool rendered = false;
		...
		if (b.flags & MDL_GHOST)
		{
			...
			{
				batchedmodel &bm = b.batched[j];
				if ((bm.flags & (MDL_CULL_VFC | MDL_GHOST)) != MDL_GHOST || bm.query)
					continue;
				if (!rendered)
				{
					b.m->startrender();
		...
		{
			batchedmodel &bm = b.batched[j];
			if (bm.flags & (MDL_CULL_VFC | MDL_GHOST))
				continue;
			if (bm.query != query)
			{
				if (query)
...
void endmodelquery()
{
	...
	{
		modelbatch &b = *batches[i];
		if (b.batched.empty() || b.batched.last().query != modelquery)
			continue;
		querybatches++;
	}
	if (querybatches <= 1)
	...
	loopi(numbatches)
	{
		...
		do
		{
			...
			if (bm.attached >= 0)
				minattached = min(minattached, bm.attached);
			renderbatchedmodel(b.m, bm);
...
{
	if (fabs(camera1->o.x - center.x) < radius + 1 && fabs(camera1->o.y - center.y) < radius + 1 &&
...
{
	if (flags & MDL_CULL_DIST && center.dist(camera1->o) / radius > maxmodelradiusdistance)
		return MDL_CULL_DIST;
	if (flags & MDL_CULL_VFC)
	{
		if (reflecting || refracting)
		{
			...
			{
				if (center.z + radius <= reflectz)
					return MDL_CULL_VFC;
			}
			else
			{
				if (fogging && center.z + radius < reflectz - refractfog)
					return MDL_CULL_VFC;
				if (!shadow && center.z - radius >= reflectz)
					return MDL_CULL_VFC;
			}
			if (center.dist(camera1->o) - radius > reflectdist)
				return MDL_CULL_VFC;
		}
		if (isfoggedsphere(radius, center))
			return MDL_CULL_VFC;
		if (shadowmapping && !isshadowmapcaster(center, radius))
			return MDL_CULL_VFC;
	}
	if (shadowmapping)
	{
		if (d)
		{
			if (flags & MDL_CULL_OCCLUDED && d->occluded >= OCCLUDE_PARENT)
				return MDL_CULL_OCCLUDED;
			if (flags & MDL_CULL_QUERY && d->occluded + 1 >= OCCLUDE_BB && d->query && d->query->owner == d &&
				checkquery(d->query))
				return MDL_CULL_QUERY;
		}
		if (!addshadowmapcaster(center, radius, radius))
			return MDL_CULL_VFC;
	}
	...
	{
		if (!reflecting && !refracting && d->occluded < OCCLUDE_BB)
			d->occluded++;
		return MDL_CULL_QUERY;
	}
	return 0;
...
void rendermodel(entitylight *light, const char *mdl, int anim, const vec &o, float yaw, float pitch, int flags,
				 dynent *d, modelattach *a, int basetime, int basetime2, float trans)
{
	if (shadowmapping && !(flags & (MDL_SHADOW | MDL_DYNSHADOW)))
		return;
	...
	if (flags & (MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED | MDL_CULL_QUERY | MDL_SHADOW | MDL_DYNSHADOW))
	{
		...
		if (reflecting || refracting || shadowmapping)
			flags &= ~MDL_CULL_QUERY;
	}

	if (flags & MDL_NORENDER)
		anim |= ANIM_NORENDER;
	...
	if (!shadowmapping)
	{
		...
		if (d)
		{
			...
			if (flags & MDL_LIGHT && light->millis != lastmillis)
			{
				...
				else if (d->type < ENT_CAMERA)
					pos.z += 0.75f * (d->eyeheight + d->aboveeye);
	...
	if (numbatches >= 0)
	{
		...
		if (!shadow || reflecting || refracting > 0)
		{
			...
			if ((flags & MDL_CULL_VFC) && refracting < 0 && center.z - radius >= reflectz)
				b.flags |= MDL_CULL_VFC;
		}
		mb.flags |= b.flags;
		b.d = d;
		...
		if (flags & MDL_CULL_QUERY)
			d->query = b.query = newquery(d);
		return;
	}

	if (shadow && !reflecting && refracting <= 0)
	{
		renderblob(flags & MDL_DYNSHADOW ? BLOB_DYNAMIC : BLOB_STATIC, d && d->ragdoll ? center : o,
				   d ? d->radius : max(bbradius.x, bbradius.y), trans);
		flushblobs();
		if ((flags & MDL_CULL_VFC) && refracting < 0 && center.z - radius >= reflectz)
			return;
	}
	...
	if (flags & MDL_CULL_QUERY && d->query)
		endquery(d->query);
...
void renderclient(dynent *d, const char *mdlname, modelattach *attachments, int hold, int attack, int attackdelay,
				  int lastaction, int lastpain, float fade, bool ragdoll)
{
	int anim = hold ? hold : ANIM_IDLE | ANIM_LOOP;
	float yaw = testanims && d == player ? 0 : d->yaw + 90, pitch = testpitch && d == player ? testpitch : d->pitch;
	vec o = d->feetpos();
	int basetime = 0;
	if (animoverride)
	...
	else
	{
		...
		else if (game::allowmove(d) && (d->move || d->strafe))
		{
			...
			{
				if (d->move < 0)
					anim |= ((d->strafe > 0 ? ANIM_RIGHT : ANIM_LEFT) | ANIM_REVERSE | ANIM_LOOP) << ANIM_SECONDARY;
				else
					anim |= ((d->strafe > 0 ? ANIM_LEFT : ANIM_RIGHT) | ANIM_LOOP) << ANIM_SECONDARY;
			}
			else if (d->move < 0)
	...
	if (d->ragdoll && (!ragdoll || (anim & ANIM_INDEX) != ANIM_DYING))
		DELETEP(d->ragdoll);
	if (!((anim >> ANIM_SECONDARY) & ANIM_INDEX))
		anim |= (ANIM_IDLE | ANIM_LOOP) << ANIM_SECONDARY;
	int flags = MDL_LIGHT;
	if (d != player && !(anim & ANIM_RAGDOLL))
	...
	if (drawtex == DRAWTEX_MODELPREVIEW)
		flags &= ~(MDL_LIGHT | MDL_FULLBRIGHT | MDL_CULL_VFC | MDL_CULL_OCCLUDED | MDL_CULL_QUERY | MDL_CULL_DIST |
				   MDL_DYNSHADOW);
...
void setbbfrommodel(dynent *d, const char *mdl)
{
	...
	d->radius = d->collidetype == COLLIDE_OBB ? sqrtf(d->xradius * d->xradius + d->yradius * d->yradius)
											  : max(d->xradius, d->yradius);
