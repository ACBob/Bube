...
static inline bool raycubeintersect(const clipplanes &p, const cube &c, const vec &v, const vec &ray, const vec &invray,
									float &dist)
{
...
#define entintersect(mask, type, func)                                                                                 \
	...
	entintersect(RAY_POLY, mapmodels, if (!mmintersect(e, o, ray, radius, mode, f)) continue;);

	entintersect(RAY_ENTS, other, entselectionbox(e, eo, es);
				 if (!rayboxintersect(eo, es, o, ray, f, orient)) continue;);

	entintersect(RAY_ENTS, mapmodels, entselectionbox(e, eo, es);
				 if (!rayboxintersect(eo, es, o, ray, f, orient)) continue;);
	...
	for (;;)
	{
		...
		if (!isempty(c) && !(c.material & MAT_ALPHA))
		{
			...
			else
			{
				...
				if (p.owner != &c || p.version != cache->version)
				{
					p.owner = &c;
					p.version = cache->version;
					genclipplanes(c, lo, 1 << lshift, p, false);
				}
				INTERSECTPLANES(side = p.side[i], goto nextcube);
				INTERSECTBOX(side = (i << 1) + 1 - lsizemask[i], goto nextcube);
				if (exitdist >= 0)
...
bool ellipseboxcollide(physent *d, const vec &dir, const vec &o, const vec &center, float yaw, float xr, float yr,
					   float hi, float lo)
{
	...
	if (dist < 0)
	{
		...
		{
			if (dir.iszero() ||
				(dir.z > 0 && (d->type >= ENT_INANIMATE || below >= d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f)))
			{
				...
				return true;
				 ...
				 (dir.z < 0 && (d->type >= ENT_INANIMATE || above >= d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f)))
		{
			collidewall = vec(0, 0, 1);
			return true;
		}
...
bool ellipsecollide(physent *d, const vec &dir, const vec &o, const vec &center, float yaw, float xr, float yr,
					float hi, float lo)
{
	float below = (o.z + center.z - lo) - (d->o.z + d->aboveeye),
		  above = (d->o.z - d->eyeheight) - (o.z + center.z + hi);
...
template <class E, class O> static inline bool plcollide(physent *d, const vec &dir, physent *o)
{
	E entvol(d);
	O obvol(o);
	vec cp;
	...
	switch (d->collidetype)
	{
		...
		case COLLIDE_ELLIPSE_PRECISE:
			if (o->collidetype == COLLIDE_OBB)
				return ellipseboxcollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye,
										 o->eyeheight);
			else
				return ellipsecollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye,
									  o->eyeheight);
		case COLLIDE_OBB:
			if (o->collidetype == COLLIDE_OBB)
...
static inline bool mmcollide(physent *d, const vec &dir, const extentity &e, const vec &center, const vec &radius,
							 float yaw)
{
	E entvol(d);
...
bool mmcollide(physent *d, const vec &dir, octaentities &oc) // collide with a mapmodel
{
	...
	loopv(oc.mapmodels)
	{
		...
		if (d->o.reject(e.o, d->radius + rejectradius))
			continue;
		...
		switch (d->collidetype)
		{
			...
			case COLLIDE_ELLIPSE_PRECISE:
				if (m->ellipsecollide)
				{
					if (ellipsecollide(d, dir, e.o, center, yaw, radius.x, radius.y, radius.z, radius.z))
						return true;
				}
			...
			case COLLIDE_OBB:
				if (m->ellipsecollide)
				{
					if (mmcollide<mpr::EntOBB, mpr::ModelEllipse>(d, dir, e, center, radius, yaw))
						return true;
				}
				else if (mmcollide<mpr::EntOBB, mpr::ModelOBB>(d, dir, e, center, radius, yaw))
					return true;
				break;
			default:
				continue;
...
template <class E>
static bool fuzzycollidesolid(physent *d, const vec &dir, float cutoff, const cube &c, const ivec &co,
...
#define CHECKSIDE(side, distval, dotval, margin, normal)                                                               \
		...
		{                                                                                                              \
			...
			{                                                                                                          \
				if (dotval >= -cutoff * dir.magnitude())                                                               \
					continue;                                                                                          \
				if (d->type < ENT_CAMERA && dotval < 0 && dist < margin)                                               \
					continue;                                                                                          \
			}                                                                                                          \
			collidewall = normal;                                                                                      \
			bestdist = dist;                                                                                           \
	...
	CHECKSIDE(O_BOTTOM, co.z - (d->o.z + d->aboveeye), -dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f,
			  vec(0, 0, -1));
	CHECKSIDE(O_TOP, d->o.z - d->eyeheight - (co.z + size), dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
template <class E> static inline bool clampcollide(const clipplanes &p, const E &entvol, const plane &w, const vec &pw)
{
...
template <class E>
static bool fuzzycollideplanes(physent *d, const vec &dir, float cutoff, const cube &c, const ivec &co,
							   int size) // collide with deformed cube geometry
{
	...
	CHECKSIDE(O_BOTTOM, p.o.z - p.r.z - (d->o.z + d->aboveeye), -dir.z,
			  d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f, vec(0, 0, -1));
	CHECKSIDE(O_TOP, d->o.z - d->eyeheight - (p.o.z + p.r.z), dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
template <class E>
static bool cubecollidesolid(physent *d, const vec &dir, float cutoff, const cube &c, const ivec &co,
							 int size) // collide with solid cube geometry
{
	int crad = size / 2;
	if (fabs(d->o.x - co.x - crad) > d->radius + crad || fabs(d->o.y - co.y - crad) > d->radius + crad ||
	...
	CHECKSIDE(O_TOP, entvol.bottom() - (co.z + size), dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
template <class E>
static bool cubecollideplanes(physent *d, const vec &dir, float cutoff, const cube &c, const ivec &co,
							  int size) // collide with deformed cube geometry
{
	const clipplanes &p = getclipplanes(c, co, size);
	...
	CHECKSIDE(O_BOTTOM, p.o.z - p.r.z - entvol.top(), -dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 4.0f,
			  vec(0, 0, -1));
	CHECKSIDE(O_TOP, entvol.bottom() - (p.o.z + p.r.z), dir.z, d->zmargin - (d->eyeheight + d->aboveeye) / 3.0f,
			  vec(0, 0, 1));
...
static inline bool cubecollide(physent *d, const vec &dir, float cutoff, const cube &c, const ivec &co, int size,
							   bool solid)
{
	switch (d->collidetype)
	{
		...
		case COLLIDE_ELLIPSE_PRECISE:
			if (isentirelysolid(c) || solid)
				return cubecollidesolid<mpr::EntCapsule>(d, dir, cutoff, c, co, size);
			else
				return cubecollideplanes<mpr::EntCapsule>(d, dir, cutoff, c, co, size);
		default:
			return false;
	}
}

static inline bool octacollide(physent *d, const vec &dir, float cutoff, const ivec &bo, const ivec &bs, const cube *c,
							   const ivec &cor, int size) // collide with octants
{
	loopoctabox(cor, size, bo, bs)
	{
		if (c[i].ext && c[i].ext->ents)
			if (mmcollide(d, dir, *c[i].ext->ents))
				return true;
		ivec o(i, cor, size);
		if (c[i].children)
		{
...
static inline bool octacollide(physent *d, const vec &dir, float cutoff, const ivec &bo, const ivec &bs)
{
	...
	if (c->ext && c->ext->ents && mmcollide(d, dir, *c->ext->ents))
		return true;
	scale--;
	...
	switch (c->material & MATF_CLIP)
	{
		...
		case MAT_GAMECLIP:
			...
			break;
		case MAT_CLIP:
			if (isclipped(c->material & MATF_VOLUME) || d->type < ENT_CAMERA)
				solid = true;
			break;
	}
	if (!solid && isempty(*c))
...
void phystest()
{
	...
	printf("PHYS(pl): %s, air %d, floor: (%f, %f, %f), vel: (%f, %f, %f), g: (%f, %f, %f)\n", states[player->physstate],
		   player->timeinair, player->floor.x, player->floor.y, player->floor.z, player->vel.x, player->vel.y,
		   player->vel.z, player->falling.x, player->falling.y, player->falling.z);
	printf("PHYS(cam): %s, air %d, floor: (%f, %f, %f), vel: (%f, %f, %f), g: (%f, %f, %f)\n",
		   states[camera1->physstate], camera1->timeinair, camera1->floor.x, camera1->floor.y, camera1->floor.z,
		   camera1->vel.x, camera1->vel.y, camera1->vel.z, camera1->falling.x, camera1->falling.y, camera1->falling.z);
}
...
bool moveplayer(physent *pl, int moveres, bool local, int curtime)
{
	...
	if (pl->strafe && maxroll)
		pl->roll = clamp(pl->roll - pow(clamp(1.0f + pl->strafe * pl->roll / maxroll, 0.0f, 1.0f), 0.33f) * pl->strafe *
										curtime * straferoll,
						 -maxroll, maxroll);
	else
...
struct platforment
{
	...
	bool operator==(const physent *o) const
	{
		return d == o;
	}
};
...
template <class E, class O> static inline bool platformcollide(physent *d, const vec &dir, physent *o, float margin)
{
	E entvol(d);
	O obvol(o, margin);
	vec cp;
...
bool platformcollide(physent *d, physent *o, const vec &dir, float margin = 0)
{
	...
	{
		if (o->collidetype == COLLIDE_OBB)
			return platformcollide<mpr::EntOBB, mpr::EntOBB>(d, dir, o, margin);
		else
			return platformcollide<mpr::EntOBB, mpr::EntCylinder>(d, dir, o, margin);
	}
	else if (o->collidetype == COLLIDE_OBB)
		return ellipseboxcollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye,
								 o->eyeheight + margin);
	else
		return ellipsecollide(d, dir, o->o, vec(0, 0, 0), o->yaw, o->xradius, o->yradius, o->aboveeye,
...
bool moveplatform(physent *p, const vec &dir)
{
	...
	ents.setsize(0);
	for (int x = int(max(p->o.x - p->radius - PLATFORMBORDER, 0.0f)) >> dynentsize,
			 ex = int(min(p->o.x + p->radius + PLATFORMBORDER, worldsize - 1.0f)) >> dynentsize;
		 x <= ex; x++)
		for (int y = int(max(p->o.y - p->radius - PLATFORMBORDER, 0.0f)) >> dynentsize,
				 ey = int(min(p->o.y + p->radius + PLATFORMBORDER, worldsize - 1.0f)) >> dynentsize;
			 y <= ey; y++)
		{
			const vector<physent *> &dynents = checkdynentcache(x, y);
			loopv(dynents)
			{
				physent *d = dynents[i];
				if (p == d || d->o.z - d->eyeheight < p->o.z + p->aboveeye ||
					p->o.reject(d->o, p->radius + PLATFORMBORDER + d->radius) || ents.find(d) >= 0)
					continue;
				ents.add(d);
			}
		}
	...
	loopv(ents)
	{
		...
		if (platformcollide(p, d, vec(0, 0, 1), PLATFORMMARGIN))
			passengers.add(&ent);
		vec doldpos(d->o);
	...
	if (dir.z > 0)
	{
		loopv(passengers) // if any stacked passengers collide, stop the platform
		{
			...
			if (colliders.find(ent) >= 0)
				return false;
			for (int n = ent->stacks; n >= 0; n = collisions[n].next)
			{
				platforment *o = collisions[n].ent;
				if (passengers.find(o) < 0)
					passengers.add(o);
			}
		}
		loopv(passengers)
		...
		loopv(passengers) // move any stacked passengers who aren't colliding with non-passengers
		{
			platforment *ent = passengers[i];
			if (colliders.find(ent) >= 0)
				continue;
