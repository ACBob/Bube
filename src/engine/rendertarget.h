...
#define BLURTILEMASK (0xFFFFFFFFU >> (32 - BLURTILES))
	...
	rendertarget()
		: texw(0), texh(0), vieww(0), viewh(0), colorfmt(GL_FALSE), depthfmt(GL_FALSE), rendertex(0), renderfb(0),
		  renderdb(0), blurtex(0), blurfb(0), blurdb(0), blursize(0), blurysize(0), blursigma(0), initialized(false)
	{
	}
	...
	{
		static const GLenum depthfmts[] = {GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT16,
	...
	void cleanup(bool fullclean = false)
	{
		if (renderfb)
		{
			glDeleteFramebuffers_(1, &renderfb);
			renderfb = 0;
		}
		if (renderdb)
		{
			glDeleteRenderbuffers_(1, &renderdb);
			renderdb = 0;
		}
		if (rendertex)
		{
	...
	void cleanupblur()
	{
		if (blurfb)
		{
			glDeleteFramebuffers_(1, &blurfb);
			blurfb = 0;
		}
		if (blurtex)
		{
		...
		if (blurdb)
		{
			glDeleteRenderbuffers_(1, &blurdb);
			blurdb = 0;
		}
	...
	void setup(int w, int h)
	{
		...
		if (attach != GL_DEPTH_ATTACHMENT && depthtest())
		{
			if (!renderdb)
			{
				glGenRenderbuffers_(1, &renderdb);
				depthfmt = GL_FALSE;
			}
			if (!depthfmt)
				glBindRenderbuffer_(GL_RENDERBUFFER, renderdb);
			...
			loop(y, BLURTILES + 1)
			{
				...
				while (mask)
				{
					...
					do
					{
						tiles[yend] &= ~strip;
						yend++;
					} while ((tiles[yend] & strip) == strip);
					float tx = xstart * tsz, ty = y * tsz, tw = (x - xstart) * tsz, th = (yend - y) * tsz,
	...
	void blur(int wantsblursize, float wantsblursigma, int wantsblurysize, int x, int y, int w, int h, bool scissor)
	{
		...
		if (blursize != wantsblursize || blurysize != wantsblurysize || (wantsblursize && blursigma != wantsblursigma))
		{
			...
			if (wantsblurysize != wantsblursize)
				setupblurkernel(wantsblurysize, wantsblursigma, bluryweights, bluryoffsets);
		...
		loopi(2)
		{
			if (i && blurysize != blursize)
				setblurshader(i, texh, blurysize, bluryweights, bluryoffsets);
			else
				setblurshader(i, i ? texh : texw, blursize, blurweights, bluroffsets);

			if (!swaptexs() || rtsharefb)
				glFramebufferTexture2D_(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, i ? rendertex : blurtex,
										0);
			else
				glBindFramebuffer_(GL_FRAMEBUFFER, i ? renderfb : blurfb);
	...
	void render(int w, int h, int blursize = 0, float blursigma = 0, int blurysize = 0)
	{
		...
		vieww = w;
		viewh = h;
		if (w != texw || h != texh || (swaptexs() && !rtsharefb ? !blurfb : blurfb))
			cleanup();
		...
		if (succeeded)
		{
			...
			if (blursize)
				doblur(blursize, blursigma, blurysize ? blurysize : blursize);
		}
	...
	{
		if (!rtscissor || scissorx1 >= scissorx2 || scissory1 >= scissory2)
			return;
		int sx = int(0.5f * (scissorx1 + 1) * w), sy = int(0.5f * (scissory1 + 1) * h),
			sw = int(0.5f * (scissorx2 - scissorx1) * w), sh = int(0.5f * (scissory2 - scissory1) * h);
		if (flipdebug())
		{
			sy = h - sy;
			sh = -sh;
	...
	void debugblurtiles(int w, int h, bool lines = false)
	{
		...
		loop(y, BLURTILES + 1)
		{
			...
			while (mask)
			{
				...
				do
				{
					blurtiles[yend] &= ~strip;
					yend++;
				} while ((blurtiles[yend] & strip) == strip);
				float vx = xstart * vxsz, vy = y * vysz, vw = (x - xstart) * vxsz, vh = (yend - y) * vysz;
				if (flipdebug())
				{
					vy = h - vy;
					vh = -vh;
