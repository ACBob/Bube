...
struct menu : g3d_callback
{
	...
	menu()
		: name(NULL), header(NULL), contents(NULL), init(NULL), onclear(NULL), showtab(true), keeptab(false), menutab(1)
	{
	}
...
struct delayedupdate
{
	...
	delayedupdate() : type(ACTION), id(NULL)
	{
		val.s = NULL;
	}
	~delayedupdate()
	{
		if (type == STRING || type == ACTION)
			DELETEA(val.s);
	}
	...
	int getint() const
	{
		switch (type)
		{
			...
			case FLOAT:
				return int(val.f);
			case STRING:
				return int(strtol(val.s, NULL, 0));
			default:
				return 0;
		}
	}
	...
	void run()
	{
		if (type == ACTION)
		{
			...
			{
				case ID_VAR:
					setvarchecked(id, getint());
					break;
				case ID_FVAR:
					setfvarchecked(id, getfloat());
					break;
				case ID_SVAR:
					setsvarchecked(id, getstring());
					break;
				case ID_ALIAS:
					alias(id->name, getstring());
					break;
			}
	}
};
...
void guibutton(char *name, char *action, char *icon)
{
	...
	int ret = cgui->button(name, GUI_BUTTON_COLOR,
						   hideicon ? NULL : (icon[0] ? icon : (strstr(action, "showgui") ? "menu" : "action")));
	if (ret & G3D_UP)
	{
...
void guiimage(char *path, char *action, float *scale, int *overlaid, char *alt, char *title)
{
	...
	{
		if (alt[0])
			t = textureload(alt, 0, true, false);
		if (t == notexture)
			return;
	}
...
{
	if (cgui && text[0])
		cgui->textbox(text, *width ? *width : 12, *height ? *height : 1, *color ? *color : 0xFFFFFF);
}
...
void guitext(char *name, char *icon)
{
	bool hideicon = !strcmp(icon, "0");
	if (cgui)
		cgui->text(name, !hideicon && icon[0] ? GUI_BUTTON_COLOR : GUI_TEXT_COLOR,
				   hideicon ? NULL : (icon[0] ? icon : "info"));
}

void guititle(char *name)
{
	if (cgui)
		cgui->title(name, GUI_TITLE_COLOR);
}

void guitab(char *name)
{
	if (cgui)
		cgui->tab(name, GUI_TITLE_COLOR);
}
...
static int getval(char *var)
{
	...
	switch (id->type)
	{
		case ID_VAR:
			return *id->storage.i;
		case ID_FVAR:
			return int(*id->storage.f);
		case ID_SVAR:
			return parseint(*id->storage.s);
		case ID_ALIAS:
			return id->getint();
		default:
			return 0;
...
static float getfval(char *var)
{
	...
	switch (id->type)
	{
		...
		case ID_FVAR:
			return *id->storage.f;
		case ID_SVAR:
			return parsefloat(*id->storage.s);
		case ID_ALIAS:
			return id->getfloat();
		default:
			return 0;
...
static const char *getsval(char *var)
{
	...
	{
		case ID_VAR:
			return intstr(*id->storage.i);
		case ID_FVAR:
			return floatstr(*id->storage.f);
		case ID_SVAR:
			return *id->storage.s;
		case ID_ALIAS:
			return id->getstr();
...
void guislider(char *var, int *min, int *max, char *onchange)
{
	if (!cgui)
		return;
	int oldval = getval(var), val = oldval, vmin = *max > INT_MIN ? *min : getvarmin(var),
		vmax = *max > INT_MIN ? *max : getvarmax(var);
	cgui->slider(val, vmin, vmax, GUI_TITLE_COLOR);
	if (val != oldval)
		updateval(var, val, onchange);
}
...
void guilistslider(char *var, char *list, char *onchange)
{
	...
	if (offset != oldoffset)
		updateval(var, vals[offset], onchange);
}
...
void guieditor(char *name, int *maxlength, int *height, int *mode)
{
	if (!cgui)
		return;
	cgui->field(name, GUI_BUTTON_COLOR, *maxlength ? *maxlength : 12, *height, NULL,
...
void notifywelcome()
{
	if (guiserversmenu)
	{
		if (guistack.length() && guistack.last() == guiserversmenu)
			clearguis();
		guiserversmenu = NULL;
	}
}
	...
	void gui(g3d_gui &g, bool firstpass)
	{
		...
		if (g.button("yes", GUI_BUTTON_COLOR, "action") & G3D_UP)
		{
			...
			if (changetypes & CHANGE_GFX)
				updatelater.add().schedule("resetgl");
			if (changetypes & CHANGE_SOUND)
				updatelater.add().schedule("resetsound");
			clearlater = true;
		}
...
void menuprocess()
{
	...
	if (mainmenu && !isconnected(true) && guistack.empty())
		showgui("main");
...
void g3d_mainmenu()
{
	...
	{
		extern int usegui2d;
		if (!mainmenu && !usegui2d && camera1->o.dist(menupos) > menuautoclose)
			cleargui();
		else
			g3d_addgui(guistack.last(), menupos, GUI_2D | GUI_FOLLOW);
	}
}
