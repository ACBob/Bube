...
HVARFR(sunlight, 0, 0, 0xFFFFFF, {
	if (sunlight <= 255)
		sunlight |= (sunlight << 8) | (sunlight << 16);
...
void setupsunlight()
{
	...
	float dist =
		min(min(sunlightdir.x ? 1 / fabs(sunlightdir.x) : 1e16f, sunlightdir.y ? 1 / fabs(sunlightdir.y) : 1e16f),
			sunlightdir.z ? 1 / fabs(sunlightdir.z) : 1e16f);
...
static const surfaceinfo brightsurfaces[6] = {brightsurface, brightsurface, brightsurface,
...
#define CHECK_PROGRESS_LOCKED(exit, before, after)                                                                     \
	...
	loopv(lightmaps)
	{
		if (lightmaps[i].type == li.type && lightmaps[i].insert(si.x, si.y, li.colorbuf, si.w, si.h))
		{
			...
			if ((li.type & LM_TYPE) == LM_BUMPMAP0)
				ASSERT(lightmaps[i + 1].insert(si.x, si.y, (uchar *)li.raybuf, si.w, si.h));
			return;
		}
	}
...
static inline bool htcmp(const lightmapinfo &k, const layoutinfo &v)
{
	...
	loopi(kh)
	{
		if (memcmp(kcolor, vcolor, kbpp * kw))
			return false;
		kcolor += kbpp * kw;
		vcolor += kbpp * LM_PACKW;
	}
	if ((ktype & LM_TYPE) != LM_BUMPMAP0)
		return true;
	const bvec *kdir = k.raybuf,
			   *vdir = (const bvec *)lightmaps[v.lmid + 1 - LMID_RESERVED].data + (v.x + v.y * LM_PACKW);
	loopi(kh)
	{
		if (memcmp(kdir, vdir, kw * sizeof(bvec)))
			return false;
		kdir += kw;
		vdir += LM_PACKW;
	}
...
{
	if (max(LM_PACKW, LM_PACKH) > hwtexsize || !lightmaps.inrange(surface.lmid - LMID_RESERVED))
		return;
	...
	glTexSubImage2D(GL_TEXTURE_2D, 0, lm.offsetx + surface.x, lm.offsety + surface.y, surface.w, surface.h,
					lm.type & LM_ALPHA ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE,
					&lm.data[(surface.y * LM_PACKW + surface.x) * lm.bpp]);
	if ((lm.type & LM_TYPE) == LM_BUMPMAP0 && lightmaps.inrange(surface.lmid + 1 - LMID_RESERVED))
	{
		LightMap &lm2 = lightmaps[surface.lmid + 1 - LMID_RESERVED];
		glBindTexture(GL_TEXTURE_2D, lightmaptexs[lm2.tex].id);
		glTexSubImage2D(GL_TEXTURE_2D, 0, lm2.offsetx + surface.x, lm2.offsety + surface.y, surface.w, surface.h,
						GL_RGB, GL_UNSIGNED_BYTE, &lm2.data[(surface.y * LM_PACKW + surface.x) * 3]);
	}

	glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
}

static uint generatelumel(lightmapworker *w, const float tolerance, uint lightmask,
						  const vector<const extentity *> &lights, const vec &target, const vec &normal, vec &sample,
						  int x, int y)
{
	vec avgray(0, 0, 0);
	float r = 0, g = 0, b = 0;
	...
	loopv(lights)
	{
		...
		if (light.attached && light.attached->type == ET_SPOTLIGHT)
		{
			...
			float maxatten = sincos360[clamp(int(light.attached->attr1), 1, 89)].x,
		...
		if (lmshadows && mag)
		{
			float dist = shadowray(w->shadowraycache, light.o, ray, mag - tolerance,
								   RAY_SHADOW | (lmshadows > 1 ? RAY_ALPHAPOLY : 0));
	...
	if (sunlight)
	{
		...
		if (angle > 0 &&
			(!lmshadows ||
			 shadowray(w->shadowraycache, vec(sunlightdir).mul(tolerance).add(target), sunlightdir, 1e16f,
					   RAY_SHADOW | (lmshadows > 1 ? RAY_ALPHAPOLY : 0) |
						   (skytexturelight ? RAY_SKIPSKY | (useskytexture ? RAY_SKYTEX : 0) : 0)) > 1e15f))
		{
			float intensity;
			switch (w->type & LM_TYPE)
			{
	...
	switch (w->type & LM_TYPE)
	{
		case LM_BUMPMAP0:
			...
			avgray.normalize();
			w->raydata[y * w->w + x].add(
				vec(S.dot(avgray) / S.magnitude(), T.dot(avgray) / T.magnitude(), normal.dot(avgray)));
			break;
	}
...
{
	if (sample.x >= int(ambientcolor[0]) + 1 || sample.y >= int(ambientcolor[1]) + 1 ||
		sample.z >= int(ambientcolor[2]) + 1)
		return true;
#define NCHECK(n)                                                                                                      \
...
static void calcskylight(lightmapworker *w, const vec &o, const vec &normal, float tolerance, uchar *skylight,
						 int flags = RAY_ALPHAPOLY, extentity *t = NULL)
{
	...
	flags |= RAY_SHADOW;
	if (skytexturelight)
		flags |= RAY_SKIPSKY | (useskytexture ? RAY_SKYTEX : 0);
	int hit = 0;
	if (w)
		loopi(17)
		{
			if (normal.dot(rays[i]) >= 0 &&
				shadowray(w->shadowraycache, vec(rays[i]).mul(tolerance).add(o), rays[i], 1e16f, flags, t) > 1e15f)
				hit++;
		}
	else
		loopi(17)
		{
			if (normal.dot(rays[i]) >= 0 &&
				shadowray(vec(rays[i]).mul(tolerance).add(o), rays[i], 1e16f, flags, t) > 1e15f)
				hit++;
		}

	loopk(3) skylight[k] =
		uchar(ambientcolor[k] + (max(skylightcolor[k], ambientcolor[k]) - ambientcolor[k]) * hit / 17.0f);
}

static inline bool hasskylight()
{
	return skylightcolor[0] > ambientcolor[0] || skylightcolor[1] > ambientcolor[1] ||
		   skylightcolor[2] > ambientcolor[2];
}
...
static inline void generatealpha(lightmapworker *w, float tolerance, const vec &pos, uchar &alpha)
{
	...
	if (w->slot->layermask)
	{
		...
		float k = 8.0f / w->vslot->scale, s = (pos[sdim[dim]] * k - w->vslot->offset.y) / w->slot->layermaskscale,
		...
		{
			case 2:
				alpha = min(alpha, maskval);
				break;
			case 3:
				alpha = max(alpha, maskval);
				break;
			case 4:
				alpha = min(alpha, uchar(0xFF - maskval));
				break;
			case 5:
				alpha = max(alpha, uchar(0xFF - maskval));
				break;
			default:
				alpha = maskval;
				break;
...
static bool generatelightmap(lightmapworker *w, float lpu, const lerpvert *lv, int numv, vec origin1, const vec &xstep1,
							 const vec &ystep1, vec origin2, const vec &xstep2, const vec &ystep2, float side0,
							 float sidestep)
{
	static const float aacoords[8][2] = {
		{0.0f, 0.0f},  {-0.5f, -0.5f}, {0.0f, -0.5f}, {-0.5f, 0.0f},

		{0.3f, -0.6f}, {0.6f, 0.3f},   {-0.3f, 0.6f}, {-0.6f, -0.3f},
	};
	...
	if ((w->type & LM_TYPE) == LM_BUMPMAP0)
		memclear(w->raydata, (LM_MAXW + 4) * (LM_MAXH + 4));
...
#define EDGE_TOLERANCE(x, y)                                                                                           \
	(x < blurlms || x + 1 > w->w - blurlms || y < blurlms || y + 1 > w->h - blurlms ? edgetolerance : 1)
			float t = EDGE_TOLERANCE(x, y) * tolerance;
			vec u = x < sidex ? vec(xstep1).mul(x).add(vec(ystep1).mul(y)).add(origin1)
			...
			if (hasskylight())
			{
				if ((w->type & LM_TYPE) == LM_BUMPMAP0 || !adaptivesample || sample->x < skylightcolor[0] ||
					sample->y < skylightcolor[1] || sample->z < skylightcolor[2])
					calcskylight(w, u, normal, t, skylight, lmshadows > 1 ? RAY_ALPHAPOLY : 0);
				else
					loopk(3) skylight[k] = max(skylightcolor[k], ambientcolor[k]);
			}
			else
				loopk(3) skylight[k] = ambientcolor[k];
			if (w->type & LM_ALPHA)
				generatealpha(w, t, u, skylight[3]);
			sample += aasample;
		}
		sample += aasample;
	}
	if (adaptivesample > 1 && min(w->w, w->h) >= 2)
		lightmask = ~lightused;
	sample = w->colordata;
	initlerpbounds(-blurlms, -blurlms, lv, numv, start, end);
	...
	for (int y = 0; y < w->h; ++y, sidex += sidestep)
	{
		...
		{
			vec &center = *sample++;
			if (adaptivesample && x > 0 && x + 1 < w->w && y > 0 && y + 1 < w->h &&
				!lumelsample(center, aasample, stride))
...
#define AA_EDGE_TOLERANCE(x, y, i) EDGE_TOLERANCE(x + aacoords[i][0], y + aacoords[i][1])
				...
				loopi(aasample - 1) generatelumel(w, AA_EDGE_TOLERANCE(x, y, i + 1) * tolerance, lightmask, w->lights,
												  vec(u).add(offsets[i + 1]), n, *sample++, x, y);
				if (lmaa == 3)
				{
					loopi(4)
					{
						vec s;
						generatelumel(w, AA_EDGE_TOLERANCE(x, y, i + 4) * tolerance, lightmask, w->lights,
									  vec(u).add(offsets[i + 4]), n, s, x, y);
						center.add(s);
					}
					center.div(5);
				}
		...
		if (aasample > 1)
		{
			vec u = w->w < sidex ? vec(xstep1).mul(w->w).add(vec(ystep1).mul(y)).add(origin1)
								 : vec(xstep2).mul(w->w).add(vec(ystep2).mul(y)).add(origin2);
			const vec *offsets = w->w < sidex ? offsets1 : offsets2;
			vec n = vec(normal).normalize();
			generatelumel(w, edgetolerance * tolerance, lightmask, w->lights, vec(u).add(offsets[1]), n, sample[1],
						  w->w - 1, y);
			if (aasample > 2)
				generatelumel(w, edgetolerance * tolerance, lightmask, w->lights, vec(u).add(offsets[3]), n, sample[3],
							  w->w - 1, y);
		}
		sample += aasample;
	}
	...
	if (aasample > 1)
	{
		...
		for (int x = 0; x <= w->w; ++x, normal.add(nstep))
		{
			vec u = x < sidex ? vec(xstep1).mul(x).add(vec(ystep1).mul(w->h)).add(origin1)
							  : vec(xstep2).mul(x).add(vec(ystep2).mul(w->h)).add(origin2);
			const vec *offsets = x < sidex ? offsets1 : offsets2;
			vec n = vec(normal).normalize();
			generatelumel(w, edgetolerance * tolerance, lightmask, w->lights, vec(u).add(offsets[1]), n, sample[1],
						  min(x, w->w - 1), w->h - 1);
			if (aasample > 2)
				generatelumel(w, edgetolerance * tolerance, lightmask, w->lights, vec(u).add(offsets[2]), n, sample[2],
							  min(x, w->w - 1), w->h - 1);
			sample += aasample;
		}
	}
...
static int finishlightmap(lightmapworker *w)
{
	...
	float weight = 1.0f / (1.0f + 4.0f * lmaa), cweight = weight * (lmaa == 3 ? 5.0f : 1.0f);
	uchar *skylight = w->ambient;
	vec *ray = w->raydata;
	...
	loop(y, w->h)
	{
		loop(x, w->w)
		{
			...
			int r = int(center.x * cweight + l.x * weight), g = int(center.y * cweight + l.y * weight),
				b = int(center.z * cweight + l.z * weight), ar = skylight[0], ag = skylight[1], ab = skylight[2];
			dstcolor[0] = max(ar, r);
			dstcolor[1] = max(ag, g);
			dstcolor[2] = max(ab, b);
			loopk(3)
			...
			{
				if (ray->iszero())
					dstray[0] = bvec(128, 128, 255);
				else
				{
					// bias the normals towards the amount of ambient/skylight in the lumel
					// this is necessary to prevent the light values in shaders from dropping too far below the skylight
					// (to the ambient) if N.L is small
					ray->normalize();
	...
	if (int(maxcolor[0]) - int(mincolor[0]) <= lighterror && int(maxcolor[1]) - int(mincolor[1]) <= lighterror &&
		int(maxcolor[2]) - int(mincolor[2]) <= lighterror && mincolor[3] >= maxcolor[3])
	{
		uchar color[3];
		loopk(3) color[k] = (int(maxcolor[k]) + int(mincolor[k])) / 2;
		if (color[0] <= int(ambientcolor[0]) + lighterror && color[1] <= int(ambientcolor[1]) + lighterror &&
			color[2] <= int(ambientcolor[2]) + lighterror && (maxcolor[3] == 0 || mincolor[3] == 255))
			return mincolor[3] == 255 ? SURFACE_AMBIENT_TOP : SURFACE_AMBIENT_BOTTOM;
		if ((w->type & LM_TYPE) != LM_BUMPMAP0 ||
			(int(maxray.x) - int(minray.x) <= bumperror && int(maxray.y) - int(minray.z) <= bumperror &&
			 int(maxray.z) - int(minray.z) <= bumperror))

		{
			memcpy(w->colorbuf, color, 3);
			if (w->type & LM_ALPHA)
				w->colorbuf[3] = mincolor[3];
			if ((w->type & LM_TYPE) == LM_BUMPMAP0)
			{
	...
	if (blurlms && (w->w > 1 || w->h > 1))
	{
		...
		if ((w->type & LM_TYPE) == LM_BUMPMAP0)
			blurnormals(blurlms, w->w, w->h, w->raybuf, (const bvec *)w->raydata, blurlms);
		w->lastlightmap->w = (w->w -= 2 * blurlms);
		w->lastlightmap->h = (w->h -= 2 * blurlms);
	}
	if (mincolor[3] == 255)
		return SURFACE_LIGHTMAP_TOP;
	else if (maxcolor[3] == 0)
		return SURFACE_LIGHTMAP_BOTTOM;
	else
		return SURFACE_LIGHTMAP_BLEND;
}

static int previewlightmapalpha(lightmapworker *w, float lpu, const vec &origin1, const vec &xstep1, const vec &ystep1,
								const vec &origin2, const vec &xstep2, const vec &ystep2, float side0, float sidestep)
{
	...
	if (minalpha == 255)
		return SURFACE_AMBIENT_TOP;
	if (maxalpha == 0)
		return SURFACE_AMBIENT_BOTTOM;
	if (minalpha == maxalpha)
		w->w = w->h = 1;
	if ((w->type & LM_TYPE) == LM_BUMPMAP0)
		loopi(w->w * w->h) w->raybuf[i] = bvec(128, 128, 255);
	return SURFACE_LIGHTMAP_BLEND;
}
	...
	loopv(ents)
	{
		...
		switch (light.type)
		{
			case ET_LIGHT: {
				...
				if (radius > 0)
				{
					if (light.o.x + radius < cx || light.o.x - radius > cx + csize || light.o.y + radius < cy ||
...
static inline void addlight(lightmapworker *w, const extentity &light, int cx, int cy, int cz, int size, const vec *v,
							const vec *n, int numv)
{
	int radius = light.attr1;
	if (radius > 0)
	{
		if (light.o.x + radius < cx || light.o.x - radius > cx + size || light.o.y + radius < cy ||
			light.o.y - radius > cy + size || light.o.z + radius < cz || light.o.z - radius > cz + size)
			return;
	}
...
static bool findlights(lightmapworker *w, int cx, int cy, int cz, int size, const vec *v, const vec *n, int numv,
					   const Slot &slot, const VSlot &vslot)
{
	w->lights.setsize(0);
	const vector<extentity *> &ents = entities::getents();
	...
	if (size <= 1 << lightcachesize && (!lightlock || !usinglightcache))
	{
		if (lightlock)
		{
			SDL_LockMutex(lightlock);
			usinglightcache = true;
		}
		...
		loopv(lights)
		{
			...
			{
				case ET_LIGHT:
					addlight(w, light, cx, cy, cz, size, v, n, numv);
					break;
			}
		}
		if (lightlock)
	...
	if (vslot.layer && (setblendmaporigin(w->blendmapcache, ivec(cx, cy, cz), size) || slot.layermask))
		return true;
...
static int packlightmaps(lightmapworker *w = NULL)
{
	...
	for (; packidx < lightmaptasks[0].length(); packidx++, numpacked++)
	{
		...
		for (; l && l->c == t.c; l = l->next)
		{
			...
			{
				if (l->type & LM_ALPHA)
					surf.lmid[0] = layout.lmid;
				else
				{
					surf.lmid[1] = layout.lmid;
					verts += numverts;
				}
			}
			else
			{
				if (l->layers & LAYER_TOP)
					surf.lmid[0] = layout.lmid;
				if (l->layers & LAYER_BOTTOM)
					surf.lmid[1] = layout.lmid;
			}
		...
		if (t.worker->needspace)
			SDL_CondSignal(t.worker->spacecond);
	}
	return numpacked;
}
...
static lightmapinfo *alloclightmap(lightmapworker *w)
{
	int needspace1 = sizeof(lightmapinfo) + w->w * w->h * w->bpp,
		needspace2 = (w->type & LM_TYPE) == LM_BUMPMAP0 ? w->w * w->h * 3 : 0, needspace = needspace1 + needspace2,
		bufend = (w->bufstart + w->bufused) % LIGHTMAPBUFSIZE, availspace = LIGHTMAPBUFSIZE - w->bufused,
		availspace1 = min(availspace, LIGHTMAPBUFSIZE - bufend), availspace2 = min(availspace, w->bufstart);
	if (availspace < needspace ||
		(max(availspace1, availspace2) < needspace && (availspace1 < needspace1 || availspace2 < needspace2)))
	{
		if (tasklock)
			SDL_LockMutex(tasklock);
		while (!w->doneworking)
		{
			...
			if (availspace >= needspace && (max(availspace1, availspace2) >= needspace ||
											(availspace1 >= needspace1 && availspace2 >= needspace2)))
				break;
			if (packlightmaps(w))
				continue;
			if (!w->spacecond || !tasklock)
				break;
			w->needspace = true;
	...
	if (availspace1 >= needspace1)
	{
		...
		if ((w->type & LM_TYPE) != LM_BUMPMAP0)
			w->raybuf = NULL;
		else if (availspace1 >= needspace)
			w->raybuf = (bvec *)&w->buf[bufend + needspace1];
		else
		{
			w->raybuf = (bvec *)w->buf;
...
static int setupsurface(lightmapworker *w, plane planes[2], int numplanes, const vec *p, const vec *n, int numverts,
						vertinfo *litverts, bool preview = false)
{
	vec u, v, t;
	vec2 c[MAXFACEVERTS];
	...
	int lw = clamp(int(ceil((cmax.x - cmin.x + 1) * lpu)), LM_MINW, LM_MAXW),
		lh = clamp(int(ceil((cmax.y - cmin.y + 1) * lpu)), LM_MINH, LM_MAXH);
	w->w = lw;
	w->h = lh;
	if (!preview)
	{
	...
	vec xstep1 = vec(v).mul(cx.y).add(vec(u).mul(cx.x)).mul(cscale.x),
		...
		origin1 = vec(v).mul(comin.y).add(vec(u).mul(comin.x)).add(p[0]), xstep2 = xstep1, ystep2 = ystep1,
		origin2 = origin1;
	...
	if (numplanes >= 2)
	{
		...
		if (cx.y)
		{
			side0 = comin.y / -(cx.y * cscale.x);
			sidestep = cy.y * cscale.y / -(cx.y * cscale.x);
		}
		else if (cy.y)
		{
			side0 = ceil(comin.y / -(cy.y * cscale.y)) * (LM_MAXW + 1);
			sidestep = -(LM_MAXW + 1);
			if (cy.y < 0)
			{
				side0 = (LM_MAXW + 1) - side0;
				sidestep = -sidestep;
			}
		}
		else
			side0 = comin.y <= 0 ? LM_MAXW + 1 : -1;
	}
	...
	else
	{
		...
		if (!generatelightmap(w, lpu, lv, numv, origin1, xstep1, ystep1, origin2, xstep2, ystep2, side0, sidestep))
			return NO_SURFACE;
		surftype = finishlightmap(w);
	}
	if (surftype < SURFACE_LIGHTMAP)
		return surftype;

	vec2 texscale(float(USHRT_MAX + 1) / LM_PACKW, float(USHRT_MAX + 1) / LM_PACKH);
	if (lw != w->w)
		texscale.x *= float(w->w - 1) / (lw - 1);
	if (lh != w->h)
		texscale.y *= float(w->h - 1) / (lh - 1);
	loopk(numverts)
	{
...
static void removelmalpha(lightmapworker *w)
{
	if (!(w->type & LM_ALPHA))
		return;
	for (uchar *dst = w->colorbuf, *src = w->colorbuf, *end = &src[w->w * w->h * 4]; src < end; dst += 3, src += 4)
	{
		dst[0] = src[0];
		dst[1] = src[1];
		dst[2] = src[2];
...
static lightmapinfo *setupsurfaces(lightmapworker *w, lightmaptask &task)
{
	...
	loopi(6)
	{
		...
		if (numverts < 4 || !convex)
			loopk(numverts) findnormal(pos[k], planes[0], n[k]);
		else
		{
...
static bool processtasks(bool finish = false)
{
	...
	while (finish || lightmaptasks[1].length())
	{
		...
		else if (lightmapping > 1)
		{
			...
			CHECK_PROGRESS_LOCKED(
				{
					SDL_UnlockMutex(tasklock);
					return false;
				},
				SDL_UnlockMutex(tasklock), SDL_LockMutex(tasklock));
		}
		else
		{
	...
	loopi(8)
	{
		...
		else if (!isempty(c[i]))
		{
			if (c[i].ext)
			{
				...
				{
					surfaceinfo &surf = c[i].ext->surfaces[j];
					if (surf.lmid[0] >= LMID_RESERVED || surf.lmid[1] >= LMID_RESERVED)
						goto nextcube;
					surf.clear();
				}
			}
			...
			if (usefacemask)
			{
				...
				if (lightmaptasks[1].length() >= MAXLIGHTMAPTASKS && !processtasks())
					return;
			}
		}
	nextcube:;
...
static bool previewblends(lightmapworker *w, cube &c, const ivec &co, int size)
{
	...
	int usefacemask = 0;
	loopi(6) if (c.texture[i] != DEFAULT_SKY && lookupvslot(c.texture[i], false).layer) usefacemask |=
		visibletris(c, i, co, size) << (4 * i);
	if (!usefacemask)
		return false;
	...
	loopi(6)
	{
		...
		VSlot &vslot = lookupvslot(c.texture[i], false), &layer = lookupvslot(vslot.layer, false);
		Shader *shader = vslot.slot->shader;
		...
		ivec v[4];
		genfaceverts(c, i, v);
		int convex = flataxisface(c, i) ? 0 : faceconvexity(v), order = usefaces & 4 || convex < 0 ? 1 : 0;
		ivec vo = ivec(co).mask(0xFFF).shl(3);
		curlitverts[numverts++].set(v[order].mul(size).add(vo));
		if (usefaces & 1)
			curlitverts[numverts++].set(v[order + 1].mul(size).add(vo));
		...
		switch (surftype)
		{
			...
			case SURFACE_LIGHTMAP_BLEND: {
				if (surf.numverts == (LAYER_BLEND | numverts) && surf.lmid[0] == surf.lmid[1] &&
				...
				numlitverts += numverts;
				layoutinfo layout;
				if (packlightmap(*w->lastlightmap, layout))
					updatelightmap(layout);
				surf.lmid[0] = surf.lmid[1] = layout.lmid;
				ushort offsetx = layout.x * ((USHRT_MAX + 1) / LM_PACKW),
					   offsety = layout.y * ((USHRT_MAX + 1) / LM_PACKH);
				loopk(numverts)
				{
...
{
	loadlayermasks();
	if (lightmapworkers.empty())
		lightmapworkers.add(new lightmapworker);
...
void cleanuplightmaps()
{
	...
	if (progresstex)
	{
		glDeleteTextures(1, &progresstex);
		progresstex = 0;
	}
}
...
bool setlightmapquality(int quality)
{
	switch (quality)
	{
		case 1:
			...
			break;
		case 0:
			lmshadows = lmshadows_;
			lmaa = lmaa_;
			lerptjoints = lerptjoints_;
			break;
		case -1:
			lmshadows = 1;
			lmaa = 0;
...
static void cleanupthreads()
{
	...
	if (lightmapping > 1)
	{
		...
		{
			lightmapworker *w = lightmapworkers[i];
			if (w->needspace && w->spacecond)
				SDL_CondSignal(w->spacecond);
		}
		SDL_UnlockMutex(tasklock);
		loopv(lightmapworkers)
		{
...
void calclight(int *quality)
{
	...
	if (numthreads > 1)
		preloadusedmapmodels(false, true);
	...
	if (calclight_canceled)
		...
		conoutf("generated %d lightmaps using %d%% of %d textures (%.1f seconds)", total,
				lightmaps.length() ? lumels * 100 / (lightmaps.length() * LM_PACKW * LM_PACKH) : 0, lightmaps.length(),
				(end - start) / 1000.0f);
}
...
void patchlight(int *quality)
{
	...
	loopv(lightmaps)
	{
		if ((lightmaps[i].type & LM_TYPE) != LM_BUMPMAP1)
			progresslightmap = i;
		total -= lightmaps[i].lightmaps;
		lumels -= lightmaps[i].lumels;
	}
	...
	SDL_TimerID timer = SDL_AddTimer(250, calclighttimer, NULL);
	if (patchnormals)
		renderprogress(0, "computing normals...");
	Uint32 start = SDL_GetTicks();
	if (patchnormals)
		calcnormals(lerptjoints > 0);
	show_calclight_progress();
	...
	if (calclight_canceled)
		...
		conoutf("patched %d lightmaps using %d%% of %d textures (%.1f seconds)", total,
				lightmaps.length() ? lumels * 100 / (lightmaps.length() * LM_PACKW * LM_PACKH) : 0, lightmaps.length(),
				(end - start) / 1000.0f);
}
...
VARF(
	fullbright, 0, 0, 1, if (lightmaptexs.length()) {
		initlights();
		lightents();
	});
...
static void rotatenormals(cube *c)
{
	loopi(8)
	{
		...
		loopj(6) if (lightmaps.inrange(ch.ext->surfaces[j].lmid[0] + 1 - LMID_RESERVED))
		{
			...
			if ((lmlv.type & LM_TYPE) != LM_BUMPMAP1)
				continue;
...
void fixrotatedlightmaps(cube &c, const ivec &co, int size)
{
	...
	loopi(6)
	{
		...
		surfaceinfo &surf = c.ext->surfaces[i];
		int numverts = surf.numverts & MAXFACEVERTS;
		if (numverts != 4 || (surf.lmid[0] < LMID_RESERVED && surf.lmid[1] < LMID_RESERVED))
			continue;
...
static void findunlit(int i)
{
	...
	{
		if (i + 1 >= lightmaps.length() || (lightmaps[i + 1].type & LM_TYPE) != LM_BUMPMAP1)
			return;
	}
	else if ((lm.type & LM_TYPE) != LM_DIFFUSE)
		return;
	uchar *data = lm.data;
	loop(y, 2) loop(x, LM_PACKW)
	{
		if (!data[0] && !data[1] && !data[2])
		{
			...
			if ((lm.type & LM_TYPE) == LM_BUMPMAP0)
				((bvec *)lightmaps[i + 1].data)[y * LM_PACKW + x] = bvec(128, 128, 255);
			lm.unlitx = x;
			lm.unlity = y;
			return;
		}
...
VARF(roundlightmaptex, 0, 4, 16, {
	cleanuplightmaps();
	initlights();
	allchanged();
});
VARF(batchlightmaps, 0, 4, 256, {
	cleanuplightmaps();
	initlights();
	allchanged();
});
...
void genlightmaptexs(int flagmask, int flagval)
{
	...
	while (total)
	{
		...
		{
			LightMap &lm = lightmaps[i];
			if (lm.tex >= 0 || (lm.type & flagmask) != flagval || (lm.type & LM_TYPE) != type)
				continue;
			...
			if (data)
				copylightmap(lm, &data[bpp * (offsety * tex.w + offsetx)], bpp * tex.w);
...
static inline void fastskylight(const vec &o, float tolerance, uchar *skylight, int flags = RAY_ALPHAPOLY,
								extentity *t = NULL, bool fast = false)
{
	flags |= RAY_SHADOW;
	if (skytexturelight)
		flags |= RAY_SKIPSKY | (useskytexture ? RAY_SKYTEX : 0);
	if (fast)
	{
		...
		if (shadowray(vec(ray).mul(tolerance).add(o), ray, 1e16f, flags, t) > 1e15f)
			...
			memcpy(skylight, ambientcolor.v, 3);
	}
	else
	{
		...
		loopk(3) skylight[k] =
			uchar(ambientcolor[k] + (max(skylightcolor[k], ambientcolor[k]) - ambientcolor[k]) * hit / 5.0f);
	}
}
...
void lightreaching(const vec &target, vec &color, vec &dir, bool fast, extentity *t, float ambient)
{
	...
	loopv(lights)
	{
		...
		if (e.attached && e.attached->type == ET_SPOTLIGHT)
		{
			...
			float maxatten = sincos360[clamp(int(e.attached->attr1), 1, 89)].x,
				  spotatten = (ray.dot(spot) - maxatten) / (1 - maxatten);
			if (spotatten <= 0)
				continue;
			intensity *= spotatten;
	...
	if (sunlight &&
		shadowray(target, sunlightdir, 1e16f,
				  RAY_SHADOW | RAY_POLY | (skytexturelight ? RAY_SKIPSKY | (useskytexture ? RAY_SKYTEX : 0) : 0),
				  t) > 1e15f)
	{
	...
	if (hasskylight())
	{
		uchar skylight[3];
		if (t)
			calcskylight(NULL, target, vec(0, 0, 0), 0.5f, skylight, RAY_POLY, t);
		else
			fastskylight(target, 0.5f, skylight, RAY_POLY, t, fast);
...
{
	if (sunlight && sunlightdir.dot(dir) > 0 &&
		shadowray(target, sunlightdir, 1e16f,
				  RAY_SHADOW | RAY_POLY | (skytexturelight ? RAY_SKIPSKY | (useskytexture ? RAY_SKYTEX : 0) : 0)) >
			1e15f)
		return &sunlightent;
