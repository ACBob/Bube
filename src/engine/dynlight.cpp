...
void adddynlight(const vec &o, float radius, const vec &color, int fade, int peak, int flags, float initradius,
				 const vec &initcolor, physent *owner)
{
	if (!maxdynlights)
		return;
...
int finddynlights()
{
	...
	loopvj(dynlights)
	{
		...
		{
			if (d.o.z + d.curradius < reflectz)
				continue;
		}
		else if (refracting < 0 && d.o.z - d.curradius > reflectz)
			continue;
		e.o = d.o;
		...
		if (closedynlights.length() >= DYNLIGHTMASK)
			break;
	}
...
void calcdynlightmask(vtxarray *va)
{
	...
	loopv(closedynlights)
	{
		...
		if (d.o.dist_to_bb(va->geommin, va->geommax) >= d.curradius)
			continue;

		mask |= (i + 1) << offset;
		offset += DYNLIGHTBITS;
		if (offset >= maxdynlights * DYNLIGHTBITS)
			break;
	}
...
{
	if (closedynlights.empty() || !va->dynlightmask)
		return 0;
