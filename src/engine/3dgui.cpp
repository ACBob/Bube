...
// implementationwise, it is *stateless*: it keeps no internal gui structure, hit tests are instant, usage &
...
static inline bool throttlepreview(bool loaded)
{
	if (loaded)
		return true;
	if (totalmillis - lastpreview < guipreviewtime)
		return false;
...
struct gui : g3d_gui
{
	...
	void autotab()
	{
		...
		{
			if (layoutpass && !tpos)
				tcurrent = NULL; // disable tabs because you didn't start with one
			if (shouldautotab && !curdepth && (layoutpass ? 0 : cury) + ysize > guiautotab * FONTH)
				tab(NULL, tcolor);
		}
	}
	...
	void tab(const char *name, int color)
	{
		...
		{
			cury = -ysize;
			int h = FONTH - 2 * INSERT, x1 = curx + tx,
				x2 = x1 + w + ((skinx[3] - skinx[2]) + (skinx[5] - skinx[4])) * SKIN_SCALE,
				y1 = cury - ((skiny[6] - skiny[1]) - (skiny[3] - skiny[2])) * SKIN_SCALE - h, y2 = cury;
			...
			drawskin(x1 - skinx[visible() ? 2 : 6] * SKIN_SCALE, y1 - skiny[1] * SKIN_SCALE, w, h, visible() ? 10 : 19,
					 9, gui2d ? 1 : 2, light, alpha);
			text_(name, x1 + (skinx[3] - skinx[2]) * SKIN_SCALE - (w ? INSERT : INSERT / 2),
				  y1 + (skiny[2] - skiny[1]) * SKIN_SCALE - INSERT, tcolor, visible());
		}
	...
	void pushlist()
	{
		...
		{
			curlist = nextlist++;
			if (curlist >= lists.length()) // should never get here unless script code doesn't use same amount of lists
										   // in layout and render passes
			{
				list &l = lists.add();
				l.parent = curlist;
				l.springs = 0;
	...
	void poplist()
	{
		...
		if (layoutpass)
		{
			...
			l.h = ysize;
			if (l.column >= 0)
				columns[l.column] = max(columns[l.column], ishorizontal() ? ysize : xsize);
		}
		curlist = l.parent;
		curdepth--;
		if (lists.inrange(curlist))
	...
	void column(int col)
	{
		if (curlist < 0 || !layoutpass || col < 0 || col >= MAXCOLUMNS)
			return;
		list &l = lists[curlist];
		l.column = col;
	}
	...
	{
		if (shouldmergehits)
			return windowhit == this && (ishorizontal() ? hitx >= x && hitx < x + w : hity >= y && hity < y + h);
		if (ishorizontal())
			h = ysize;
		else
			w = xsize;
	...
	int image(Texture *t, float scale, const char *overlaid)
	{
		...
		if (visible())
			icon_(t, overlaid != NULL, curx, cury, size, ishit(size + SHADOW, size + SHADOW), overlaid);
	...
	int texture(VSlot &vslot, float scale, bool overlaid)
	{
		...
		int size = (int)(scale * 2 * FONTH) - SHADOW;
		if (visible())
			previewslot(vslot, overlaid, curx, cury, size, ishit(size + SHADOW, size + SHADOW));
		return layout(size + SHADOW, size + SHADOW);
	}
	...
	int playerpreview(int model, int team, int weap, float sizescale, const char *overlaid)
	{
		...
		if (model >= 0 && visible())
		{
			...
			int x1 = int(floor(screenw * (xi * scale.x + origin.x))),
				y1 = int(floor(screenh * (1 - ((yi + ys) * scale.y + origin.y)))),
				x2 = int(ceil(screenw * ((xi + xs) * scale.x + origin.x))),
				y2 = int(ceil(screenh * (1 - (yi * scale.y + origin.y))));
			glDisable(GL_BLEND);
			...
			if (overlaid)
			{
				...
				if (overlaid[0])
					text_(overlaid, xi + xs / 12, yi + ys - ys / 12 - FONTH, hit ? 0xFF0000 : 0xFFFFFF, hit, hit);
				if (!overlaytex)
	...
	int prefabpreview(const char *prefab, const vec &color, float sizescale, const char *overlaid, bool throttle)
	{
		...
		if (prefab[0] && visible() && (!throttle || throttlepreview(prefabloaded(prefab))))
		{
			...
			if (overlaid)
			{
				...
				if (overlaid[0])
					text_(overlaid, xi + FONTH / 2, yi + FONTH / 2, hit ? 0xFF0000 : 0xFFFFFF, hit, hit);
				if (!overlaytex)
					overlaytex = textureload("data/guioverlay.png", 3);
				gle::color(light);
				glBindTexture(GL_TEXTURE_2D, overlaytex->id);
	...
	void slider(int &val, int vmin, int vmax, int color, const char *label)
	{
		...
		if (visible())
		{
			...
			if (ishorizontal())
			{
				...
				px = x + (FONTH - w) / 2;
				py = y + (ysize - FONTH) -
					 ((ysize - FONTH) * (offset - vmin)) / ((vmax == vmin) ? 1 : (vmax - vmin)); // vmin at bottom
			}
			else
			{
				hit = ishit(xsize, FONTH, x, y);
				px = x + FONTH / 2 - w / 2 +
					 ((xsize - w) * (offset - vmin)) / ((vmax == vmin) ? 1 : (vmax - vmin)); // vmin at left
				py = y;
			}
			...
			if (hit && actionon)
			{
				...
				if (ishorizontal())
					vnew = int((vnew * (y + ysize - FONTH / 2 - hity)) / (ysize - FONTH));
				else
					vnew = int((vnew * (hitx - x - FONTH / 2)) / (xsize - w));
				vnew += vmin;
	...
	char *field_(const char *name, int color, int length, int height, const char *initval, int initmode,
				 int fieldtype = FIELDEDIT)
	{
		...
		if (layoutpass)
		{
			...
			{
				if (strcmp(e->lines[0].text, initval))
					e->clear(initval);
			}
			e->linewrap = (length < 0);
			...
			{
				int temp;
				text_bounds(e->lines[0].text, temp, e->pixelheight,
							e->pixelwidth); // only single line editors can have variable height
			}
			else
		...
		if (visible() && !layoutpass)
		{
			...
			if (hit && editing && (mousebuttons & G3D_PRESSED) != 0 && fieldtype == FIELDEDIT)
				e->hit(int(floor(hitx - (curx + FONTW / 2))), int(floor(hity - cury)),
					   (mousebuttons & G3D_DRAGGED) != 0); // mouse request position
			if (editing && ((fieldmode == FIELDCOMMIT) || (fieldmode == FIELDABORT) ||
							!hit)) // commit field if user pressed enter or wandered out of focus
			{
				if (fieldmode == FIELDCOMMIT || (fieldmode != FIELDABORT && !hit))
					result = e->currentline().text;
			...
			if (editing)
				...
				gle::colorub(color >> 16, (color >> 8) & 0xFF, color & 0xFF);
	...
	void rect_(float x, float y, float w, float h, int usetc)
	{
		...
		gle::attrib(tc[usetc]);
		gle::attribf(x + w, y);
		gle::attrib(tc[usetc + 1]);
		gle::attribf(x, y + h);
		gle::attrib(tc[usetc + 3]);
		gle::attribf(x + w, y + h);
		gle::attrib(tc[usetc + 2]);
		xtraverts += gle::end();
	}
	...
	{
		if (shadow)
			draw_text(text, x + SHADOW, y + SHADOW, 0x00, 0x00, 0x00, -0xC0);
	...
	void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit, const char *title = NULL)
	{
		...
		if (overlaid)
		{
			...
			rect_(x, y, xs, ys, 0);
			if (title)
				text_(title, x + xs / 12, y + ys - ys / 12 - FONTH, hit ? 0xFF0000 : 0xFFFFFF, hit && actionon, hit);
		}
	}
	...
	void previewslot(VSlot &vslot, bool overlaid, int x, int y, int size, bool hit)
	{
		...
		else if (slot.thumbnail && slot.thumbnail != notexture)
			t = slot.thumbnail;
		else
			return;
		...
		loopk(4)
		{
			tc[k].x = tc[k].x / xt - xoff / t->xs;
			tc[k].y = tc[k].y / yt - yoff / t->ys;
		}
		if (slot.loaded)
			gle::color(vec(color).mul(vslot.colorscale));
		else
			gle::color(color);
		glBindTexture(GL_TEXTURE_2D, t->id);
		gle::begin(GL_TRIANGLE_STRIP);
		...
		if (glowtex)
		{
			...
			if (hit || overlaid)
				gle::color(vec(vslot.glowcolor).mul(color));
			else
		...
		if (layertex)
		{
			...
			gle::begin(GL_TRIANGLE_STRIP);
			gle::attribf(x + xs / 2, y + ys / 2);
			gle::attrib(tc[0]);
			gle::attribf(x + xs, y + ys / 2);
			gle::attrib(tc[1]);
			gle::attribf(x + xs / 2, y + ys);
			gle::attrib(tc[3]);
			gle::attribf(x + xs, y + ys);
			gle::attrib(tc[2]);
			gle::end();
	...
	void textbox(const char *text, int width, int height, int color)
	{
		...
		if (h > height)
			height = h;
		if (visible())
			draw_text(text, curx, cury, color >> 16, (color >> 8) & 0xFF, color & 0xFF, 0xFF, -1, width);
		layout(width, height);
	}
	...
	int button_(const char *text, int color, const char *icon, bool clickable, bool center)
	{
		...
		if (visible())
		{
			...
			if (icon && text)
				x += padding;
			if (text)
				text_(text, x, cury, color, center || (hit && clickable && actionon), hit && clickable);
		}
	...
	static void drawskin(int x, int y, int gapw, int gaph, int start, int n, int passes = 1,
						 const vec &light = vec(1, 1, 1),
						 float alpha = 0.80f) // int vleft, int vright, int vtop, int vbottom, int start, int n)
	{
		if (!skintex)
		...
		loopj(passes)
		{
			...
			gle::color(j ? light : vec(1, 1, 1),
					   passes <= 1 || j ? alpha : alpha / 2); // ghost when its behind something in depth
			loopi(n)
			{
				...
				float tleft = left * wscale, tright = right * wscale, ttop = top * hscale, tbottom = bottom * hscale;
				if (p.flags & 0x1)
				{
					gapx1 = left;
					gapx2 = right;
				...
				while (ystep > 0)
				{
					...
					while (xstep > 0)
					{
						...
						gle::attribf(xo, yo);
						gle::attribf(tleft, ttop);
						gle::attribf(xo + xstep, yo);
						gle::attribf(tright, ttop);
						gle::attribf(xo + xstep, yo + ystep);
						gle::attribf(tright, tbottom);
						gle::attribf(xo, yo + ystep);
						gle::attribf(tleft, tbottom);
						if (!(p.flags & 0x01))
							break;
						xo += xstep;
					}
	...
	void adjustscale()
	{
		int w = xsize + (skinx[2] - skinx[1]) * SKIN_SCALE + (skinx[10] - skinx[9]) * SKIN_SCALE,
			h = ysize + (skiny[9] - skiny[7]) * SKIN_SCALE;
		if (tcurrent)
			h += ((skiny[5] - skiny[1]) - (skiny[3] - skiny[2])) * SKIN_SCALE + FONTH - 2 * INSERT;
		else
			h += (skiny[6] - skiny[3]) * SKIN_SCALE;

		float aspect = forceaspect ? 1.0f / forceaspect : float(screenh) / float(screenw), fit = 1.0f;
		if (w * aspect * basescale > 1.0f)
			fit = 1.0f / (w * aspect * basescale);
		if (h * basescale * fit > maxscale)
			fit *= maxscale / (h * basescale * fit);
		origin = vec(0.5f - ((w - xsize) / 2 - (skinx[2] - skinx[1]) * SKIN_SCALE) * aspect * scale.x * fit,
					 0.5f + (0.5f * h - (skiny[9] - skiny[7]) * SKIN_SCALE) * scale.y * fit, 0);
	...
	void start(int starttime, float initscale, int *tab, bool allowinput)
	{
		...
		if (layoutpass)
			scale.x = scale.y = scale.z =
				guifadein ? basescale * min((totalmillis - starttime) / 300.0f, 1.0f) : basescale;
		...
		else
		{
			...
			drawskin(curx - skinx[2] * SKIN_SCALE, cury - skiny[6] * SKIN_SCALE, xsize, ysize, 0, 9, gui2d ? 1 : 2,
					 light, alpha);
			if (!tcurrent)
				drawskin(curx - skinx[5] * SKIN_SCALE, cury - skiny[6] * SKIN_SCALE, xsize, 0, 9, 1, gui2d ? 1 : 2,
						 light, alpha);
		}
	}
	...
	void adjustcolumns()
	{
		...
		for (int i = firstlist; i < lists.length(); i++)
		{
			...
			{
				if (depth & 1)
					adjusthorizontalcolumn(l.column, i);
				else
					adjustverticalcolumn(l.column, i);
			}
		}
	}
	...
	void end()
	{
		if (layoutpass)
		{
			...
			if (!windowhit && !passthrough)
			{
				...
				if ((mousebuttons & G3D_PRESSED) && (fabs(hitx - firstx) > 2 || fabs(hity - firsty) > 2))
					mousebuttons |= G3D_DRAGGED;
				if (dist >= 0 && hitx >= -xsize / 2 && hitx <= xsize / 2 && hity <= 0)
				{
					if (hity >= -ysize || (tcurrent &&
										   hity >= -ysize - (FONTH - 2 * INSERT) -
													   ((skiny[6] - skiny[1]) - (skiny[3] - skiny[2])) * SKIN_SCALE &&
										   hitx <= tx - xsize / 2))
						windowhit = this;
				}
			}
		...
		else
		{
			if (tcurrent && tx < xsize)
				drawskin(curx + tx - skinx[5] * SKIN_SCALE, -ysize - skiny[6] * SKIN_SCALE, xsize - tx, FONTH, 9, 1,
						 gui2d ? 1 : 2, light, alpha);
		}
		poplist();
	}
...
const gui::patch gui::patches[] = {
	...
	{1, 2, 3, 6, 0}, // body
	{2, 9, 5, 6, 0x01}, {9, 10, 3, 6, 0},

	{1, 2, 6, 7, 0x10}, {2, 9, 6, 7, 0x11}, {9, 10, 6, 7, 0x10},

	{1, 2, 7, 9, 0},	{2, 9, 7, 9, 0x01}, {9, 10, 7, 9, 0},

	{5, 6, 3, 5, 0x01}, // top

	{2, 3, 1, 2, 0}, // selected tab
	{3, 4, 1, 2, 0x01}, {4, 5, 1, 2, 0},	{2, 3, 2, 3, 0x10},	 {3, 4, 2, 3, 0x11},
	{4, 5, 2, 3, 0x10}, {2, 3, 3, 5, 0},	{3, 4, 3, 5, 0x01},	 {4, 5, 3, 5, 0},

	{6, 7, 1, 2, 0}, // deselected tab
	{7, 8, 1, 2, 0x01}, {8, 9, 1, 2, 0},	{6, 7, 2, 3, 0x10},	 {7, 8, 2, 3, 0x11},
	{8, 9, 2, 3, 0x10}, {6, 7, 3, 5, 0},	{7, 8, 3, 5, 0x01},	 {8, 9, 3, 5, 0},
};
...
bool g3d_input(const char *str, int len)
{
	...
	if (fieldmode == FIELDKEY || fieldmode == FIELDSHOW || !e)
		return false;
...
bool g3d_key(int code, bool isdown)
{
	...
	if (fieldmode == FIELDKEY)
	{
		...
		if (keyname && isdown)
		{
			if (e->lines.length() != 1 || !e->lines[0].empty())
				e->insert(" ");
			e->insert(keyname);
		}
		return true;
	...
	if (code == -1 && g3d_windowhit(isdown, true))
		return true;
	else if (code == -3 && g3d_windowhit(isdown, false))
		return true;
				...
				case -4: // window "management"
					if (isdown)
					{
						if (windowhit->gui2d)
						{
							...
							for (int j = guis2d.length() - 1; j > i; j--)
								*guis2d[j].savedorigin = *guis2d[j - 1].savedorigin;
							*windowhit->savedorigin = origin;
							if (guis2d.length() > 1)
							{
								if (camera1->o.dist(*windowhit->savedorigin) <=
									camera1->o.dist(*guis2d.last().savedorigin))
					...
					if (isdown)
					{
						if (windowhit->gui2d)
						{
							...
							*guis2d.last().savedorigin = origin;
							if (guis2d.length() > 1)
							{
								if (camera1->o.dist(*guis2d.last().savedorigin) >=
									camera1->o.dist(*guis2d[0].savedorigin))
...
void g3d_addgui(g3d_callback *cb, vec &origin, int flags)
{
	...
	if (!gui2d && flags & GUI_FOLLOW && useguifollow)
		origin.z = player->o.z - (player->eyeheight - 1);
...
static inline bool g3d_sort(const gui &a, const gui &b)
{
...
void g3d_render2d()
{
	...
	flusheditors();
	if (!fieldsactive)
		fieldmode = FIELDSHOW; // didn't draw any fields, so lose focus - mainly for menu closed
...
void consolebox(int x1, int y1, int x2, int y2)
{
	...
	flushhudmatrix();
	gui::drawskin(-gui::skinx[1] * SKIN_SCALE, -gui::skiny[4] * SKIN_SCALE, int(bw), int(bh), 0, 9, 1, vec(1, 1, 1),
				  0.60f);
	gui::drawskin((-gui::skinx[1] + gui::skinx[2] - gui::skinx[5]) * SKIN_SCALE, -gui::skiny[4] * SKIN_SCALE, int(bw),
				  0, 9, 1, 1, vec(1, 1, 1), 0.60f);
	pophudmatrix();
}
