...
struct UniformLoc
{
	...
	UniformLoc(const char *name = NULL, const char *blockname = NULL, int binding = -1, int stride = -1)
		: name(name), blockname(blockname), loc(-1), version(-1), binding(binding), stride(stride), offset(-1),
		  size(-1), data(NULL)
	{
	}
};
...
struct Shader
{
	...
	Shader()
		: name(NULL), vsstr(NULL), psstr(NULL), defer(NULL), type(SHADER_DEFAULT), program(0), vsobj(0), psobj(0),
		  detailshader(NULL), variantshader(NULL), altshader(NULL), variantrows(NULL), standard(false), forced(false),
		  used(false), reusevs(NULL), reuseps(NULL), owner(NULL)
	{
	...
	void setvariant_(int col, int row, Shader *fallbackshader)
	{
		...
		if (detailshader->variantrows)
		{
			...
			{
				if (!detailshader->variants[col]->invalid())
				{
					s = detailshader->variants[col];
					break;
				}
			}
		}
...
struct GlobalShaderParam
{
	...
	template <class T> void setv(const T *v, int n = 1)
	{
	...
	template <class T> T *reserve(int n = 1)
	{
...
struct LocalShaderParam
{
	...
	template <class T> void sett(T x, T y, T z, T w)
	{
		ShaderParamBinding *b = resolve();
		if (b)
			switch (b->format)
...
struct Slot
{
	...
	Slot(int index = -1) : index(index), variants(NULL), autograss(NULL), layermaskname(NULL), layermask(NULL)
	{
		reset();
	}
...
extern Shader *nullshader, *hudshader, *hudnotextureshader, *textureshader, *notextureshader, *nocolorshader,
	*foggedshader, *foggednotextureshader, *stdworldshader;
