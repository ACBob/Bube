static struct flaretype
{
	...
	uchar alpha;						  /* color alpha */
				  ...
				  {3, 1.00f, 0.10f, 102},  {1, 0.50f, 0.20f, 77},	{3, 0.20f, 0.05f, 77},
				  {0, 0.00f, 0.04f, 77},   {5, -0.25f, 0.07f, 127}, {5, -0.40f, 0.02f, 153},
				  {5, -0.60f, 0.04f, 102}, {5, -1.00f, 0.03f, 51},	{-1, 1.00f, 0.30f, 255}, // shine - red, green, blue
				  {-2, 1.00f, 0.20f, 255}, {-3, 1.00f, 0.25f, 255}};
...
struct flarerenderer : partrenderer
{
	...
	void newflare(vec &o, const vec &center, uchar r, uchar g, uchar b, float mod, float size, bool sun, bool sparkle)
	{
		...
		if (!raycubelos(o, camera1->o, target))
			return;
	...
	void makelightflares()
	{
		...
		loopv(lights)
		{
			...
			if (!sun && (len > radius))
				continue;
			if (isvisiblesphere(0.0f, e.o) > (sun ? VFC_FOGGED : VFC_FULL_VISIBLE))
				continue;
			vec center = vec(camdir).mul(flaredir.dot(camdir)).add(camera1->o);
			float mod, size;
			if (sun) // fixed size
	...
	void render()
	{
		...
		loopi(numflares)
		{
			...
			loopj(f.sparkle ? 12 : 9)
			{
				...
				gle::attribf(o.x + (-camright.x + camup.x) * sz, o.y + (-camright.y + camup.y) * sz,
