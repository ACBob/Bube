...
Shader *nullshader = NULL, *hudshader = NULL, *hudnotextureshader = NULL, *textureshader = NULL,
	   *notextureshader = NULL, *nocolorshader = NULL, *foggedshader = NULL, *foggednotextureshader = NULL,
	   *stdworldshader = NULL;
...
void loadshaders()
{
	...
	if (!nullshader || !hudshader || !hudnotextureshader || !stdworldshader)
...
static void showglslinfo(GLenum type, GLuint obj, const char *name, const char **parts = NULL, int numparts = 0)
{
	...
	if (length > 1)
	{
		conoutf(CON_ERROR, "GLSL ERROR (%s:%s)",
				type == GL_VERTEX_SHADER ? "VS" : (type == GL_FRAGMENT_SHADER ? "FS" : "PROG"), name);
		FILE *l = getlogfile();
		if (l)
		{
...
static void compileglslshader(GLenum type, GLuint &obj, const char *def, const char *name, bool msg = true)
{
	...
	if (glslversion >= 130)
	{
		if (type == GL_VERTEX_SHADER)
			parts[numparts++] = "#define attribute in\n"
								"#define varying out\n";
		else if (type == GL_FRAGMENT_SHADER)
		{
			...
			if (glversion >= 300)
				parts[numparts++] = "out vec4 cube2_FragColor;\n"
...
static void bindglsluniform(Shader &s, UniformLoc &u)
{
	...
	if (bidx != GL_INVALID_INDEX && uidx != GL_INVALID_INDEX)
	{
		...
		if (dbgubo)
			conoutf(CON_DEBUG, "UBO: %s:%s:%d, offset: %d, size: %d, stride: %d", u.name, u.blockname, u.binding,
					offsetval, sizeval, strideval);
	}
}
...
#define UNIFORMTEX(name, tmu)                                                                                          \
	{                                                                                                                  \
		...
		if (type & SHADER_NORMALSLMS)
		{
			...
			UNIFORMTEX("lightmap", 1);
		if (type & SHADER_ENVMAP)
		...
		loopv(slot->sts)
		{
			...
			switch (t.type)
			{
				...
				case TEX_SPEC:
					if (t.combined < 0)
						UNIFORMTEX("specmap", tmu++);
					break;
				case TEX_DEPTH:
					if (t.combined < 0)
						UNIFORMTEX("depthmap", tmu++);
					break;
				case TEX_ALPHA:
					if (t.combined < 0)
						UNIFORMTEX("alphamap", tmu++);
					break;
				case TEX_UNKNOWN: {
...
#define SETDEFAULTPARAMS                                                                                               \
	...
	{
		if (this == lastshader || type & (SHADER_DEFERRED | SHADER_INVALID))
			return;
	...
	bool Shader::compile()
	{
		if (!vsstr)
			...
			compileglslshader(GL_VERTEX_SHADER, vsobj, vsstr, name, dbgshader || !variantshader);
		if (!psstr)
			psobj = !reuseps || reuseps->invalid() ? 0 : reuseps->psobj;
		else
			compileglslshader(GL_FRAGMENT_SHADER, psobj, psstr, name, dbgshader || !variantshader);
	...
	bool Shader::isnull(const Shader *s)
	{
		return !s;
	}
	...
	static void genuniformlocs(Shader &s, const char *vs, const char *ps, Shader *reusevs, Shader *reuseps)
	{
		...
		if (reusevs)
			...
			while ((vs = strstr(vs, "//:uniform")))
			{
				...
				if (numargs >= 3)
					s.uniformlocs.add(UniformLoc(getshaderparamname(name), getshaderparamname(blockname), binding,
												 numargs >= 4 ? stride : 0));
				else if (numargs >= 1)
	...
	void setupshaders()
	{
		...
		if (!nullshader || !hudshader || !hudnotextureshader)
	...
	{
		if (!strstr(vs, "//:water") && !strstr(ps, "//:water"))
			return false;
	...
	{
		return glversion < 300 && maxvaryings < 48;
	}
	...
	static void gendynlightvariant(Shader &s, const char *sname, const char *vs, const char *ps, int row = 0)
	{
		...
		if (sscanf(pspragma, "//:dynlight %100s", pslight) != 1)
			return;
		...
		loopi(MAXDYNLIGHTS)
		{
			...
			psdl.setsize(0);
			if (vsmain >= vs)
				vsdl.put(vs, vsmain - vs);
			if (psmain >= ps)
				psdl.put(ps, psmain - ps);
			...
			{
				defformatstring(dir, "%sdynlight%ddir%s", !k ? "varying vec3 " : " ", k,
								k == i || k + 1 == numlights ? ";\n" : ",");
			...
			loopk(i + 1)
			{
				...
				if (k < numlights)
					...
					psdl.put(tc, strlen(tc));

				defformatstring(
					dl, "%s.rgb += dynlightcolor[%d] * (1.0 - clamp(dot(dynlight%ddir, dynlight%ddir), 0.0, 1.0));\n",
					pslight, k, k, k);
			...
			if (!variant)
				return;
			if (row < 4)
				genwatervariant(s, name, vsdl.getbuf(), psdl.getbuf(), row + 2);
		}
	}
	...
	static void genshadowmapvariant(Shader &s, const char *sname, const char *vs, const char *ps, int row = 1)
	{
		...
		if (sscanf(pspragma, "//:shadowmap %100s", pslight) != 1)
			return;
		...
		if (vsmain >= vs)
			vssm.put(vs, vsmain - vs);
		if (psmain >= ps)
			pssm.put(ps, psmain - ps);

		const char *vsdecl = "uniform mat4 shadowmapproject;\n"
							 "varying vec3 shadowmaptc;\n";
		vssm.put(vsdecl, strlen(vsdecl));

		const char *psdecl = "uniform sampler2D shadowmap;\n"
		...
		const char *tcgen = "shadowmaptc = vec3(shadowmapproject * vvertex);\n";
		vssm.put(tcgen, strlen(tcgen));
		const char *sm = smoothshadowmappeel
		...
		defformatstring(smlight, "%s.rgb -= shadowed*clamp(%s.rgb - shadowmapambient.rgb, 0.0, 1.0);\n", pslight,
						pslight);
		...
		if (strstr(vs, "//:dynlight"))
			gendynlightvariant(s, name, vssm.getbuf(), pssm.getbuf(), row);
	}
	...
	static void genfogshader(vector<char> &vsbuf, vector<char> &psbuf, const char *vs, const char *ps)
	{
		...
		if (psmain && psend)
		{
			...
			const char *psfog = pspragma && !strncmp(pspragma + pragmalen, "rgba", 4)
									? "\ngl_FragColor = mix((FOG_COLOR), gl_FragColor, clamp(fogcoord*fogparams.x + "
									  "fogparams.y, 0.0, 1.0));\n"
									: "\ngl_FragColor.rgb = mix((FOG_COLOR).rgb, gl_FragColor.rgb, "
									  "clamp(fogcoord*fogparams.x + fogparams.y, 0.0, 1.0));\n";
			int clen = 0;
			if (pspragma)
			{
				pspragma += pragmalen;
	...
	static void genuniformdefs(vector<char> &vsbuf, vector<char> &psbuf, const char *vs, const char *ps,
							   Shader *variant = NULL)
	{
	...
	void Shader::fixdetailshader(bool shouldforce, bool recurse)
	{
		...
		if (recurse && detailshader)
			loopv(detailshader->variants) detailshader->variants[i]->fixdetailshader(shouldforce, false);
	}
...
#define GENSHADER(cond, body)                                                                                          \
	...
	{                                                                                                                  \
		...
		{
			if (strstr(vs, "//:water"))
				genwatervariant(*s, s->name, vs, ps);
			if (strstr(vs, "//:shadowmap"))
				genshadowmapvariant(*s, s->name, vs, ps);
			if (strstr(vs, "//:dynlight"))
				gendynlightvariant(*s, s->name, vs, ps);
		}
		slotparams.shrink(0);
	}
	...
	void variantshader(int *type, char *name, int *row, char *vs, char *ps)
	{
		...
		{
			if (strstr(vs, "//:dynlight"))
				gendynlightvariant(*s, varname, vs, ps, *row);
	...
	ICOMMAND(setuniformparam, "sffff", (char *name, float *x, float *y, float *z, float *w),
			 addslotparam(name, *x, *y, *z, *w));
	ICOMMAND(setshaderparam, "sffff", (char *name, float *x, float *y, float *z, float *w),
			 addslotparam(name, *x, *y, *z, *w));
	ICOMMAND(defuniformparam, "sffff", (char *name, float *x, float *y, float *z, float *w),
			 addslotparam(name, *x, *y, *z, *w));
...
#define NUMPOSTFXBINDS 10
	...
	void renderpostfx()
	{
		...
		loopv(postfxpasses)
		{
			...
			if (tex >= 0)
			{
				if (binds[p.outputbind] >= 0)
					postfxtexs[binds[p.outputbind]].used = -1;
	...
	static bool addpostfx(const char *name, int outputbind, int outputscale, uint inputs, uint freeinputs,
						  const vec4 &params)
	{
		if (!*name)
			return false;
	...
	ICOMMAND(addpostfx, "siisffff",
			 (char *name, int *bind, int *scale, char *inputs, float *x, float *y, float *z, float *w), {
				 ...
				 addpostfx(name, clamp(*bind, 0, NUMPOSTFXBINDS - 1), max(*scale, 0), inputmask, freemask,
						   vec4(*x, *y, *z, *w));
			 });
	...
	void cleanupshaders()
	{
		...
		loadedshaders = false;
		nullshader = hudshader = hudnotextureshader = textureshader = notextureshader = nocolorshader = foggedshader =
			foggednotextureshader = stdworldshader = NULL;
	...
	void setblurshader(int pass, int size, int radius, float *weights, float *offsets)
	{
		if (radius < 1 || radius > MAXBLURRADIUS)
			return;
		static Shader *blurshader[7][2] = {{NULL, NULL}, {NULL, NULL}, {NULL, NULL}, {NULL, NULL},
