...
void drawenvboxface(float s0, float t0, int x0, int y0, int z0, float s1, float t1, int x1, int y1, int z1, float s2,
					float t2, int x2, int y2, int z2, float s3, float t3, int x3, int y3, int z3, Texture *tex)
{
...
void drawenvbox(int w, float z1clip = 0.0f, float z2clip = 1.0f, int faces = 0x3F, Texture **sky = NULL)
{
	...
	if (faces & 0x01)
		drawenvboxface(0.0f, v2, -w, -w, z2, 1.0f, v2, -w, w, z2, 1.0f, v1, -w, w, z1, 0.0f, v1, -w, -w, z1, sky[0]);

	if (faces & 0x02)
		drawenvboxface(1.0f, v1, w, -w, z1, 0.0f, v1, w, w, z1, 0.0f, v2, w, w, z2, 1.0f, v2, w, -w, z2, sky[1]);

	if (faces & 0x04)
		drawenvboxface(1.0f, v1, -w, -w, z1, 0.0f, v1, w, -w, z1, 0.0f, v2, w, -w, z2, 1.0f, v2, -w, -w, z2, sky[2]);

	if (faces & 0x08)
		drawenvboxface(1.0f, v1, w, w, z1, 0.0f, v1, -w, w, z1, 0.0f, v2, -w, w, z2, 1.0f, v2, w, w, z2, sky[3]);

	if (z1clip <= 0 && faces & 0x10)
		drawenvboxface(0.0f, 1.0f, -w, w, -w, 0.0f, 0.0f, w, w, -w, 1.0f, 0.0f, w, -w, -w, 1.0f, 1.0f, -w, -w, -w,
					   sky[4]);

	if (z2clip >= 1 && faces & 0x20)
		drawenvboxface(0.0f, 1.0f, w, w, w, 0.0f, 0.0f, -w, w, w, 1.0f, 0.0f, -w, -w, w, 1.0f, 1.0f, w, -w, w, sky[5]);
}
...
namespace fogdome
{
	...
	void init(const bvec &color, float minalpha = 0.0f, float maxalpha = 1.0f, float capsize = -1, float clipz = 1,
			  int hres = 16, int depth = 2)
	{
	...
	void draw()
	{
		...
		glDrawRangeElements_(GL_TRIANGLES, 0, numverts - 1, numindices + fogdomecap * capindices, GL_UNSIGNED_SHORT,
							 indices);
		xtraverts += numverts;
		glde++;
...
HVARFR(atmosunlight, 0, 0, 0xFFFFFF, {
	if (atmosunlight <= 255)
		atmosunlight |= (atmosunlight << 8) | (atmosunlight << 16);
...
HVARFR(atmosundisk, 0, 0, 0xFFFFFF, {
	if (atmosundisk <= 255)
		atmosundisk |= (atmosundisk << 8) | (atmosundisk << 16);
...
static void drawatmosphere(int w, float z1clip = 0.0f, float z2clip = 1.0f, int faces = 0x3F)
{
	...
	vec atmoshells = vec(earthairheight, earthhazeheight, earthozoneheight)
						 .mul(atmoheight)
						 .add(planetradius)
						 .square()
						 .sub(planetradius * planetradius);
	...
	LOCALPARAMF(mieparams, miescale * (1 + gm * gm), miescale * -2 * gm,
				1 - (1 - cosf(0.5f * atmosundisksize * (1 - atmosundiskcorona) * RAD)));
	...
	vec suncolor =
		atmosunlight ? atmosunlightcolor.tocolor().mul(atmosunlightscale) : sunlightcolor.tocolor().mul(sunlightscale);
	...
	if (maxsunweight > 127)
		sunweight.mul(127 / maxsunweight);
	sunweight.add(1e-4f);
	...
	vec zenithweight =
		vec(betar).mul(zenithdepth.x).madd(betam, zenithdepth.y).madd(betao, zenithdepth.z - zenithdepth.x);
	vec zenithextinction = vec(zenithweight).sub(sunweight).exp2();
	vec diskcolor = (atmosundisk ? atmosundiskcolor.tocolor() : suncolor)
						.square()
						.mul(zenithextinction)
						.mul(atmosundiskbright * (glaring ? 1 : 1.5f))
						.min(1);
	LOCALPARAM(sundiskcolor, diskcolor);

	// convert from view cosine into mu^2 for limb darkening, where mu = sqrt(1 - sin^2) and sin^2 = 1 - cos^2, thus
	// mu^2 = 1 - (1 - cos^2*scale) convert corona offset into scale for mu^2, where sin = (1-corona) and thus mu^2 = 1
	// - (1-corona^2)
...
{
	if (spin || yaw % 90)
		return faces & 0x0F ? faces | 0x0F : faces;
	static const int faceidxs[3][4] = {{3, 2, 0, 1}, {1, 0, 3, 2}, {2, 3, 1, 0}};
	yaw /= 90;
	if (yaw < 1 || yaw > 3)
		return faces;
	const int *idxs = faceidxs[yaw - 1];
	return (faces & ~0x0F) | (((faces >> idxs[0]) & 1) << 0) | (((faces >> idxs[1]) & 1) << 1) |
		   (((faces >> idxs[2]) & 1) << 2) | (((faces >> idxs[3]) & 1) << 3);
}
...
void drawskybox(int farplane, bool limited, bool force)
{
	...
	bool alwaysrender = editmode || !insideworld(camera1->o) || reflecting || force, explicitonly = false;
	if (limited)
	{
		explicitonly = alwaysrender || !sparklyfix || refracting;
		if (!drawskylimits(explicitonly) && !alwaysrender)
			return;
		extern int ati_skybox_bug;
		if (!alwaysrender && !renderedskyfaces && !ati_skybox_bug)
			explicitonly = false;
	}
	else if (!alwaysrender)
	{
		...
		{
			if (va->occluded >= OCCLUDE_BB && va->skyfaces & 0x80)
				continue;
	...
	{
		if (refracting < 0)
			topclip = 0.5f + 0.5f * (reflectz - camera1->o.z) / float(worldsize);
		else if (reflectz > skyclip)
			skyclip = reflectz;
	}
	if (skyclip)
		skyclip = 0.5f + 0.5f * (skyclip - camera1->o.z) / float(worldsize);
	...
	if (!glaring)
	{
		...
		if (cloudbox[0])
		{
			...
			drawenvbox(farplane / 2, skyclip ? skyclip : cloudclip, topclip,
					   yawskyfaces(renderedskyfaces, yawclouds, spinclouds), clouds);
		...
		if (cloudlayer[0] && cloudheight && renderedskyfaces & (cloudheight < 0 ? 0x1F : 0x2F))
		{
			...
			drawenvoverlay(farplane / 2, cloudoverlay, cloudoffsetx + cloudscrollx * lastmillis / 1000.0f,
	...
	if (limited)
	{
		if (explicitonly)
			...
			glDepthFunc(GL_LESS);
		if (!reflecting && !refracting && !drawtex && editmode && showsky)
			drawskyoutline();
	}
	else
		glDepthFunc(GL_LESS);
...
bool limitsky()
{
	return (explicitsky && (useskytexture || editmode)) ||
		   (sparklyfix && skyarea / (double(worldsize) * double(worldsize) * 6) < 0.9);
}
