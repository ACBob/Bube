...
struct blobrenderer
{
	...
	blobrenderer(const char *texname)
		: texname(texname), tex(NULL), cache(NULL), cachesize(0), blobs(NULL), maxblobs(0), startblob(0), endblob(0),
		  verts(NULL), maxverts(0), startvert(0), endvert(0), availverts(0), indexes(NULL), maxindexes(0),
		  startindex(0), endindex(0), availindexes(0), ebo(0), vbo(0), edata(NULL), vdata(NULL), numedata(0),
		  numvdata(0), startrender(NULL), endrender(NULL), lastblob(NULL)
	{
	}
	...
	bool freeblob()
	{
		...
		if (b.flags & BL_RENDER)
			flushblobs();

		startblob++;
		if (startblob >= maxblobs)
			startblob = 0;

		startvert = b.endvert;
		if (startvert >= maxverts)
			startvert = 0;
		availverts += b.endvert - b.startvert;

		startindex = b.endindex;
		if (startindex >= maxindexes)
			startindex = 0;
	...
	template <int C> static int split(const vec *in, int numin, float below, float above, vec *out)
	{
		int numout = 0;
	...
	template <int C> static int clip(const vec *in, int numin, float below, float above, vec *out)
	{
		int numout = 0;
		const vec *p = &in[numin - 1];
		float pc = (*p)[C];
	...
	inline int addvert(const vec &pos)
	{
		...
		v.pos = pos;
		v.tc = vec2((pos.x - blobmin.x) / (blobmax.x - blobmin.x), (pos.y - blobmin.y) / (blobmax.y - blobmin.y));
		uchar alpha;
		if (pos.z < blobmin.z + blobfadelow)
			alpha = uchar(blobalphalow * (pos.z - blobmin.z));
		else if (pos.z > blobmax.z - blobfadehigh)
			alpha = uchar(blobalphahigh * (blobmax.z - pos.z));
		else
			alpha = blobalpha;
	...
	{
		if (endvert != int(lastblob->endvert) || endindex != int(lastblob->endindex))
			dupblob();
...
#undef GENFACEVERT
		...
		else if (cu.ext && (numverts = cu.ext->surfaces[orient].numverts & MAXFACEVERTS))
		{
			...
			return;
		else if (!vismask || (vismask & 0x40 && visibleface(cu, orient, o, size, MAT_AIR,
															(cu.material & MAT_ALPHA) ^ MAT_ALPHA, MAT_ALPHA)))
		{
			ivec v[4];
		...
		if (flat >= 0)
		{
			...
			if (offset < blobmin[dim] || offset > blobmax[dim])
				return;
			flat = dim;
		}
...
#define CLIPSIDE(clip, below, above)                                                                                   \
	{                                                                                                                  \
		...
		if (numv < 3)                                                                                                  \
			...
			if (flat != 0)
				CLIPSIDE(clip<0>, blobmin.x, blobmax.x);
			if (flat != 1)
				CLIPSIDE(clip<1>, blobmin.y, blobmax.y);
			if (flat != 2)
			{
	...
	void findmaterials(vtxarray *va)
	{
		...
		loopi(matsurfs)
		{
			...
			if (!isclipped(m.material & MATF_VOLUME) || m.orient == O_BOTTOM)
			{
				i += m.skip;
				continue;
			}
			...
			{
				materialsurface &m = matbuf[i];
				if (m.o[dim] >= blobmin[dim] && m.o[dim] <= blobmax[dim] && m.o[c] + m.csize >= blobmin[c] &&
					m.o[c] <= blobmax[c] && m.o[r] + m.rsize >= blobmin[r] && m.o[r] <= blobmax[r])
				{
					static cube dummy;
	...
	void findescaped(cube *cu, const ivec &o, int size, int escaped)
	{
		loopi(8)
		{
			if (escaped & (1 << i))
			{
				ivec co(i, o, size);
				if (cu[i].children)
					findescaped(cu[i].children, co, size >> 1, cu[i].escaped);
				else
				{
					int vismask = cu[i].merged;
					if (vismask)
						loopj(6) if (vismask & (1 << j)) gentris(cu[i], j, co, size);
				}
			}
		}
	}
	...
	void gentris(cube *cu, const ivec &o, int size, int escaped = 0)
	{
		...
		loopi(8)
		{
			if (overlap & (1 << i))
			{
				...
				if (cu[i].children)
					gentris(cu[i].children, co, size >> 1, cu[i].escaped);
				else
				{
					...
					{
						if (vismask & 0x80)
							loopj(6) gentris(cu[i], j, co, size, NULL, vismask);
						else
							loopj(6) if (vismask & (1 << j)) gentris(cu[i], j, co, size);
					}
				}
	...
	void fadeblob(blobinfo *b, float fade)
	{
		float minz = b->o.z - (blobheight + blobfadelow), maxz = b->o.z + blobfadehigh,
			  scale = fade * blobintensity * 255 / 100.0f, scalelow = scale / blobfadelow,
			  scalehigh = scale / blobfadehigh;
		uchar alpha = uchar(scale);
		b->millis = totalmillis;
		do
		{
			if (b->endvert - b->startvert >= 3)
				for (blobvert *v = &verts[b->startvert], *end = &verts[b->endvert]; v < end; v++)
				{
					float z = v->pos.z;
					if (z < minz + blobfadelow)
						v->color.a = uchar(scalelow * (z - minz));
					else if (z > maxz - blobfadehigh)
						v->color.a = uchar(scalehigh * (maxz - z));
					else
						v->color.a = alpha;
				}
			...
			if (offset >= maxblobs)
				offset = 0;
			if (offset < endblob ? offset > startblob || startblob > endblob : offset > startblob)
				b = &blobs[offset];
			else
				break;
	...
	void renderblob(const vec &o, float radius, float fade)
	{
		...
		if (glversion < 300 && lastrender != this)
		{
			...
			if (!lastrender || lastrender->tex != tex)
		...
		else if (fade < 1 && totalmillis - b->millis > 0)
			fadeblob(b, fade);
		do
		{
			if (b->endvert - b->startvert >= 3)
			{
				if (glversion >= 300)
				{
					...
					{
						numedata = numvdata = 0;
						startrender = endrender = b;
					}
					else
					{
						endrender->next = ushort(b - blobs);
						endrender = b;
					}
					b->flags |= BL_RENDER;
					b->next = 0xFFFF;
				...
				{
					glDrawRangeElements_(GL_TRIANGLES, b->startvert, b->endvert - 1, b->endindex - b->startindex,
	...
	void flushblobs()
	{
		...
		{
			if (!edata)
				edata = new ushort[maxindexes];
			if (!vdata)
				vdata = new blobvert[maxverts];
			estart = edata;
			vstart = vdata;
		}
		else
		{
			estart =
				(ushort *)glMapBufferRange_(GL_ELEMENT_ARRAY_BUFFER, 0, numedata * sizeof(ushort),
											GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
			vstart = (blobvert *)glMapBufferRange_(GL_ARRAY_BUFFER, 0, numvdata * sizeof(blobvert),
...
static blobrenderer blobs[] = {blobrenderer("<grey>packages/particles/blob.png"),
							   blobrenderer("<grey>packages/particles/blob.png")};

void initblobs(int type)
{
	if (type < 0 || (type == BLOB_STATIC && blobs[BLOB_STATIC].blobs))
		blobs[BLOB_STATIC].init(showblobs ? blobstattris : 0);
	if (type < 0 || (type == BLOB_DYNAMIC && blobs[BLOB_DYNAMIC].blobs))
		blobs[BLOB_DYNAMIC].init(showblobs ? blobdyntris : 0);
}
...
void renderblob(int type, const vec &o, float radius, float fade)
{
	if (!showblobs)
		return;
	if (refracting < 0 && o.z - blobheight - blobfadelow >= reflectz)
		return;
...
void flushblobs()
{
	...
	if (blobrenderer::lastrender)
		blobrenderer::cleanuprenderstate();
