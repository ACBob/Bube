...
struct decalrenderer
{
	...
	decalrenderer(const char *texname, int flags = 0, int fadeintime = 0, int fadeouttime = 1000, int timetolive = -1)
		: texname(texname), flags(flags), fadeintime(fadeintime), fadeouttime(fadeouttime), timetolive(timetolive),
		  tex(NULL), decals(NULL), maxdecals(0), startdecal(0), enddecal(0), verts(NULL), maxverts(0), startvert(0),
		  endvert(0), lastvert(0), availverts(0), vbo(0), dirty(false), decalu(0), decalv(0)
	{
	}
	...
	int freedecal()
	{
		if (startdecal == enddecal)
			return 0;

		decalinfo &d = decals[startdecal];
		startdecal++;
		if (startdecal >= maxdecals)
			startdecal = 0;
	...
	void fadedecal(decalinfo &d, uchar alpha)
	{
		...
		if (flags & DF_OVERBRIGHT)
			rgb = bvec(128, 128, 128);
		else
		{
			rgb = d.color;
			if (flags & (DF_ADD | DF_INVMOD))
				rgb.scale(alpha, 255);
		}
		bvec4 color(rgb, alpha);

		decalvert *vert = &verts[d.startvert], *end = &verts[d.endvert < d.startvert ? maxverts : d.endvert];
		while (vert < end)
		{
			vert->color = color;
			vert++;
	...
	void clearfadeddecals()
	{
		...
		decalinfo *d = &decals[startdecal], *end = &decals[enddecal < startdecal ? maxdecals : enddecal];
		while (d < end && d->millis <= threshold)
			d++;
		...
		startdecal = d - decals;
		if (prevstart == startdecal)
			return;
		if (startdecal != enddecal)
			startvert = decals[startdecal].startvert;
		else
			startvert = endvert = lastvert = 0;
	...
	void fadeindecals()
	{
		if (!fadeintime)
			return;
		decalinfo *d = &decals[enddecal], *end = &decals[enddecal < startdecal ? 0 : startdecal];
		while (d > end)
		{
			d--;
	...
	void adddecal(const vec &center, const vec &dir, float radius, const bvec &color, int info)
	{
		...
		if (dbgdec)
		{
			...
			conoutf(CON_DEBUG, "tris = %d, verts = %d, total tris = %d", nverts / 3, nverts,
					(maxverts - 3 - availverts) / 3);
		}
		if (endvert == lastvert)
			return;
...
#undef GENFACEVERT
	...
	void findmaterials(vtxarray *va)
	{
		...
		loopi(matsurfs)
		{
			...
			if (dc ? decalnormal[dim] <= 0 : decalnormal[dim] >= 0)
			{
				i += m.skip;
				continue;
			}
...
decalrenderer decals[] = {decalrenderer("<grey>packages/particles/scorch.png", DF_ROTATE, 500),
...
{
	if (!showdecals || type < 0 || (size_t)type >= sizeof(decals) / sizeof(decals[0]) ||
		center.dist(camera1->o) - radius > maxdecaldistance)
		return;
