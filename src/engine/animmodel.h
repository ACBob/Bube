...
struct animmodel : model
{
	...
	struct shaderparams
	{
		float spec, ambient, glow, glowdelta, glowpulse, specglare, glowglare, fullbright, envmapmin, envmapmax,
			scrollu, scrollv, alphatest;

		shaderparams()
			: spec(1.0f), ambient(0.3f), glow(3.0f), glowdelta(0), glowpulse(0), specglare(1), glowglare(1),
			  fullbright(0), envmapmin(0), envmapmax(0), scrollu(0), scrollv(0), alphatest(0.9f)
		{
		}
	};
	...
	struct skin : shaderparams
	{
		...
		skin()
			: owner(0), tex(notexture), masks(notexture), envmap(NULL), normalmap(NULL), shader(NULL), alphablend(true),
			  cullface(true), key(NULL)
		{
		}
		...
		bool envmapped()
		{
			return envmapmax > 0 && envmapmodels;
		}
		bool bumpmapped()
		{
			return normalmap && bumpmodels;
		}
		bool tangents()
		{
			return bumpmapped();
...
#define SETMODELSHADER(m, name) DOMODELSHADER(name, (m)->setshader(name##shader))
			...
			if (masked())
				opts[optslen++] = 'm';
			if (!fullbright && (masked() || spec >= 0.01f))
				opts[optslen++] = 's';
			opts[optslen++] = '\0';
		...
		void preloadBIH()
		{
			if (tex->type & Texture::ALPHA && !tex->alphamask)
				loadalphamask(tex);
		}
		...
		{
			if (!cullface && enablecullface)
			{
				glDisable(GL_CULL_FACE);
				enablecullface = false;
			}
			else if (cullface && !enablecullface)
			{
			...
			if (as->cur.anim & ANIM_NOSKIN)
			{
				...
				if (shadowmapping)
					SETMODELSHADER(b, shadowmapcaster);
				else /*if(as->cur.anim&ANIM_SHADOW)*/
					SETMODELSHADER(b, notexturemodel);
				return;
			}
			setshader(b, as);
			...
			else if (enablealphablend && transparent >= 1)
			{
				glDisable(GL_BLEND);
				enablealphablend = false;
			}
	...
	struct mesh
	{
		...
		void calctangents(B *bumpverts, V *verts, TC *tcverts, int numverts, T *tris, int numtris, bool areaweight)
		{
			...
			loopi(numverts)
			{
				const vec &n = verts[i].norm, &t = tangent[i], &bt = bitangent[i];
				B &bv = bumpverts[i];
				matrix3 m;
				m.c = n;
		...
		void calctangents(B *bumpverts, V *verts, TC *tcverts, int numverts, T *tris, int numtris, bool areaweight,
						  int numframes)
		{
			loopi(numframes) calctangents(&bumpverts[i * numverts], &verts[i * numverts], tcverts, numverts, tris,
										  numtris, areaweight);
		}
	};
	...
	struct part
	{
		...
		part(animmodel *model, int index = 0)
			: model(model), index(index), meshes(NULL), numanimparts(1), pitchscale(1), pitchoffset(0), pitchmin(0),
			  pitchmax(0), translate(0, 0, 0)
		{
		...
		bool link(part *p, const char *tag, const vec &translate = vec(0, 0, 0), int anim = -1, int basetime = 0,
				  vec *pos = NULL)
		{
		...
		{
			if (animpart < 0 || animpart >= MAXANIMPARTS)
				return;
	...
	void render(int anim, int basetime, int basetime2, const vec &o, float yaw, float pitch, dynent *d, modelattach *a,
				const vec &color, const vec &dir, float trans)
	{
	...
	void initmatrix(matrix4x3 &m)
	{
		...
		if (offsetpitch)
			m.rotate_around_y(-offsetpitch * RAD);
	}
	...
	static bool enabletc, enablealphablend, enablecullface, enablenormals, enabletangents, enablebones,
		enabledepthoffset;
...
bool animmodel::enabletc = false, animmodel::enablealphablend = false, animmodel::enablecullface = true,
	 animmodel::enablenormals = false, animmodel::enabletangents = false, animmodel::enablebones = false,
...
GLuint animmodel::lastvbuf = 0, animmodel::lasttcbuf = 0, animmodel::lastnbuf = 0, animmodel::lastxbuf = 0,
	   animmodel::lastbbuf = 0, animmodel::lastebuf = 0, animmodel::lastenvmaptex = 0, animmodel::closestenvmaptex = 0;
...
#define loopskins(meshname, s, body)                                                                                   \
	...
	static void setlink(int *parent, int *child, char *tagname, float *x, float *y, float *z)
	{
		...
		if (!MDL::loading->parts[*parent]->link(MDL::loading->parts[*child], tagname, vec(*x, *y, *z)))
			conoutf(CON_ERROR, "could not link model %s", MDL::loading->name);
	}
