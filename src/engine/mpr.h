...
namespace mpr
{
	...
	struct EntOBB : Ent
	{
		...
		vec center() const
		{
			return vec(ent->o.x, ent->o.y, ent->o.z + (ent->aboveeye - ent->eyeheight - zmargin) / 2);
		}

		vec contactface(const vec &wn, const vec &wdir) const
		{
			vec n = orient.transform(wn).div(
					vec(ent->xradius, ent->yradius, (ent->aboveeye + ent->eyeheight + zmargin) / 2)),
		...
		{
			return vec(ln.x > 0 ? ent->xradius : -ent->xradius, ln.y > 0 ? ent->yradius : -ent->yradius,
		...
		float left() const
		{
			return supportcoordneg(orient.a.x, orient.b.x, orient.c.x) + ent->o.x;
		}
		float right() const
		{
			return supportcoord(orient.a.x, orient.b.x, orient.c.x) + ent->o.x;
		}
		float back() const
		{
			return supportcoordneg(orient.a.y, orient.b.y, orient.c.y) + ent->o.y;
		}
		float front() const
		{
			return supportcoord(orient.a.y, orient.b.y, orient.c.y) + ent->o.y;
		}
		float bottom() const
		{
			return ent->o.z - ent->eyeheight - zmargin;
		}
		float top() const
		{
	...
	struct EntCylinder : EntFuzzy
	{
		...
		vec contactface(const vec &n, const vec &dir) const
		{
			float dxy = n.dot2(n) / (ent->radius * ent->radius),
				  dz = n.z * n.z * 4 / (ent->aboveeye + ent->eyeheight + zmargin);
			vec fn(0, 0, 0);
			if (dz > dxy && dir.z)
				fn.z = n.z * dir.z < 0 ? (n.z > 0 ? 1 : -1) : 0;
	...
	{
		ModelOBB(const vec &ent, const vec &center, const vec &radius, int yaw) : Model(ent, center, radius, yaw)
		{
		}
	...
	{
		ModelEllipse(const vec &ent, const vec &center, const vec &radius, int yaw) : Model(ent, center, radius, yaw)
		{
		}
