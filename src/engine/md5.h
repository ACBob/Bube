...
struct md5 : skelloader<md5>
{
	...
	struct md5mesh : skelmesh
	{
		...
		void load(stream *f, char *buf, size_t bufsize)
		{
			...
			while (f->getline(buf, bufsize) && buf[0] != '}')
			{
				...
				else if (sscanf(buf, " weight %d %d %f ( %f %f %f ) ", &index, &w.joint, &w.bias, &w.pos.x, &w.pos.y,
								&w.pos.z) == 6)
				{
					w.pos.y = -w.pos.y;
			...
			while (f->getline(buf, sizeof(buf)))
			{
				...
				else if (strstr(buf, "joints {"))
				{
					...
					while (f->getline(buf, sizeof(buf)) && buf[0] != '}')
					{
						...
						*curname = '\0';
						if (sscanf(curbuf, " %d ( %f %f %f ) ( %f %f %f )", &parent, &j.pos.x, &j.pos.y, &j.pos.z,
		...
		skelanimspec *loadanim(const char *filename)
		{
			...
			while (f->getline(buf, sizeof(buf)))
			{
				...
				else if (strstr(buf, "baseframe {"))
				{
					...
					{
						md5joint j;
						if (sscanf(buf, " ( %f %f %f ) ( %f %f %f )", &j.pos.x, &j.pos.y, &j.pos.z, &j.orient.x,
								   &j.orient.y, &j.orient.z) == 6)
						{
							j.pos.y = -j.pos.y;
							j.orient.x = -j.orient.x;
							j.orient.z = -j.orient.z;
				...
				else if (sscanf(buf, " frame %d", &tmp) == 1)
				{
					...
					loopv(basejoints)
					{
						...
						if (adjustments.inrange(i))
							adjustments[i].adjust(frame[i]);
						frame[i].mul(skel->bones[i].invbase);
						if (h.parent >= 0)
							frame[i].mul(skel->bones[h.parent].base, dualquat(frame[i]));
	...
	meshgroup *loadmeshes(const char *name, va_list args)
	{
		...
		if (!group->load(name, va_arg(args, double)))
		{
			delete group;
			return NULL;
		}
