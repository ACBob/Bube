...
const char *partnames[] = {"part",	"tape",	   "trail",	   "text",		"texticon",
						   "meter", "metervs", "fireball", "lightning", "flare"};
...
struct partrenderer
{
	...
	partrenderer(int type, int collide = 0) : tex(NULL), texname(NULL), texclamp(0), type(type), collide(collide)
	{
	}
		...
		else
		{
			...
			if (collide && o.z < p->val && step)
			{
				if (collide >= 0)
				{
					...
					{
						adddecal(collide, vec(o.x, o.y, collidez), vec(p->o).sub(o).normalize(), 2 * p->size, p->color,
...
struct listrenderer : partrenderer
{
	...
	listrenderer(int type, int collide = 0) : partrenderer(type, collide), list(NULL)
	{
	}
...
{
	meterrenderer(int type) : listrenderer(type | PT_NOTEX | PT_LERP)
	{
	}
...
{
	textrenderer(int type) : listrenderer(type)
	{
	}
...
{
	texticonrenderer(const char *texname, int type) : listrenderer(texname, 3, type)
	{
	}
...
template <int T> static inline void modifyblend(const vec &o, int &blend)
{
	blend = min(blend << 2, 255);
}

template <> inline void modifyblend<PT_TAPE>(const vec &o, int &blend)
{
}

template <int T> static inline void genpos(const vec &o, const vec &d, float size, int grav, int ts, partvert *vs)
{
...
template <> inline void genpos<PT_TAPE>(const vec &o, const vec &d, float size, int ts, int grav, partvert *vs)
{
	vec dir1 = d, dir2 = d, c;
	dir1.sub(o);
	dir2.sub(camera1->o);
...
template <> inline void genpos<PT_TRAIL>(const vec &o, const vec &d, float size, int ts, int grav, partvert *vs)
{
	vec e = d;
	if (grav)
		e.z -= float(ts) / grav;
...
#define ROTCOEFFS(n)                                                                                                   \
	{                                                                                                                  \
		vec(-1, 1, 0).rotate_around_z(n * 2 * M_PI / 32.0f), vec(1, 1, 0).rotate_around_z(n * 2 * M_PI / 32.0f),       \
			vec(1, -1, 0).rotate_around_z(n * 2 * M_PI / 32.0f), vec(-1, -1, 0).rotate_around_z(n * 2 * M_PI / 32.0f)  \
	}
...
template <int T> struct varenderer : partrenderer
{
	partvert *verts;
	particle *parts;
	...
	varenderer(const char *texname, int type, int collide = 0)
		: partrenderer(texname, 3, type, collide), verts(NULL), parts(NULL), maxparts(0), numparts(0), lastupdate(-1),
		  rndmask(0), vbo(0)
	{
		if (type & PT_HFLIP)
			rndmask |= 0x01;
...
{
	softquadrenderer(const char *texname, int type, int collide = 0) : quadrenderer(texname, type | PT_SOFT, collide)
	{
	}

	int adddepthfx(vec &bbmin, vec &bbmax)
	{
		if (!depthfxtex.highprecision() && !depthfxtex.emulatehighprecision())
			return 0;
		int numsoft = 0;
		loopi(numparts)
		{
...
static partrenderer *parts[] = {
	new quadrenderer("<grey>packages/particles/blood.png", PT_PART | PT_FLIP | PT_MOD | PT_RND4,
					 DECAL_BLOOD),										  // blood spats (note: rgb is inverted)
	...
	new quadrenderer("<grey>packages/particles/flames.png",
					 PT_PART | PT_HFLIP | PT_RND4 | PT_GLARE), // flame on - no flipping please, they have orientation
	...
	new quadrenderer("packages/particles/muzzleflash1.jpg",
					 PT_PART | PT_FEW | PT_FLIP | PT_GLARE | PT_TRACK), // muzzle flash
	new quadrenderer("packages/particles/muzzleflash2.jpg",
					 PT_PART | PT_FEW | PT_FLIP | PT_GLARE | PT_TRACK), // muzzle flash
	new quadrenderer("packages/particles/muzzleflash3.jpg",
					 PT_PART | PT_FEW | PT_FLIP | PT_GLARE | PT_TRACK),						// muzzle flash
...
void finddepthfxranges()
{
	...
	numdepthfxranges =
		fireballs.finddepthfxranges(depthfxowners, depthfxranges, 0, MAXDFXRANGES, depthfxmin, depthfxmax);
	numdepthfxranges = bluefireballs.finddepthfxranges(depthfxowners, depthfxranges, numdepthfxranges, MAXDFXRANGES,
													   depthfxmin, depthfxmax);
	loopk(3)
	{
	...
	if (depthfxscissor < 2 && numdepthfxranges > 0)
		depthfxtex.addscissorbox(depthfxmin, depthfxmax);
}
...
void debugparticles()
{
	...
	pushhudmatrix();
	hudmatrix.ortho(0, FONTH * n * 2 * screenw / float(screenh), FONTH *n * 2, 0, -1, 1); // squeeze into top-left
																						  // corner
	flushhudmatrix();
	hudshader->set();
...
void renderparticles(bool mainpass)
{
	...
	if (dbgparts && mainpass)
		loopi(sizeof(parts) / sizeof(parts[0])) parts[i]->debuginfo();

	if (glaring && !particleglare)
		return;

	loopi(sizeof(parts) / sizeof(parts[0]))
	{
		if (glaring && !(parts[i]->type & PT_GLARE))
			continue;
		parts[i]->update();
	}

	bool rendered = false;
	uint lastflags = PT_LERP | PT_SHADER, flagmask = PT_LERP | PT_MOD | PT_SHADER | PT_NOTEX;
	...
	loopi(sizeof(parts) / sizeof(parts[0]))
	{
		...
		if (!rendered)
		{
			...
			if (glaring)
				GLOBALPARAMF(colorscale, particleglare, particleglare, particleglare, 1);
			else
...
static void splash(int type, int color, int radius, int num, int fade, const vec &p, float size, int gravity)
{
	if (camera1->o.dist(p) > maxparticledistance && !seedemitter)
		return;
	float collidez = parts[type]->collide ? p.z - raycube(p, vec(0, 0, -1), COLLIDERADIUS, RAY_CLIPMAT) +
	...
	loopi(num)
	{
		...
		int f = (num < 10) ? (fmin + rnd(fmax))
						   : (fmax - (i * (fmax - fmin)) /
										 (num - 1)); // help deallocater by using fade distribution rather than random
		newparticle(p, tmp, f, type, color, size, gravity)->val = collidez;
	}
}

static void regularsplash(int type, int color, int radius, int num, int fade, const vec &p, float size, int gravity,
						  int delay = 0)
{
...
void regular_particle_splash(int type, int num, int fade, const vec &p, int color, float size, int radius, int gravity,
							 int delay)
{
	if (!canaddparticles())
		return;
...
void particle_texticon(const vec &s, int ix, int iy, float offset, int type, int fade, int color, float size,
					   int gravity)
{
	if (!canaddparticles())
		return;
	...
	loopi(num)
	{
		...
		else
		{
			...
			if (parts[type]->collide)
				n->val = from.z -
						 raycube(from, vec(0, 0, -1), parts[type]->collide >= 0 ? COLLIDERADIUS : max(from.z, 0.0f),
								 RAY_CLIPMAT) +
						 (parts[type]->collide >= 0 ? COLLIDEERROR : 0);
		}
	}
...
static void regularflame(int type, const vec &p, float radius, float height, int color, int density = 3,
						 float scale = 2.0f, float speed = 200.0f, float fade = 600.0f, int gravity = -15)
{
	if (!canemitparticles())
		return;
...
void regular_particle_flame(int type, const vec &p, float radius, float height, int color, int density, float scale,
							float speed, float fade, int gravity)
{
	if (!canaddparticles())
		return;
...
static void makeparticles(entity &e)
{
	switch (e.attr1)
	{
		case 0: // fire and smoke -  <radius> <height> <rgb> - 0 values default to compat for old maps
		{
			...
			regularflame(PART_SMOKE, vec(e.o.x, e.o.y, e.o.z + 4.0f * min(radius, height)), radius, height, 0x303020, 1,
						 4.0f, 100.0f, 2000.0f, -20);
			break;
		}
		...
		case 13: // snow
		{
			static const int typemap[] = {PART_STREAK, -1, -1, PART_LIGHTNING, -1, PART_STEAM,
			...
			if (e.attr2 >= 256)
				regularshape(type, max(1 + e.attr3, 1), colorfromattr(e.attr4), e.attr2 - 256, 5,
							 e.attr5 > 0 ? min(int(e.attr5), 10000) : 200, e.o, size, gravity);
			else
				newparticle(e.o, offsetvec(e.o, e.attr2, max(1 + e.attr3, 0)), 1, type, colorfromattr(e.attr4), size,
							gravity);
			break;
		}
		case 5: // meter, metervs - <percent> <rgb> <rgb2>
		case 6: {
			particle *p = newparticle(e.o, vec(0, 0, 1), 1, e.attr1 == 5 ? PART_METER : PART_METER_VS,
									  colorfromattr(e.attr3), 2.0f);
		...
		case 11: // flame <radius> <height> <rgb> - radius=100, height=100 is the classic size
			regularflame(PART_FLAME, e.o, float(e.attr2) / 100.0f, float(e.attr3) / 100.0f, colorfromattr(e.attr4), 3,
						 2.0f);
			break;
		case 12: // smoke plume <radius> <height> <rgb>
			regularflame(PART_SMOKE, e.o, float(e.attr2) / 100.0f, float(e.attr3) / 100.0f, colorfromattr(e.attr4), 1,
						 4.0f, 100.0f, 2000.0f, -20);
			break;
...
bool printparticles(extentity &e, char *buf, int len)
{
	switch (e.attr1)
	{
		...
		case 12:
		case 13:
			nformatstring(buf, len, "%s %d %d %d 0x%.3hX %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3,
						  e.attr4, e.attr5);
			return true;
		case 3:
			nformatstring(buf, len, "%s %d %d 0x%.3hX %d %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3,
						  e.attr4, e.attr5);
			return true;
		case 5:
		case 6:
			nformatstring(buf, len, "%s %d %d 0x%.3hX 0x%.3hX %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3,
						  e.attr4, e.attr5);
			return true;
	}
	return false;
