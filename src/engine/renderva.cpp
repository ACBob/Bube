	...
	loopv(vas)
	{
		...
		if (v.curvfc != VFC_NOT_VISIBLE)
		{
			...
			if (v.children.length())
				findvisiblevas(v.children, prevvfc >= VFC_NOT_VISIBLE);
...
static inline bool insideva(const vtxarray *va, const vec &v, int margin = 2)
{
	...
	return v.x >= va->o.x - margin && v.y >= va->o.y - margin && v.z >= va->o.z - margin && v.x <= va->o.x + size &&
		   v.y <= va->o.y + size && v.z <= va->o.z + size;
}
...
struct queryframe
{
	...
	void reset()
	{
		loopi(max) queries[i].owner = NULL;
	}
...
void findvisiblemms(const vector<extentity *> &ents, bool doquery)
{
	...
	{
		if (va->mapmodels.empty() || va->curvfc >= VFC_FOGGED || va->occluded >= OCCLUDE_BB)
			continue;
		loopv(va->mapmodels)
		{
...
void rendermapmodel(extentity &e)
{
	...
	if (mmi)
		rendermodel(&e.light, mmi->name, anim, e.o, e.attr1, 0, MDL_CULL_VFC | MDL_CULL_DIST | MDL_DYNLIGHT, NULL, NULL,
					basetime);
}
...
void renderreflectedmapmodels()
{
	...
	if (reflecting)
	{
		...
		{
			if (va->mapmodels.empty() || va->distance > reflectdist)
				continue;
			loopv(va->mapmodels)
			{
				octaentities *oe = va->mapmodels[i];
	...
	for (octaentities *oe = mms; oe; oe = reflecting ? oe->rnext : oe->next)
		if (reflecting || oe->distance >= 0)
		{
			if (reflecting || refracting > 0 ? oe->bbmax.z <= reflectz : oe->bbmin.z >= reflectz)
				continue;
...
void renderoutline()
{
	...
	for (vtxarray *va = visibleva; va; va = va->next)
	{
		if (va->occluded >= OCCLUDE_BB)
			continue;
		if (!va->alphaback && !va->alphafront && (!va->texs || va->occluded >= OCCLUDE_GEOM))
			continue;
...
void rendershadowmapreceivers()
{
	...
	for (vtxarray *va = visibleva; va; va = va->next)
	{
		if (!va->texs || va->curvfc >= VFC_FOGGED || !isshadowmapreceiver(va))
			continue;
...
void renderdepthobstacles(const vec &bbmin, const vec &bbmax, float scale, float *ranges, int numranges)
{
	...
	for (vtxarray *va = visibleva; va; va = va->next)
	{
		if (!va->texs || va->occluded >= OCCLUDE_GEOM || va->o.x > bbmax.x || va->o.y > bbmax.y || va->o.z > bbmax.z ||
...
struct renderstate
{
	...
	renderstate()
		: colormask(true), depthmask(true), blending(false), alphaing(0), vbuf(0), vattribs(false), vquery(false),
		  colorscale(1, 1, 1), alphascale(0), slot(NULL), texgenslot(NULL), vslot(NULL), texgenvslot(NULL),
		  texgenscroll(0, 0), texgendim(-1), visibledynlights(0), dynlightmask(0)
	{
...
struct geombatch
{
	...
	geombatch(const elementset &es, ushort *edata, vtxarray *va)
		: es(es), vslot(lookupvslot(es.texture)), edata(edata), va(va), next(-1), batch(-1)
	{
	}
	...
	int compare(const geombatch &b) const
	{
		...
		if (va->vbuf > b.va->vbuf)
			return 1;
		if (va->dynlightmask < b.va->dynlightmask)
			return -1;
		if (va->dynlightmask > b.va->dynlightmask)
			return 1;
...
static void changeslottmus(renderstate &cur, int pass, Slot &slot, VSlot &vslot)
{
	...
	if (cur.alphaing)
	{
		...
		if (cur.colorscale != vslot.colorscale || cur.alphascale != alpha)
		{
			...
			GLOBALPARAMF(colorparams, 2 * alpha * vslot.colorscale.x, 2 * alpha * vslot.colorscale.y,
						 2 * alpha * vslot.colorscale.z, alpha);
	...
	if (slot.shader->type & SHADER_NORMALSLMS)
		tmu++;
	...
	loopvj(slot.sts)
	{
		...
		if (t.type == TEX_DIFFUSE || t.combined >= 0)
			continue;
...
static void changeshader(renderstate &cur, Shader *s, Slot &slot, VSlot &vslot, bool shadowed)
{
	if (glaring)
	{
		...
		if (cur.blending)
		{
			if (!noglareblendshader)
				noglareblendshader = lookupshaderbyname("noglareblendworld");
			fallback = noglareblendshader;
		}
		else if (cur.alphaing)
		{
			if (!noglarealphashader)
				noglarealphashader = lookupshaderbyname("noglarealphaworld");
			fallback = noglarealphashader;
		}
		else
		{
			if (!noglareshader)
				noglareshader = lookupshaderbyname("noglareworld");
			fallback = noglareshader;
		}
		if (s->hasoption(4))
			s->setvariant(cur.visibledynlights, 4, slot, vslot, fallback);
		else
			s->setvariant(cur.blending ? 1 : 0, 4, slot, vslot, fallback);
	}
	else if (fading && !cur.blending && !cur.alphaing)
	{
		if (shadowed)
			s->setvariant(cur.visibledynlights, 3, slot, vslot);
		else
			s->setvariant(cur.visibledynlights, 2, slot, vslot);
	}
	else if (shadowed)
		s->setvariant(cur.visibledynlights, 1, slot, vslot);
	else if (!cur.visibledynlights)
		s->set(slot, vslot);
	else
		s->setvariant(cur.visibledynlights - 1, 0, slot, vslot);
}
...
static void changetexgen(renderstate &cur, int dim, Slot &slot, VSlot &vslot)
{
	if (cur.texgenslot != &slot || cur.texgenvslot != &vslot)
	{
			 ...
			 cur.texgenvslot->scroll != vslot.scroll))
		{
			...
			float xs = r.flipx ? -tex->xs : tex->xs, ys = r.flipy ? -tex->ys : tex->ys;
...
static void renderbatch(renderstate &cur, int pass, geombatch &b)
{
	...
	for (geombatch *curbatch = &b;; curbatch = &geombatches[curbatch->batch])
	{
		...
		if (len)
		{
			...
			{
				minvert = min(minvert, curbatch->es.minvert[1]);
				maxvert = max(maxvert, curbatch->es.maxvert[1]);
			}
			drawtris(len, curbatch->edata, minvert, maxvert);
			vtris += len / 3;
		}
		if (curbatch->es.length[1] > len && !shadowed)
			shadowed = curbatch;
		if (curbatch->batch < 0)
			break;
	}
	if (shadowed)
		for (geombatch *curbatch = shadowed;; curbatch = &geombatches[curbatch->batch])
		{
			if (curbatch->va->shadowed && curbatch->es.length[1] > curbatch->es.length[0])
			{
				...
				drawtris(len, curbatch->edata + curbatch->es.length[0], curbatch->es.minvert[1],
						 curbatch->es.maxvert[1]);
				vtris += len / 3;
			}
			if (curbatch->batch < 0)
				break;
...
static void renderbatches(renderstate &cur, int pass)
{
	...
	while (curbatch >= 0)
	{
		...
		if (cur.vslot != &b.vslot)
		{
			...
			if (cur.texgendim != b.es.dim || (cur.texgendim <= 2 && cur.texgenvslot != &b.vslot))
				changetexgen(cur, b.es.dim, *b.vslot.slot, b.vslot);
		}
		else if (cur.texgendim != b.es.dim)
			changetexgen(cur, b.es.dim, *b.vslot.slot, b.vslot);
		if (pass == RENDERPASS_LIGHTMAP)
...
void renderfoggedvas(renderstate &cur, bool doquery = false)
{
	...
	if (!fogshader)
		fogshader = lookupshaderbyname("fogworld");
	if (fading)
		fogshader->setvariant(0, 2);
	else
		fogshader->set();
	...
	loopv(foggedvas)
	{
		...
		if (cur.vbuf != va->vbuf)
			changevbuf(cur, RENDERPASS_FOG, va);
...
static void rendergeommultipass(renderstate &cur, int pass, bool fogpass)
{
	...
	for (vtxarray *va = FIRSTVA; va; va = NEXTVA)
	{
		...
		if (refracting)
		{
			if ((refracting < 0 ? va->geommin.z > reflectz : va->geommax.z <= reflectz) || va->occluded >= OCCLUDE_GEOM)
				continue;
			if (ishiddencube(va->o, va->size))
				continue;
		}
		else if (reflecting)
		{
			if (va->geommax.z <= reflectz)
				continue;
		}
...
{
	if (causticspass && (!causticscale || !causticmillis))
		causticspass = 0;

	bool mainpass = !reflecting && !refracting && !drawtex && !glaring, doOQ = oqfrags && oqgeom && mainpass,
		 doZP = doOQ && zpass, doSM = shadowmap && !drawtex && !glaring;
	renderstate cur;
	if (mainpass)
	{
		flipqueries();
	...
	if (doZP)
	{
		...
		for (vtxarray *va = visibleva; va; va = va->next)
		{
			if (!va->texs || va->occluded < OCCLUDE_GEOM)
				continue;
			else if ((va->parent && va->parent->occluded >= OCCLUDE_BB) || (va->query && checkquery(va->query)))
			{
	...
	if (foggedvas.length())
		renderfoggedvas(cur, doOQ && !zpass);
...
void renderalphageom(bool fogpass)
{
	...
	for (vtxarray *va = FIRSTVA; va; va = NEXTVA)
	{
		...
		if (refracting)
		{
			if ((refracting < 0 ? va->geommin.z > reflectz : va->geommax.z <= reflectz) || va->occluded >= OCCLUDE_BB)
				continue;
			if (ishiddencube(va->o, va->size))
				continue;
	...
	loop(front, 2) if (front || hasback)
	{
		...
		cur.alphascale = -1;
		loopv(alphavas) if (front || alphavas[i]->alphabacktris)
			renderva(cur, alphavas[i], RENDERPASS_LIGHTMAP, fogpass);
		if (geombatches.length())
			renderbatches(cur, RENDERPASS_LIGHTMAP);
...
void findreflectedvas(vector<vtxarray *> &vas, int prevvfc = VFC_PART_VISIBLE)
{
	loopv(vas)
	{
		...
		if (prevvfc >= VFC_NOT_VISIBLE)
			va->curvfc = prevvfc;
		if (va->curvfc == VFC_FOGGED || va->curvfc == PVS_FOGGED || va->o.z + va->size <= reflectz ||
			isfoggedcube(va->o, va->size))
			continue;
		bool render = true;
		...
		else if (va->curvfc == PVS_FULL_VISIBLE)
			continue;
		if (render)
		{
			if (va->curvfc >= VFC_NOT_VISIBLE)
		...
		if (va->children.length())
			findreflectedvas(va->children, va->curvfc);
	}
}
...
void renderskyva(vtxarray *va, bool explicitonly = false)
{
	...
	drawvatris(va, explicitonly ? va->explicitsky : va->sky + va->explicitsky,
			   explicitonly ? va->skydata + va->sky : va->skydata);
...
static inline void updateskystats(vtxarray *va)
{
	...
	if (!(va->skyfaces & 0x1F) || camera1->o.z < va->skyclip)
		renderedskyclip = min(renderedskyclip, va->skyclip);
	else
		renderedskyclip = 0;
}
...
void renderreflectedskyvas(vector<vtxarray *> &vas, int prevvfc = VFC_PART_VISIBLE)
{
	loopv(vas)
	{
		...
		if (prevvfc >= VFC_NOT_VISIBLE)
			va->curvfc = prevvfc;
		if ((va->curvfc == VFC_FULL_VISIBLE && va->occluded >= OCCLUDE_BB) || va->curvfc == PVS_FULL_VISIBLE)
			continue;
		if (va->o.z + va->size <= reflectz || ishiddencube(va->o, va->size))
			continue;
		...
		if (va->children.length())
			renderreflectedskyvas(va->children, va->curvfc);
	}
}
...
bool rendersky(bool explicitonly)
{
	...
	if (reflecting)
	{
		...
		for (vtxarray *va = visibleva; va; va = va->next)
		{
			if ((va->occluded >= OCCLUDE_BB && va->skyfaces & 0x80) || !(va->sky + va->explicitsky))
				continue;

			// count possibly visible sky even if not actually rendered
			updateskystats(va);
			if (explicitonly && !va->explicitsky)
				continue;
