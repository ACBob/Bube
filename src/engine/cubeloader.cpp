...
struct cubeloader
{
	...
	{
		if (ce.type >= 7)
			ce.type++; // grenade ammo
		if (ce.type >= 8)
			ce.type++; // pistol ammo
		if (ce.type == 16)
			ce.type = ET_MAPMODEL;
		else if (ce.type >= ET_MAPMODEL && ce.type < 16)
			ce.type++;
		if (ce.type >= ET_ENVMAP)
			ce.type++;
		if (ce.type >= ET_PARTICLES)
			ce.type++;
		if (ce.type >= ET_SOUND)
			ce.type++;
		if (ce.type >= ET_SPOTLIGHT)
			ce.type++;
		...
		switch (e.type)
		{
			...
			case ET_LIGHT:
				e.attr1 *= 4;
				if (!ce.attr3 && !ce.attr4)
				{
					e.attr3 = e.attr4 = e.attr2;
					break;
				}
				// fall through
			default:
	...
	int getfloorceil(c_sqr &s, int &floor, int &ceil)
	{
		...
		switch (s.type)
		{
			case C_SOLID:
				...
				break;
			case C_FHF:
				floor -= (cap = neighbours(s) & (~3)) / 4;
				break;
			case C_CHF:
				ceil += (cap = neighbours(s) & (~3)) / 4;
				break;
		}
		return cap;
	}
	...
	bool cornersolid(int z, c_sqr *s)
	{
		return s->type == C_SOLID || z < s->floor || z >= s->ceil;
	}
	...
	void create_cubes()
	{
		...
		for (int x = x0 - 1; x <= x1 + 1; x++)
			for (int y = y0 - 1; y <= y1 + 1; y++)
			{
				...
				{
					cube &c = getcube(x, y, z);
					c.texture[O_LEFT] = c.texture[O_RIGHT] = c.texture[O_BACK] = c.texture[O_FRONT] =
						s.type != C_SOLID && z < ceil ? s.wtex : s.utex;
					...
					else if (s.type == C_CORNER)
					{
						...
						if (tc && lc && !bc && !rc)
							createcorner(c, 0, 8, 0, 0); // TOP LEFT
						else if (tc && !lc && !bc && rc)
							createcorner(c, 0, 0, 0, 8); // TOP RIGHT
						else if (!tc && lc && bc && !rc)
							createcorner(c, 0, 8, 8, 8); // BOT LEFT
						else if (!tc && !lc && bc && rc)
							createcorner(c, 8, 8, 0, 8); // BOT RIGHT
						else							 // fix texture on ground of a corner
						{
							if (ts->floor - 1 == z && bs->floor - 1 != z)
							{
								c.texture[O_TOP] = ts->ftex;
							}
							else if (ts->floor - 1 != z && bs->floor - 1 == z)
							{
								c.texture[O_TOP] = bs->ftex;
							}
							if (ts->ceil == z && bs->ceil != z)
							{
								c.texture[O_BOTTOM] = ts->ctex;
							}
							else if (ts->ceil != z && bs->ceil == z)
							{
								c.texture[O_BOTTOM] = bs->ctex;
							}
						}
					}
				}
	...
	void load_cube_world(char *mname)
	{
		...
		if (!f)
		{
			conoutf(CON_ERROR, "could not read cube map %s", cgzname);
			return;
		}
		c_header hdr;
		...
		if (strncmp(hdr.head, "CUBE", 4))
		{
			...
			mod = true;
		if (hdr.version > 5 && !mod)
		{
			conoutf(CON_ERROR, "map %s requires a newer version of the Cube 1 importer", cgzname);
			delete f;
			return;
		}
		if (!haslocalclients())
