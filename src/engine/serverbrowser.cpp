...
struct serverinfo : pingattempts
{
	...
	serverinfo() : port(-1), numplayers(0), resolved(UNRESOLVED), keep(false), password(NULL)
	{
...
void addserver(const char *name, int port, const char *password, bool keep)
{
	...
	loopv(servers)
	{
		...
		if (strcmp(s->name, name) || s->port != port)
			continue;
...
const char *showservers(g3d_gui *cgui, uint *header, int pagemin, int pagemax)
{
	...
	for (int start = 0; start < servers.length();)
	{
		...
		loopi(10)
		{
			if (!game::serverinfostartcolumn(cgui, i))
				break;
			for (int j = start; j < end; j++)
			{
				if (!i && j + 1 - start >= pagemin && (j + 1 - start >= pagemax || cgui->shouldtab()))
				{
					end = j;
					break;
				}
				...
				if (si.address.host == ENET_HOST_ANY)
					sdesc = "[unknown host]";
				else if (si.ping == serverinfo::WAITING)
					sdesc = "[waiting for response]";
				if (game::serverinfoentry(cgui, i, si.name, si.port, sdesc, si.map, sdesc == si.sdesc ? si.ping : -1,
										  si.attr, si.numplayers))
					sc = &si;
			}
...
void updatefrommaster()
{
	...
	if (data.empty())
		conoutf(CON_ERROR, "master server not replying");
	else
	{
		clearservers();
		...
		while (char *end = (char *)memchr(line, '\n', data.length() - (line - data.getbuf())))
		{
			*end = '\0';
			...
			if (matchstring(line, cmdlen, "addserver"))
			{
				string ip;
				int port;
				if (sscanf(args, "%100s %d", ip, &port) == 2)
					addserver(ip, port);
			}
			else if (matchstring(line, cmdlen, "echo"))
				conoutf("\f1%s", args);
...
ICOMMAND(keepserver, "sis", (const char *name, int *port, const char *password),
