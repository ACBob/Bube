...
struct soundconfig
{
	...
	{
		if (flags & SND_NO_ALT || numslots <= 1)
			return slots;
		if (flags & SND_USE_ALT)
			return slots + 1 + rnd(numslots - 1);
...
soundchannel &newchannel(int n, soundslot *slot, const vec *loc = NULL, extentity *ent = NULL, int flags = 0,
						 int radius = 0)
{
	if (ent)
	{
...
static Mix_Chunk *loadwavscaled(const char *name)
{
	...
	int mixerchannels = 0;
	if (!Mix_QuerySpec(&mixerfreq, &mixerformat, &mixerchannels))
		return NULL;
	...
	if (!audiobuf)
		SDL_LoadWAV(findfile(name, "rb"), &spec, &audiobuf, &audiolen);
	if (!audiobuf)
		return NULL;
	...
	if (scale >= 2)
	{
		...
		if (scalebuf)
		{
			...
			{
				case 1:
					scalewav((uchar *)scalebuf, (uchar *)audiobuf, audiolen, scale);
					break;
				case 2:
					scalewav((ushort *)scalebuf, (ushort *)audiobuf, audiolen, scale);
					break;
				case 4:
					scalewav((uint *)scalebuf, (uint *)audiobuf, audiolen, scale);
					break;
				case 8:
					scalewav((ullong *)scalebuf, (ullong *)audiobuf, audiolen, scale);
					break;
				default:
					SDL_free(scalebuf);
					scalebuf = NULL;
...
void registersound(char *name, char *filepath, int *vol)
{
	intret(gamesounds.addsound(name, filepath, *vol, 0));
}
COMMAND(registersound, "ssi");

void mapsound(char *name, char *filepath, int *vol, int *maxuses)
{
	intret(mapsounds.addsound(name, filepath, *vol, *maxuses < 0 ? 0 : max(1, *maxuses)));
}
COMMAND(mapsound, "ssii");

void altsound(char *name, char *filepath, int *vol)
{
	gamesounds.addalt(name, filepath, *vol);
}
COMMAND(altsound, "ssi");

void altmapsound(char *name, char *filepath, int *vol)
{
	mapsounds.addalt(name, filepath, *vol);
}
...
int playsoundid(int n, const vec *loc, extentity *ent, int flags, int loops, int fade, int chanid, int radius,
				int expire)
{
	...
	if (!slot.sample->chunk && !slot.sample->load())
		return -1;

	if (dbgsound)
		conoutf(CON_DEBUG, "sound: %s", slot.sample->name);
...
{
	if (!gamesounds.configs.inrange(n) || !channels.inrange(chanid) || !channels[chanid].inuse ||
		!gamesounds.playing(channels[chanid], gamesounds.configs[n]))
		return false;
	if (dbgsound)
		conoutf(CON_DEBUG, "stopsound: %s", channels[chanid].slot->sample->name);
...
int playsound(const char *s, const vec *loc, int vol, int flags, int loops, int fade, int chanid, int radius,
			  int expire)
{
	if (!vol)
		vol = 100;
	...
	if (id < 0)
		id = gamesounds.addsound(s, s, vol);
...
void initmumble()
{
	...
	if (mumblelink)
	{
		...
		if (mumbleinfo)
			wcsncpy(mumbleinfo->name, L"Sauerbraten", 256);
	}
	#elif defined(_POSIX_SHARED_MEMORY_OBJECTS)
