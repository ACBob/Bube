...
struct LightMap
{
	...
	LightMap()
		: type(LM_DIFFUSE), bpp(3), tex(-1), offsetx(-1), offsety(-1), lightmaps(0), lumels(0), unlitx(-1), unlity(-1),
		  data(NULL)
	{
	}
...
struct LightMapTexture
{
	...
	LightMapTexture() : w(0), h(0), type(LM_DIFFUSE), id(0), unlitx(-1), unlity(-1)
	{
	}
};
...
extern void lerpnormal(float u, float v, const lerpvert *lv, int numv, lerpbounds &start, lerpbounds &end, vec &normal,
					   vec &nstep);
