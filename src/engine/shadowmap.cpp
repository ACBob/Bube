...
HVARFR(shadowmapambient, 0, 0, 0xFFFFFF, {
	if (shadowmapambient <= 255)
		shadowmapambient |= (shadowmapambient << 8) | (shadowmapambient << 16);
	shadowmapambientcolor =
		bvec((shadowmapambient >> 16) & 0xFF, (shadowmapambient >> 8) & 0xFF, shadowmapambient & 0xFF);
});
...
static struct shadowmaptexture : rendertarget
{
	...
	bool dorender()
	{
		...
		shadowmatrix.ortho(-shadowmapradius, shadowmapradius, -shadowmapradius, shadowmapradius, -shadowmapdist,
						   shadowmapdist);
		shadowmatrix.mul(matrix3(vec(1, 0, 0), vec(0, 1, 0), vec(skewdir.x, skewdir.y, 1)));
		shadowmatrix.translate(skewdir.x * shadowmapheight + shadowoffset.x,
							   skewdir.y * shadowmapheight + shadowoffset.y + dir.magnitude(), -shadowmapheight);
		...
		GLOBALPARAMF(shadowmapbias, -shadowmapbias / float(shadowmapdist),
...
{
	if (o.z + zrad <= shadowfocus.z - shadowmapdist || o.z - zrad >= shadowfocus.z)
		return false;
	...
	if (!shadowmaptex.addblurtiles(x1, y1, x2, y2, 2))
		return false;
...
{
	if (!shadowmap || !shadowmapcasters)
		return false;

	if (va->shadowmapmax.z <= shadowfocus.z - shadowmapdist || va->shadowmapmin.z >= shadowmapmaxz)
		return false;

	float xyrad = SQRT2 * 0.5f * max(va->shadowmapmax.x - va->shadowmapmin.x, va->shadowmapmax.y - va->shadowmapmin.y),
		  zrad = 0.5f * (va->shadowmapmax.z - va->shadowmapmin.z), x1, y1, x2, y2;
	if (xyrad < 0 || zrad < 0)
		return false;
...
bool isshadowmapcaster(const vec &o, float rad)
{
	...
	if (!shadowmapping || o.z + rad <= shadowfocus.z - shadowmapdist || o.z - rad >= shadowfocus.z ||
...
void pushshadowmap()
{
	if (!shadowmap || !shadowmaptex.rendertex)
		return;
	...
	{
		r = shadowmapambientcolor[0];
		g = shadowmapambientcolor[1];
		b = shadowmapambientcolor[2];
	}
