...
void modifyoctaentity(int flags, int id, extentity &e, cube *c, const ivec &cor, int size, const ivec &bo,
					  const ivec &br, int leafsize, vtxarray *lastva = NULL)
{
...
static bool modifyoctaent(int flags, int id, extentity &e)
{
	...
	if (e.type == ET_LIGHT)
		clearlightcache(id);
	else if (e.type == ET_PARTICLES)
		clearparticleemitters();
	else if (flags & MODOE_LIGHTENT)
		lightent(e);
	return true;
}
...
{
	modifyoctaent(MODOE_ADD | MODOE_UPDATEBB | MODOE_LIGHTENT, id);
}
...
static inline void findents(octaentities &oe, int low, int high, bool notspawned, const vec &pos, const vec &invradius,
							vector<int> &found)
{
	vector<extentity *> &ents = entities::getents();
	loopv(oe.other)
...
static inline void findents(cube *c, const ivec &o, int size, const ivec &bo, const ivec &br, int low, int high,
							bool notspawned, const vec &pos, const vec &invradius, vector<int> &found)
{
...
void findents(int low, int high, bool notspawned, const vec &pos, const vec &radius, vector<int> &found)
{
	...
	if (c->children && 1 << scale >= octaentsize)
		findents(c->children, ivec(bo).mask(~((2 << scale) - 1)), 1 << scale, bo, br, low, high, notspawned, pos,
				 invradius, found);
}
...
{
	undoent *ue = u->ents();
	loopi(u->numents) entedit(ue[i].i, (entity &)e = ue[i].e);
}
...
void entdrag(const vec &ray)
{
	...
	entfocus(entgroup.last(), entselectionbox(e, eo, es);
			 ...
			 if (!editmoveplane(e.o, ray, d, eo[d] + (dc ? es[d] : 0), handle, v, entmoving == 1)) return;

			 ivec g(v); int z = g[d] & (~(sel.grid - 1)); g.add(sel.grid / 2).mask(~(sel.grid - 1)); g[d] = z;

			 r = (entselsnap ? g[R[d]] : v[R[d]]) - e.o[R[d]]; c = (entselsnap ? g[C[d]] : v[C[d]]) - e.o[C[d]];);
	...
	switch (e.type)
	{
		...
		case ET_SPOTLIGHT:
			if (e.attached)
			{
				...
				if (!radius)
					radius = 2 * e.o.dist(e.attached->o);
...
extentity *newentity(bool local, const vec &o, int type, int v1, int v2, int v3, int v4, int v5, int &idx)
{
	...
	if (local)
	{
		...
		{
			conoutf(CON_ERROR, "too many entities");
			return NULL;
		}
	}
	else
...
void printent(extentity &e, char *buf, int len)
{
	...
	nformatstring(buf, len, "%s %d %d %d %d %d", entities::entname(e.type), e.attr1, e.attr2, e.attr3, e.attr4,
				  e.attr5);
}
...
ICOMMAND(entselect, "e", (uint * body),
...
float gatherspawninfos(dynent *d, int tag, vector<spawninfo> &spawninfos)
{
	...
	loopv(ents)
	{
		...
		if (e.type != ET_PLAYERSTART || e.attr2 != tag)
			continue;
...
static const extentity *poprandomspawn(vector<spawninfo> &spawninfos, float &total)
{
	...
	if (total > 0.0f)
	{
		...
		do
			x -= spawninfos[index].weight;
		while (x > 0 && ++index < spawninfos.length() - 1);
	}
	else
		index = rnd(spawninfos.length());
...
void findplayerspawn(dynent *d, int forceent, int tag)
{
	...
	d->pitch = 0;
	d->roll = 0;
	if (ents.inrange(forceent) && tryspawn(d, *ents[forceent]))
		return;
	vector<spawninfo> spawninfos;
	float total = gatherspawninfos(d, tag, spawninfos);
	while (const extentity *e = poprandomspawn(spawninfos, total))
		if (tryspawn(d, *e))
			return;
