...
struct avisegmentinfo
{
	...
	avisegmentinfo(stream::offset offset, int firstindex)
		: offset(offset), videoindexoffset(0), soundindexoffset(0), firstindex(firstindex), videoindexsize(0),
		  soundindexsize(0), indexframes(0), videoframes(0), soundframes(0)
	{
	}
};
...
struct aviwriter
{
	...
	void close()
	{
		...
		if (dbgmovie)
			conoutf(CON_DEBUG, "fileframes: sound=%d, video=%d+%d(dups)\n", soundframes, videoframes,
	...
	aviwriter(const char *name, uint w, uint h, uint fps, bool sound)
		: f(NULL), yuv(NULL), videoframes(0), totalsize(0), videow(w & ~1), videoh(h & ~1), videofps(fps),
		  soundfrequency(0), soundchannels(0), soundformat(0)
	{
	...
	bool open()
	{
		...
		listchunk("LIST", "INFO");
		const char *software = "Cube 2: Sauerbraten"; // TODO: I'm not sure what this does, Should I rebrand it?
	...
	static inline void boxsample(const uchar *src, const uint stride, const uint area, const uint w, uint h,
								 const uint xlow, const uint xhigh, const uint ylow, const uint yhigh, uint &bdst,
								 uint &gdst, uint &rdst)
	{
...
namespace recorder
{
	...
	float calcquality()
	{
		return 1.0f - float(dps) / float(dps + file->videofps); // strictly speaking should lock to read dps
																// - 1.0=perfect, 0.5=half of frames are beingdropped
	}
	...
	void start(const char *filename, int videofps, int videow, int videoh, bool sound)
	{
		...
		conoutf("movie recording to: %s %dx%d @ %dfps%s", file->filename, file->videow, file->videoh, file->videofps,
	...
	void readbuffer(videobuffer &m, uint nextframe)
	{
		...
		if (usefbo)
		{
			...
			if (accelyuv)
			{
				...
				SETSHADER(moviey);
				screenquadflipped(m.w / float(scalew), m.h / float(scaleh));
				...
				glReadPixels(m.w / 4, m.h / 2, m.w / 8, m.h / 2, GL_BGRA, GL_UNSIGNED_BYTE,
							 &m.video[planesize + planesize / 4]);
	...
	void drawhud()
	{
		...
		draw_textf("recorded %.1f%s %d%%", w * 3 - 10 * FONTH, h * 3 - FONTH - FONTH * 3 / 2, totalsize, unit,
				   int(calcquality() * 100));
