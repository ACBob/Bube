		...
		bool loadiqmmeshes(const char *filename, const iqmheader &hdr, uchar *buf)
		{
			...
			loopi(hdr.num_vertexarrays)
			{
				...
				switch (va.type)
				{
					case IQM_POSITION:
						...
						lilswap(vpos, 3 * hdr.num_vertexes);
						break;
					case IQM_NORMAL:
						if (va.format != IQM_FLOAT || va.size != 3)
							return false;
						vnorm = (float *)&buf[va.offset];
						lilswap(vnorm, 3 * hdr.num_vertexes);
						break;
					case IQM_TANGENT:
						if (va.format != IQM_FLOAT || va.size != 4)
							return false;
						vtan = (float *)&buf[va.offset];
						lilswap(vtan, 4 * hdr.num_vertexes);
						break;
					case IQM_TEXCOORD:
						if (va.format != IQM_FLOAT || va.size != 2)
							return false;
						vtc = (float *)&buf[va.offset];
						lilswap(vtc, 2 * hdr.num_vertexes);
						break;
			...
			loopi(hdr.num_meshes)
			{
				...
				if (m->numverts)
				{
					...
					if (vtan)
						m->bumpverts = new bumpvert[m->numverts];
					if (!vindex || !vweight)
					{
						blendcombo c;
						c.finalize(0);
				...
				float *mpos = vpos + 3 * fv, *mnorm = vnorm ? vnorm + 3 * fv : NULL,
					  *mtan = vtan ? vtan + 4 * fv : NULL, *mtc = vtc ? vtc + 2 * fv : NULL;
				...
				loopj(a.num_frames)
				{
					...
					loopk(skel->numbones)
					{
						...
						if (adjustments.inrange(k))
							adjustments[k].adjust(frame[k]);
						boneinfo &b = skel->bones[k];
						frame[k].mul(b.invbase);
						if (b.parent >= 0)
							frame[k].mul(skel->bones[b.parent].base, dualquat(frame[k]));
