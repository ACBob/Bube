...
bool hasVAO = false, hasFBO = false, hasAFBO = false, hasDS = false, hasTF = false, hasTRG = false, hasTSW = false,
	 hasS3TC = false, hasFXT1 = false, hasLATC = false, hasRGTC = false, hasAF = false, hasFBB = false, hasUBO = false,
	 hasMBR = false;
...
void gl_checkextensions()
{
	...
	else if (hasext("GL_EXT_framebuffer_object"))
	{
		...
		if (hasext("GL_EXT_framebuffer_blit"))
		{
			...
			hasFBB = true;
			if (dbgexts)
				conoutf(CON_INIT, "Using GL_EXT_framebuffer_blit extension.");
		}
	...
	if (ati)
	{
		...
		if (hasTF && hasTRG)
			fpdepthfx = 1;
		// On Catalyst 10.2, issuing an occlusion query on the first draw using a given cubemap texture causes a nasty
		// crash
		ati_cubemap_bug = 1;
	}
	else if (nvidia)
	{
		...
		if (glversion < 300 && !hasext("GL_EXT_gpu_shader4"))
			filltjoints = 0; // DX9 or less NV cards seem to not cause many sparklies
	...
	if (glversion >= 300 || hasext("GL_ARB_map_buffer_range"))
	{
		...
		hasMBR = true;
		if (glversion < 300 && dbgexts)
			conoutf(CON_INIT, "Using GL_ARB_map_buffer_range.");
	}
	...
	if (glversion >= 310 || hasext("GL_ARB_uniform_buffer_object"))
	{
		...
		if (glversion < 310 && dbgexts)
			conoutf(CON_INIT, "Using GL_ARB_uniform_buffer_object extension.");
	}
	...
	if (glversion >= 300 || hasext("GL_ARB_vertex_array_object"))
	{
		...
		hasVAO = true;
		if (glversion < 300 && dbgexts)
			conoutf(CON_INIT, "Using GL_ARB_vertex_array_object extension.");
	}
	else if (hasext("GL_APPLE_vertex_array_object"))
	{
		...
		hasVAO = true;
		if (dbgexts)
			conoutf(CON_INIT, "Using GL_APPLE_vertex_array_object extension.");
	}
	...
	if (glversion >= 330 || hasext("GL_ARB_texture_swizzle") || hasext("GL_EXT_texture_swizzle"))
	{
		...
		if (glversion < 330 && dbgexts)
			conoutf(CON_INIT, "Using GL_ARB_texture_swizzle extension.");
	}
			 ...
			 hasext("GL_ANGLE_texture_compression_dxt5"))
	{
		hasS3TC = true;
		if (dbgexts)
			conoutf(CON_INIT, "Using GL_EXT_texture_compression_dxt1 extension.");
	}
	if (hasext("GL_3DFX_texture_compression_FXT1"))
	{
		...
		if (dbgexts)
			conoutf(CON_INIT, "Using GL_3DFX_texture_compression_FXT1.");
	}
	if (hasext("GL_EXT_texture_compression_latc"))
	{
		hasLATC = true;
		if (dbgexts)
			conoutf(CON_INIT, "Using GL_EXT_texture_compression_latc extension.");
	}
	if (glversion >= 300 || hasext("GL_ARB_texture_compression_rgtc") || hasext("GL_EXT_texture_compression_rgtc"))
	{
		hasRGTC = true;
		if (glversion < 300 && dbgexts)
			conoutf(CON_INIT, "Using GL_ARB_texture_compression_rgtc extension.");
	}
...
bool isthirdperson()
{
	return player != camera1 || detachedcamera || reflecting;
}
...
void calcspherescissor(const vec &center, float size, float &sx1, float &sy1, float &sx2, float &sy2)
{
	...
	if (reflecting)
		worldpos.z = 2 * reflectz - worldpos.z;
...
static void setfog(int fogmat, float below = 1, int abovemat = MAT_AIR)
{
	...
	if (below < 1)
		blendfog(abovemat, 1 - below, 1 - logblend, curfogstart, curfogend, curfogcolor);
...
void drawreflection(float z, bool refract, int fogdepth, const bvec &col)
{
	...
	if (reflectclip && z >= 0)
	{
		...
		{
			zclip = z + zoffset;
			if (camera1->o.z <= zclip)
				zclip = z;
		}
		else
		{
			zclip = z - zoffset;
			if (camera1->o.z >= zclip && camera1->o.z <= z + 4.0f)
				zclip = z;
			if (reflecting)
				zclip = 2 * z - zclip;
		}
		plane clipplane;
		invcammatrix.transposedtransform(plane(0, 0, refracting > 0 ? 1 : -1, refracting > 0 ? -zclip : zclip),
										 clipplane);
	...
	if (refracting && z >= 0 && !isthirdperson() &&
		fabs(camera1->o.z - z) <= 0.5f * (player->eyeheight + player->aboveeye))
	{
		...
		if (reflectclip)
		{
			matrix4 avatarclip;
			plane clipplane;
			invcammatrix.transposedtransform(plane(0, 0, refracting, reflectclipavatar / 4.0f - refracting * z),
											 clipplane);
			avatarclip.clip(clipplane, avatarproj);
			projmatrix = avatarclip;
		}
		else
			projmatrix = avatarproj;
		setcamprojmatrix(false, true);
		game::renderavatar();
		projmatrix = oldprojmatrix;
		setcamprojmatrix(false, true);
...
void drawcubemap(int size, const vec &o, float yaw, float pitch, const cubemapside &side, bool onlysky)
{
	...
	if (!side.flipx || !side.flipy)
		projmatrix.scalexy(!side.flipx ? -1 : 1, !side.flipy ? -1 : 1);
	if (side.swapxy)
	{
	...
	if (onlysky)
		drawskybox(farplane, false, true);
	else
	{
...
void clipminimap(ivec &bbmin, ivec &bbmax, cube *c = worldroot, const ivec &co = ivec(0, 0, 0),
...
void drawminimap()
{
	...
	cmcamera.type = ENT_CAMERA;
	cmcamera.o =
		vec(minimapcenter.x, minimapcenter.y, max(minimapcenter.z + minimapradius.z + 1, float(minimapheight)));
	cmcamera.yaw = 0;
	cmcamera.pitch = -90;
	cmcamera.roll = 0;
	camera1 = &cmcamera;
...
void addmotionblur()
{
	if (!motionblur || max(screenw, screenh) > hwtexsize)
		return;
	...
	gle::colorf(1, 1, 1,
				lastmotion ? pow(motionblurscale, max(float(lastmillis - lastmotion) / motionblurmillis, 1.0f)) : 0);
	screenquad(1, 1);
...
void gl_drawframe()
{
	...
	if (isliquid(fogmat & MATF_VOLUME))
	{
		...
		if (camera1->o.z < z + 1)
			fogblend = min(z + 1 - camera1->o.z, 1.0f);
		else
			fogmat = abovemat;
	...
	addglare();
	if (isliquid(fogmat & MATF_VOLUME))
		drawfogoverlay(fogmat, fogblend, abovemat);
	renderpostfx();
	...
	if (!hidehud && !mainmenu)
	{
		if (!hidestats)
		{
			...
			if (wallclock)
			{
				...
				static string buf;
				if (localvals && strftime(buf, sizeof(buf),
										  wallclocksecs ? (wallclock24 ? "%H:%M:%S" : "%I:%M:%S%p")
														: (wallclock24 ? "%H:%M" : "%I:%M%p"),
										  localvals))
				{
			...
			if (editmode || showeditstats)
			{
				...
				int nextstats[8] = {vtris * 100 / max(wtris, 1),
				...
				abovehud -= 2 * FONTH;
				draw_textf("wtr:%dk(%d%%) wvt:%dk(%d%%) evt:%dk eva:%dk", FONTH / 2, abovehud, wtris / 1024,
						   curstats[0], wverts / 1024, curstats[1], curstats[2], curstats[3]);
				draw_textf("ond:%d va:%d gl:%d(%d) oq:%d lm:%d rp:%d pvs:%d", FONTH / 2, abovehud + FONTH,
						   allocnodes * 8, allocva, curstats[4], curstats[5], curstats[6], lightmaps.length(),
						   curstats[7], getnumviewcells());
				limitgui = abovehud;
			}
			...
			{
				abovehud -= FONTH;
				draw_textf("cube %s%d%s", FONTH / 2, abovehud, selchildcount < 0 ? "1/" : "", abs(selchildcount),
