...
static inline bool htcmp(const vec &v, const normalgroup &n)
{
	return v == n.pos;
}
...
static inline bool findtnormal(const normalgroup &g, const vec &surface, vec &v)
{
	...
	for (int cur = g.tnormals; cur >= 0;)
	{
		...
		static const vec flats[6] = {vec(-1, 0, 0), vec(1, 0, 0),  vec(0, -1, 0),
...
void addnormals(cube &c, const ivec &o, int size)
{
	...
	loopi(6) if ((vis = visibletris(c, i, o, size)))
	{
		...
		if (!numplanes)
			loopk(numverts) norms[k] = addnormal(pos[k], i);
		else if (numplanes == 1)
			loopk(numverts) norms[k] = addnormal(pos[k], planes[0]);
		else
		{
			...
			for (int k = 3; k < numverts; k++)
				norms[k] = addnormal(pos[k], planes[1]);
		}
...
void calclerpverts(const vec2 *c, const vec *n, lerpvert *lv, int &numv)
{
	...
	loopj(numv)
	{
		if (j)
		{
			if (c[j] == c[j - 1] && n[j] == n[j - 1])
				continue;
			if (j == numv - 1 && c[j] == c[0] && n[j] == n[0])
				continue;
		}
		lv[i].normal = n[j];
		lv[i].tc = c[j];
...
void initlerpbounds(float u, float v, const lerpvert *lv, int numv, lerpbounds &start, lerpbounds &end)
{
	...
	loopi(numv - 1)
	{
		if (lv[i + 1].tc.y < first->tc.y)
		{
			second = first;
			first = &lv[i + 1];
		}
		else if (!second || lv[i + 1].tc.y < second->tc.y)
			second = &lv[i + 1];
	}

	if (int(first->tc.y) < int(second->tc.y))
	{
		start.min = end.min = first;
	}
	else if (first->tc.x > second->tc.x)
	{
		start.min = second;
		end.min = first;
	}
...
void updatelerpbounds(float v, const lerpvert *lv, int numv, lerpbounds &start, lerpbounds &end)
{
	if (v >= start.max->tc.y)
	{
		const lerpvert *next = start.winding > 0 ? (start.max == lv ? &lv[numv - 1] : start.max - 1)
	...
	if (v >= end.max->tc.y)
	{
		const lerpvert *next = end.winding > 0 ? (end.max == &lv[numv - 1] ? lv : end.max + 1)
...
void lerpnormal(float u, float v, const lerpvert *lv, int numv, lerpbounds &start, lerpbounds &end, vec &normal,
				vec &nstep)
{
