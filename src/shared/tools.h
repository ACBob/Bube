...
template <size_t N> inline void vformatstring(char (&d)[N], const char *fmt, va_list v)
{
...
template <size_t N> inline char *copystring(char (&d)[N], const char *s)
{
...
template <size_t N> inline char *concatstring(char (&d)[N], const char *s)
{
...
template <size_t N> inline char *prependstring(char (&d)[N], const char *s)
{
...
template <class T> inline void memclear(T *p, size_t n)
{
	memset((void *)p, 0, n * sizeof(T));
}
template <class T> inline void memclear(T &p)
{
	memset((void *)&p, 0, sizeof(T));
}
template <class T, size_t N> inline void memclear(T (&p)[N])
{
	memset((void *)p, 0, N * sizeof(T));
}
...
template <class T> struct databuf
{
	...
	template <class U> databuf(T *buf, U maxlen) : buf(buf), len(0), maxlen((int)maxlen), flags(0)
	{
	}
...
template <class T> static inline float heapscore(const T &n)
{
	return n;
}
...
struct sortnameless
{
	...
	template <class T> bool operator()(T *x, T *y) const
	{
		return sortless()(x->name, y->name);
	}
	template <class T> bool operator()(const T *x, const T *y) const
...
template <class T, class F> static inline void insertionsort(T *start, T *end, F fun)
{
...
template <class T, class F> static inline void insertionsort(T *buf, int n, F fun)
{
	insertionsort(buf, buf + n, fun);
}

template <class T> static inline void insertionsort(T *buf, int n)
{
	insertionsort(buf, buf + n, sortless());
}

template <class T, class F> static inline void quicksort(T *start, T *end, F fun)
{
	while (end - start > 10)
	{
...
template <class T, class F> static inline void quicksort(T *buf, int n, F fun)
{
	quicksort(buf, buf + n, fun);
}

template <class T> static inline void quicksort(T *buf, int n)
{
...
template <size_t N> inline char *copystring(char (&d)[N], const stringslice &s)
{
	return copystring(d, s, N);
}
...
template <class H, class E, class K, class T> struct hashbase
{
	...
	template <class U> T &insert(uint h, const U &key)
	{
		chain *c = insert(h);
		H::setkey(c->elem, key);
...
#define HTFIND(success, fail)                                                                                          \
	...
	template <class U> T *access(const U &key)
	{
		HTFIND(&, NULL);
	}

	template <class U, class V> T &access(const U &key, const V &elem)
	{
		HTFIND(, insert(h, key) = elem);
	}

	template <class U> T &operator[](const U &key)
	{
		HTFIND(, insert(h, key));
	}

	template <class U> T &find(const U &key, T &notfound)
	{
		HTFIND(, notfound);
	}

	template <class U> const T &find(const U &key, const T &notfound)
	{
		HTFIND(, notfound);
	}

	template <class U> bool remove(const U &key)
	{
...
template <class T> struct hashset : hashbase<hashset<T>, T, T, T>
{
	...
	template <class V> T &add(const V &elem)
	{
...
template <class T> struct hashnameset : hashbase<hashnameset<T>, T, const char *, T>
{
	...
	template <class U> static inline const char *getkey(const U &elem)
	{
		return elem.name;
	}
	template <class U> static inline const char *getkey(U *elem)
	{
		return elem->name;
	}
	static inline T &getdata(T &elem)
...
template <class K, class T> struct hashtable : hashbase<hashtable<K, T>, hashtableentry<K, T>, K, T>
{
	...
	template <class U> static inline void setkey(elemtype &elem, const U &key)
	{
		elem.key = key;
	}
};
...
template <class T, int SIZE> struct queue
{
	...
	T remove(int offset)
	{
		...
		if (head + offset >= SIZE)
			for (int i = head + offset - SIZE + 1; i < tail; i++)
				data[i - 1] = data[i];
		else if (head < tail)
			for (int i = head + offset + 1; i < tail; i++)
				data[i - 1] = data[i];
		else
		{
			for (int i = head + offset + 1; i < SIZE; i++)
				data[i - 1] = data[i];
			data[SIZE - 1] = data[0];
...
struct stream
{
	...
	template <class T> size_t put(const T *v, size_t n)
	{
	...
	template <class T> size_t get(T *v, size_t n)
	{
...
template <size_t N> static inline void getstring(char (&t)[N], ucharbuf &p)
{
	getstring(t, p, N);
}
