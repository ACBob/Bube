...
#ifndef STANDALONE
		if (dbgzip)
			conoutf(CON_DEBUG, "%s: file %s, size %d, compress %d, flags %x", archname, name, hdr.uncompressedsize,
...
static void mountzip(ziparchive &arch, vector<zipfile> &files, const char *mountdir, const char *stripdir)
{
	...
	loopv(files)
	{
		...
		formatstring(fname, "%s%s", mdir,
					 striplen && !strncmp(f.name, stripdir, striplen) ? &f.name[striplen] : f.name);
...
struct zipstream : stream
{
	...
	void stopreading()
	{
		...
		if (dbgzip)
			conoutf(CON_DEBUG,
					info->compressedsize ? "%s: zfile.total_out %u, info->size %u" : "%s: reading %u, info->size %u",
					info->name, info->compressedsize ? uint(zfile.total_out) : reading - info->offset, info->size);
#endif
...
#ifndef STANDALONE
					if (dbgzip)
						conoutf(CON_DEBUG, "inflate error: %s", zError(err));
#endif
					stopreading();
				}
				break;
...
#ifndef STANDALONE
ICOMMAND(addzip, "sss", (const char *name, const char *mount, const char *strip),
