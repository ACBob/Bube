...
template <int BI_DIGITS> struct bigint
{
	...
	template <int Y_DIGITS> bigint &add(const bigint<Y_DIGITS> &y)
	{
		return add(*this, y);
	}
	...
	template <int Y_DIGITS> bigint &sub(const bigint<Y_DIGITS> &y)
	{
		return sub(*this, y);
	}
...
struct gfield : gfint
{
	...
	template <int Y_DIGITS> gfield &add(const bigint<Y_DIGITS> &y)
	{
		return add(*this, y);
	}

	template <int X_DIGITS> gfield &mul2(const bigint<X_DIGITS> &x)
	{
		return add(x, x);
	}
	gfield &mul2()
	...
	template <int Y_DIGITS> gfield &sub(const bigint<Y_DIGITS> &y)
	{
		return sub(*this, y);
	}
	...
	template <int X_DIGITS> gfield &square(const bigint<X_DIGITS> &x)
	{
		return mul(x, x);
	}
	gfield &square()
	...
	template <int Y_DIGITS> gfield &mul(const bigint<Y_DIGITS> &y)
	{
		return mul(*this, y);
	}
	...
	template <int Y_DIGITS> gfield &pow(const bigint<Y_DIGITS> &y)
	{
		return pow(*this, y);
	}
...
struct ecjacobian
{
	...
	template <int Q_DIGITS> void mul(const bigint<Q_DIGITS> &q)
	{
		ecjacobian tmp(*this);
		mul(tmp, q);
	}
...
#elif GF_BITS == 521
const gfield gfield::P("1ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
					   "ffffffffffffffffffffffffffffffffffff");
const gfield ecjacobian::B("051953eb968e1c9a1f929a21a0b68540eea2da725b99b315f3b8b489918ef109e156193951ec7e937b1652c0bd3"
