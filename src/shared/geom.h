...
struct vec
{
	...
	template <class T> float dot2(const T &o) const
	{
		return x * o.x + y * o.y;
	}
	...
	template <class B> vec &madd(const vec &a, const B &b)
	{
		return add(vec(a).mul(b));
	}
	template <class B> vec &msub(const vec &a, const B &b)
	{
	...
	void orthonormalize(vec &s, vec &t) const
	{
		...
		t.sub(vec(*this).mul(dot(t))).sub(vec(s).mul(s.dot(t)));
	}
	...
	{
		return vec(((color >> 16) & 0xFF) * (1.0f / 255.0f), ((color >> 8) & 0xFF) * (1.0f / 255.0f),
...
struct vec4
{
	...
	template <class T> float dot3(const T &o) const
	{
		return x * o.x + y * o.y + z * o.z;
	}
	...
	template <class B> vec4 &madd(const vec4 &a, const B &b)
	{
		return add(vec4(a).mul(b));
	}
	template <class B> vec4 &msub(const vec4 &a, const B &b)
	{
...
struct quat : vec4
{
	...
	template <class M> void convertmatrix(const M &m)
	{
...
struct dualquat
{
	...
	dualquat(const quat &q, const vec &p)
		: real(q), dual(0.5f * (p.x * q.w + p.y * q.z - p.z * q.y), 0.5f * (-p.x * q.z + p.y * q.w + p.z * q.x),
						0.5f * (p.x * q.y - p.y * q.x + p.z * q.w), -0.5f * (p.x * q.x + p.y * q.y + p.z * q.z))
	{
	}
	...
	{
		return vec()
			.cross(real, vec().cross(real, v).add(vec(v).mul(real.w)).add(vec(dual)))
			.add(vec(dual).mul(real.w))
			.sub(vec(real).mul(dual.w))
			.mul(2)
			.add(v);
	}
...
struct matrix3
{
	...
	{
		a = vec(axis.x * axis.x * (1 - ck) + ck, axis.x * axis.y * (1 - ck) + axis.z * sk,
				axis.x * axis.z * (1 - ck) - axis.y * sk);
		b = vec(axis.x * axis.y * (1 - ck) - axis.z * sk, axis.y * axis.y * (1 - ck) + ck,
				axis.y * axis.z * (1 - ck) + axis.x * sk);
		c = vec(axis.x * axis.z * (1 - ck) + axis.y * sk, axis.y * axis.z * (1 - ck) - axis.x * sk,
...
struct matrix4x3
{
	...
	void posttranslate(const vec &p)
	{
		d.add(p);
	}
	void posttranslate(float x, float y, float z)
	{
		posttranslate(vec(x, y, z));
	}
	void posttranslate(const vec &p, float scale)
	{
		d.madd(p, scale);
	}
...
struct bvec
{
	...
	explicit bvec(const vec &v)
		: x(uchar((v.x + 1) * (255.0f / 2.0f))), y(uchar((v.y + 1) * (255.0f / 2.0f))),
	...
	{
		return bvec((((c >> 11) & 0x1F) * 527 + 15) >> 6, (((c >> 5) & 0x3F) * 259 + 35) >> 6,
					((c & 0x1F) * 527 + 15) >> 6);
	}
...
struct dvec4
{
	...
	template <class B> dvec4 &madd(const dvec4 &a, const B &b)
	{
...
extern bool linecylinderintersect(const vec &from, const vec &to, const vec &start, const vec &end, float radius,
								  float &dist);
