...
namespace gle
{
	...
	extern const char *const attribnames[MAXATTRIBS] = {"vvertex", "vcolor",   "vtexcoord0",  "vtexcoord1",
	...
	int enabled = 0;
	static int numattribs = 0, attribmask = 0, numlastattribs = 0, lastattribmask = 0, vertexsize = 0,
			   lastvertexsize = 0;
...
#define MAXVBOSIZE (4 * 1024 * 1024)
	...
	void drawquads(int offset, int count)
	{
		...
		glDrawRangeElements_(GL_TRIANGLES, offset * 4, (offset + count) * 4 - 1, count * 6, GL_UNSIGNED_SHORT,
							 (ushort *)0 + offset * 6);
	}
	...
	void begin(GLenum mode, int numverts)
	{
		...
		if (glversion >= 300 && !intel_mapbufferrange_bug)
		{
			...
			if (vbooffset + len >= MAXVBOSIZE)
			{
				...
				glBindBuffer_(GL_ARRAY_BUFFER, vbo);
			void *buf = glMapBufferRange_(GL_ARRAY_BUFFER, vbooffset, len,
			...
			if (buf == attribdata)
			{
				...
				void *dst =
					intel_mapbufferrange_bug
						? NULL
						: glMapBufferRange_(GL_ARRAY_BUFFER, vbooffset, attribbuf.length(),
											GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT);
				if (dst)
				{
